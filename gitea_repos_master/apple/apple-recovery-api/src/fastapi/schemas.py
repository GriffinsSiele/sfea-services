...
class AppleSearchData(BaseModel):
    """
    ...
    class Config:
        openapi_examples = {
            "E-mail": {
                ...
                "value": {"email": "tiuha@bk.ru"},
            },
            "Номер телефона": {
                "summary": "Поиск по номеру телефона",
                "value": {"phone": "79166367863"},
            },
            "Поиск с учетом TTL": {
...
class AppleDataRecords(BaseModel):
    """
    Модель ответа, содержит информацию о результатах поиска.
    """

    result: str = Field(description='Результат, например "tiuha@bk.ru" является AppleID')
    result_code: str = Field(description='Код результата, значение "FOUND"')
    locked: str = Field(description='Заблокирован, значение "Да" или "Нет"')
    auth: str = Field(
        description='Аутентификация, значение "Двухфакторная" или "Только пароль"'
    )
    paid: str = Field(description='Платный аккаунт, значение "Да" или "Нет"')
    multi_email: str = Field(description='Несколько E-mail, значение "Да" или "Нет"')
    not_active: str = Field(
        description='Указанный Apple ID неактивен, значение "Да" или "Нет"'
    )
    phone: Optional[str] = Field(default=None, description="Телефон")
    email: Optional[str] = Field(default=None, description="Адрес электронной почты")


class AppleSearchResponse(BaseModel):
    """
                ...
                "content": {
                    "application/json": {
                        "example": {
                            ...
                            "records": [
                                {
                                    "result": "'tiuha@bk.ru' является AppleID",
                                    "result_code": "FOUND",
                                    "locked": "Да",
                                    "auth": "Двухфакторная Аутентификация",
                                    "paid": "Да",
                                    "multi_email": "Да",
                                    "not_active": "Нет",
                                    "number": "*********41",
