...
class RouterLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _read_response_body(self, response: Response) -> str | None:
        try:
            ...
            return json.loads(response_body[0].decode()) if response_body else None
    ...
    async def log_response_body(self, request: Request, response: Response):
        ...
        logging.info(short(response_log_dict, 20))

    async def dispatch(
        self, request: Request, call_next: RequestResponseEndpoint
    ) -> Response:
        if request.url.path in settings.IGNORE_LOG_ROUTES:
