...
class ThreadManager:
    ...
    def run(self, search_manager: Type[SearchManager], *args, **kwargs):
        try:
            return self._run(search_manager, *args, **kwargs)
        except Exception as e:
            logging.fatal(e)
            exit(1)

    def _run(self, search_manager: Type[SearchManager], *args, **kwargs):
        while self.is_alive():
            payload = self.kdbq.check_queue()
            ...
            if self.mongo:
                ...
                if not session and self.rsm:
                    is_ok, sleep_time = self.rsm.solve_lack_sessions(self.mongo)
                    if not is_ok:
                        logging.info("Task returned, wait for solving lack of sessions")
                        self.kdbq.return_to_queue(payload)
                        sleep(sleep_time)
                        continue

            self._process_payload(payload, session, search_manager, *args, **kwargs)
    ...
    def _process_payload(
        ...
        **kwargs,
        ...
        try:
            ...
            self.kdbq.set_answer(payload, KeyDBResponseBuilder.ok(response))
            if self.mongo:
                self.mongo.session_success(session)
            self.count_reload += 1
            HealthCheck().checkpoint()
        except Exception as e:
            self.exception_handler.call(
                e,
                mongo=self.mongo,
                kdbq=self.kdbq,
                session=session,
