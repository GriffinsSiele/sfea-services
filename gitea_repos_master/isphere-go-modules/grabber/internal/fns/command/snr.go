...
func (t *Snr) Describe() *cli.Command {
	return &cli.Command{
		Category: "fns",
		Name:     "fns:snr",
		Action:   t.Execute,
		...
		Flags: cli.FlagsByName{
			...
			&cli.StringFlag{Name: "table", Value: "snr"},
			&cli.BoolFlag{Name: "no-cache", Value: false},
			&cli.StringFlag{Name: "url", Value: fns.SnrURL},
		},
	}
}

func (t *Snr) Execute(c *cli.Context) error {
	var (
		ctx      = c.Context
	...
	if err := fns.Invoke(ctx, database, url, noCache,
		func(obj *model.File[model.SnrDocument], flush fns.FlushFunc) error {
			var rows [][]any

			for _, document := range obj.Documents {
				for _, regime := range document.Regimes {
			...
			if err := flush(rows, []string{"id", "inn", "eshn", "usn", "ausn", "srp", "updated_at"}, schema, table); err != nil {
