...
func NewPhones() *Phones {
	return &Phones{}
}

func (t *Phones) Describe() *cli.Command {
	return &cli.Command{
		Category: "vk",
		Name:     "vk:phone",
		Action:   t.Execute,
		Usage:    "База данных утечки VK (phones)",
		Flags: cli.FlagsByName{
			...
			&cli.StringFlag{Name: "table", Value: "phones"},
			&cli.StringFlag{Name: "filename", Value: "var/vk_phones.csv"},
		},
	}
}

func (t *Phones) Execute(c *cli.Context) error {
	var (
		ctx      = c.Context
	...
	go func() {
		...
		for row := range ch {
			...
			if len(rows) > 1_000_000 {
				if _, err = conn.CopyFrom(ctx, pgx.Identifier{schema, table}, []string{"id", "vk_id", "phone"}, pgx.CopyFromRows(rows)); err != nil {
