...
@section('content')
    ...
    <div class="row">
        <div class="col-3">
            <a href="{{ route('users.edit', $user->Id) }}">Редактирование пользователя</a>
        </div>
    </div>
    ...
    {{ Form::model($user, array('route' => array('users.limits.parallel', $user->Id), 'method' => 'POST')) }}
    <div class="row">

        <div class="col-4 col-lg-3">
            {{ Form::label('ParallelQueriesLimit', 'Одновременных запроов') }}
            {{ Form::text('ParallelQueriesLimit', null, ['class' => 'form-control']) }}
        </div>
    ...
    @if(count($availableLimits)> 0)
        ...
        <form method="POST" action="{{ route('users.limits.add', $user->Id) }}">
        @csrf
        <div class="row">
            <div class="col-4 col-lg-3">
    ...
    @foreach($limits as $limit)
        <tr>
            ...
            <td><a href="{{ route('users.limits.remove', ['user'=>$user->Id, 'limit'=>$limit->Id]) }}">удалить</a></td>
        </tr>
