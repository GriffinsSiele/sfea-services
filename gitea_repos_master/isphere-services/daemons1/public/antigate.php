    ...
    $is_russian = 0)
{
    ...
    if (substr($image,0,3)=='GIF') $ext='gif';
    else
        if (substr($image,1,3)=='PNG') $ext='png';
        else
            if (ord(substr($image,0,1))==255) $ext='jpg';
            else {
                $filename = $image;
                ...
                if ($fp!=false) {
                    $image="";
                    while (!feof($fp)) $image.=fgets($fp,4096);
                    fclose($fp);
...
*/
    if ($is_verbose) echo "connecting to host...";
    ...
    if ($fp!=false) {
        ...
        if ($is_verbose) echo "could not connect to host\n";
    ...
    $sendhost = 'rucaptcha.com')
{
    ...
    if (strpos($result,"NOT_READY")!==false) {
    ...
    if ($result=='OK') {
        ...
        if ($is_verbose) echo "captcha not recognized with code $result\n";
        return $result;
    }
}

function antigate_reportgood(
    $captcha_id,
    $apikey,
    $is_verbose = false,
    $sendhost = 'rucaptcha.com')
...
//    return 'OK_REPORT_RECORDED';
    ...
    if ($result=='OK_REPORT_RECORDED') {
        if ($is_verbose) echo "captcha reported as good\n";
        return $result;
    } else {
        if ($is_verbose) echo "captcha report error\n";
        return $result;
    }
}

function antigate_reportbad(
    $captcha_id,
    $apikey,
    $is_verbose = false,
    $sendhost = 'rucaptcha.com')
...
//    return 'OK_REPORT_RECORDED';
    ...
    $result = trim($ex[0]);
    if ($result=='OK_REPORT_RECORDED') {
        if ($is_verbose) echo "captcha reported as bad\n";
        return $result;
    } else {
        if ($is_verbose) echo "captcha report error\n";
        return $result;
