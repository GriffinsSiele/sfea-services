...
$sourceaccessfilter = ""; //" AND mod(sourceaccessid,10)+1=$daemonnum";
$sessionfilter = " AND (proxyid IS NULL OR mod(session.proxyid,10)+1=$daemonnum)";

// Проверка на повторный запуск
$flock = fopen("daemon.lock", "w"); 
...
// Отмечаем просроченные запросы
db_execute($db,"update RequestNew SET status=-1, processed_at=DATE_ADD(created_at,INTERVAL 10 MINUTE) WHERE status=0 AND created_at>DATE_SUB(now(),INTERVAL 1 DAY) AND created_at<DATE_SUB(now(),INTERVAL 10 MINUTE)");
...
// Удаляем капчи с истекшим сроком
    $sql = <<<SQL
UPDATE session SET captcha='',captchaimage=NULL,statuscode='captchaexpired'
...
// AND sourceid IN (SELECT id from source WHERE captcha_format IN ('recaptcha','v2ent','v3','hcaptcha','turnstile'))
    $sql = <<<SQL
UPDATE session SET sessionstatusid=2,statuscode='',captchaimage=null
...
AND session.sourceid NOT IN (SELECT sourceid FROM session WHERE sessionstatusid=1 GROUP BY 1 HAVING count(*)>250)
...
//                            print_log($form);
                ...
                if($captcha && (substr($captcha,0,5)!="ERROR") && ($row->captcha_format=='json')) {
                    ...
                    if($captcha_field) {
                        $path = explode('/',$captcha_field);
                        if(is_array($json) && sizeof($path)==1 && isset($json[$captcha_field])){
                            $captcha = base64_decode($json[$captcha_field]);
                            print_log("Captcha decoded successfully ($code)");
                        } elseif(is_array($json) && sizeof($path)==2 && isset($json[$path[0]][$path[1]])){
                            $captcha = base64_decode($json[$path[0]][$path[1]]);
                            print_log("Captcha decoded successfully ($code)");
                        } else {
                            $captcha ="ERROR_JSON";
                            print_log("Captcha decoding error ($code)");
...
//            print_log("Format $captcha_format for session: $sessionid ($code)");
            ...
            if ($captcha_format) {
                if (db_execute($db,"UPDATE session SET lasttime=now(),endtime=NULL,sessionstatusid=1,statuscode='renew',".($captcha_image?"":"captchatime=now(),")."captcha_reporttime=NULL,captcha=''".($token?",token='$token'":"")." WHERE id=$sessionid")) {
...
//                        file_put_contents("captcha/$code/$sessionid.htm",$http_response_header."\n\n".$form);
                ...
                if ($antigateid && (strpos($antigateid,'ERROR')===false)) {
                    ...
                    db_execute($db,"UPDATE session SET captcha_service='".$host."'".($host<>'_neuro'?",captcha_id=$antigateid":",captcha_id=NULL")." WHERE id=$sessionid");
                    ...
                    if ($antigateid && (strpos($antigateid,'ERROR')===false)) {
                        $sessions[$sessionid] = array('sourceid'=>$sourceid,'code'=>$code,'captcha_format'=>$captcha_format,'captcha_type'=>$captcha_type,'captcha_size'=>$captcha_size,'cookies'=>$cookies,'antigatehost'=>$host,'antigatekey'=>$key,'antigateid'=>$antigateid,'starttime'=>microtime(true),'method'=>$captcha_check_method,'url'=>$captcha_check_url,'params'=>$params,'field'=>$captcha_field,'token_field'=>$token_field,'token'=>$token,'token_regexp'=>$captcha_check_token_regexp,'proxy'=>$proxy,'proxy_auth'=>$proxy_auth);
                        db_execute($db,"UPDATE session SET captcha_service='".$host."',captcha_id=$antigateid WHERE id=$sessionid");
                        print_log("Captcha id from ".$host." - $antigateid ($code)");
                    } else {
                        print_log("Failed sending captcha to ".$host." - $antigateid ($code)");
                        db_execute($db,"UPDATE session SET sessionstatusid=4,statuscode='failedcaptcha',endtime=now() WHERE id=$sessionid AND sessionstatusid=1");
                    }
                }
            }
        } else {
...
SELECT
	*,
	GREATEST(IFNULL((SELECT count(sourceid) FROM session WHERE sourceid=source.id AND sessionstatusid IN (3,4) AND endtime IS NOT NULL AND used IS NOT NULL AND endtime >= SUBDATE(NOW(), INTERVAL 5 MINUTE))/10,0),min_sessions)-
	IFNULL((SELECT count(sourceid) FROM session WHERE sourceid=source.id AND (source.session_time=0 OR unix_timestamp(now())-unix_timestamp(starttime)<source.session_time) AND sessionstatusid IN (1,2,7)),0) count
FROM source
WHERE status=1
HAVING count>=1
ORDER BY count DESC
...
//                    if ($proxy_sessions) $sql .= " AND id not in (SELECT proxyid FROM session WHERE sourceid=$sourceid AND endtime IS NULL AND proxyid IS NOT NULL)";
                    if ($proxy_sessions) $sql .= " AND (rotation>0 OR (SELECT COUNT(*) FROM session WHERE proxyid=proxy.id AND sourceid=$sourceid AND sessionstatusid IN (1,2,6,7))<$proxy_sessions)";
...
AND sourceaccessid not in (SELECT sourceaccessid FROM session WHERE endtime IS NULL AND sourceaccessid IS NOT NULL)
$sourceaccessfilter
ORDER BY lasttime
;
SQL;
...
//                            print_log($form);
            ...
            if (($form || $login_post) && $captcha_format) {
                if (db_execute($db,"INSERT INTO session (sourceid,cookies,starttime,lasttime,sessionstatusid,captcha,token,server,sourceaccessid,proxyid) VALUES ($sourceid,'$cookies_str',now(),now(),1,'','$token','$server',".($sourceaccessid?$sourceaccessid:'NULL').",".($proxy?"'".$proxyid."'":'NULL').")")) {
...
//                        file_put_contents("captcha/$code/$sessionid.htm",$http_response_header."\n\n".$form);
            ...
            if (($login_post_path || $form) && !$captcha_url && !$captcha_format && (!$auth_path_regexp || $auth_path)) {
                db_execute($db,"INSERT INTO session (sourceid,cookies,starttime,lasttime,sessionstatusid,captcha,token,server,sourceaccessid,proxyid) VALUES ($sourceid,'$cookies_str',now(),now(),2,'','$token','$server',".($sourceaccessid?$sourceaccessid:'NULL').",".($proxy?"'".$proxyid."'":'NULL').")");
                $sessionid = $db->insert_id;
                if ($sessionid) {
...
//                        db_execute($db,"UPDATE sourceaccess SET unlocktime=NULL WHERE sourceaccessid=$sourceaccessid");
                    ...
                    print_log("Session insert failed ($code login $login)\n"."INSERT INTO session (sourceid,cookies,starttime,lasttime,sessionstatusid,captcha,server,sourceaccessid,proxyid) VALUES ($sourceid,'$cookies_str',now(),now(),2,'','$server',".($sourceaccessid?$sourceaccessid:'NULL').",".($proxy?"'".$proxyid."'":'NULL').")");
                }
            }
        }
    }

    if (count($sessions)>0) foreach($sessions as $sessionid => &$s) {
...
//                db_execute($db,"UPDATE session SET captchaimage=NULL,captcha_reporttime=NULL,captchatime=now(),captcha='$captcha_value'".($s['captcha_format']=='image'?"":",lasttime=now(),endtime=null,sessionstatusid=1 ")." WHERE id=$sessionid");
                db_execute($db,"UPDATE session SET captchaimage=NULL,captcha_reporttime=NULL,captcha='$captcha_value'".($s['captcha_format']=='image'?"":",captchatime=now(),endtime=null,sessionstatusid=1 ")." WHERE id=$sessionid");
...
//                            print_log("Check regexp: {$s['token_regexp']} ($code)");
                            ...
                            db_execute($db,"UPDATE session SET cookies='$cookies_str', token='$token' WHERE id=$sessionid");
                        } elseif (!$check) {
//                            $captcha_value = false;
...
//                            print_log($check);
                ...
                db_execute($db,"update session SET captcha='$captcha_value' WHERE id=$sessionid AND sessionstatusid=1");
            ...
            if ($captcha_value) {
                if (strpos($captcha_value,'ERROR')===false) {
                    db_execute($db,"UPDATE session SET sessionstatusid=2 WHERE id=$sessionid AND sessionstatusid=1");
                } elseif ($captcha_value=='ERROR_CHECKING_CAPTCHA') {
//                    db_execute($db,"UPDATE session SET sessionstatusid=7,lasttime=now(),captcha_service=NULL,captcha_id=NULL WHERE id=$sessionid AND sessionstatusid=1 AND statuscode='renew'");
//                    db_execute($db,"UPDATE session SET captchaimage=NULL,captchatime=now(),captcha='',sessionstatusid=4,statuscode='checkingerror',endtime=now() WHERE id=$sessionid AND sessionstatusid=1");
                    db_execute($db,"UPDATE session SET sessionstatusid=4,statuscode='checkingerror',captchaimage=NULL,endtime=now() WHERE id=$sessionid AND sessionstatusid=1 AND statuscode<>'renew'");
                    db_execute($db,"UPDATE session SET sessionstatusid=7,statuscode='checkingerror',captchaimage=NULL,captcha='',captcha_service=NULL,captcha_id=NULL,captcha_reporttime=NULL WHERE id=$sessionid AND sessionstatusid=1 AND statuscode='renew'");
                } else {
//                    db_execute($db,"UPDATE session SET sessionstatusid=7,lasttime=now(),captcha_service=NULL,captcha_id=NULL WHERE id=$sessionid AND sessionstatusid=1 AND statuscode='renew'");
//                    db_execute($db,"UPDATE session SET captchaimage=NULL,captchatime=now(),sessionstatusid=4,statuscode='invalidcaptcha',endtime=now() WHERE id=$sessionid AND sessionstatusid=1");
                    db_execute($db,"UPDATE session SET sessionstatusid=4,statuscode='invalidcaptcha',captchaimage=NULL,endtime=now() WHERE id=$sessionid AND sessionstatusid=1 AND statuscode<>'renew'");
                    db_execute($db,"UPDATE session SET sessionstatusid=4,statuscode='invalidcaptcha',captchaimage=NULL WHERE id=$sessionid AND sessionstatusid=1 AND statuscode='renew'");
                }
...
file_put_contents("CYCLE.txt", (intval(file_get_contents("CYCLE.txt")) + 1));
fclose($flock);
sleep(5);
