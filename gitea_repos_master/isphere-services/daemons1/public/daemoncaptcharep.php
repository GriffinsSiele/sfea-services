...
// Помечаем отправленными просроченные отчеты для anti-captcha.com
    $sql = <<<SQL
UPDATE session SET captcha_reporttime=now() WHERE sessionstatusid=4 AND captcha_reporttime IS NULL AND captcha_service='api.anti-captcha.com' AND captchatime<SUBDATE(now(), INTERVAL 1 MINUTE)
$sessionfilter
SQL;
    db_execute($db,$sql);
...
$sessionfilter
AND (SELECT count(*) FROM session WHERE sessionstatusid=4 AND captcha_service IS NOT NULL AND captcha_reporttime IS NULL)>20
ORDER BY session.captchatime DESC LIMIT 1
SQL;
    $result = $db->query($sql);
