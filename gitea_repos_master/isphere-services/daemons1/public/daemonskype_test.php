...
if (!$db) die("Connection to SQL server failed\r\n");
$sourceid = 16;
...
//            print_log("Not enough proxies ($code)");
    ...
    if (isset($skype) && ($token = $skype->getToken())) {
        ...
        if ($count==0) {
            ...
            db_execute($db,"UPDATE session SET sessionstatusid=5, endtime=now() WHERE sourceid=16 AND sourceaccessid=$sourceaccessid AND sessionstatusid IN (2,6,7)");
            // Создаём новую сессию
            if (!$proxy) $proxyid = 'NULL';
            db_execute($db,"INSERT INTO session (sourceid,cookies,starttime,lasttime,sessionstatusid,captcha,token,server,sourceaccessid,proxyid) VALUES (16,'',now(),now(),2,'','$token','',$sourceaccessid,$proxyid)");
        }
    } else {
