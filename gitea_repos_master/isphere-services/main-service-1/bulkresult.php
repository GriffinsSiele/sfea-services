...
//      $mysqli = mysqli_connect ($database['server'],$database['login'],$database['password'],$database['name']);
      ...
      if (!$result || !$result->num_rows){
          ...
          exit;
            ...
            $result->close();
            $bulk['filename'] = strtr($bulk['filename'],array(','=>'.'));
            $files = glob('/opt/bulk/'.$id.'/result.*');
            if (sizeof($files)>0) {
                $file = $files[0];
...
//                header('Content-Description: File Transfer');
                ...
                header('Content-Disposition: attachment; filename='.pathinfo($bulk['filename'],PATHINFO_FILENAME).' - результаты.'.pathinfo($file,PATHINFO_EXTENSION));
...
//                file_put_contents('BULK_ERRORS.txt', $id."\n", FILE_APPEND);
                echo "Результаты по реестру $id отсутствуют";
                exit;
            }
