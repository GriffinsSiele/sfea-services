...
class RequestContext
{
    ...
    public function __construct($source, $checktype, $requestid, $path, $start, $param, $level, $plugin_code, $initData)
    {
        ...
        $this->level = $level;
        $this->start = $start?$start:$param;
        $this->param = $param;
        $this->parent = $path;
        $this->path = ($path?$path.'/':'').$param;
        $this->id = $requestid.'__'.strtr($param,array('['=>'_',']'=>'')).'__'.$checktype.'__'.$level; //$checktype.'_'.$requestid.'_'.$level;
        $this->ch = null;
        $this->plugin = isset($plugin_interface[$plugin_code])?$plugin_interface[$plugin_code]:null;
        $this->initData = array_merge(array('checktype'=>$checktype),$initData);
...
//        curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,false);
    ...
    {
        if (!isset($this->starttime)) $this->endtime = $this->starttime = microtime(true);
...
*/
    ...
    public function setError($error, $errortype = 0)
    {
        $this->error = $error;
//        $this->error .= $error.'#';
