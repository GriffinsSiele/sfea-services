...
function normal_phone($phone)
{
    ...
    if (!$plus && (strlen($phone) == 11) && (substr($phone, 0, 1) == '8')) {
        $phone = '7' . substr($phone, 1);
    }
    if (!$plus && (strlen($phone) == 10)) {
        $phone = '7' . $phone;
    }
...
class RequestManager
{
    ...
    public function initRequestContexts($params, $plugins, $level, $path = false, $start = false)
    {
        ...
        $name = array();
        if (isset($params['person']['last_name'])) {
            $name['last_name'] = $params['person']['last_name'];
        }
        if (isset($params['person']['first_name'])) {
            $name['first_name'] = $params['person']['first_name'];
        }
        if (isset($params['person']['date'])) {
            $name['date'] = $params['person']['date'];
        }
    ...
    public function performRequests($params, $plugins)
    {
        ...
        do { // цикл выполнения запроса
            ...
            foreach ($workingPool as $ind => $rContext) {
                if (isset($rContext)) {
                    if ($rContext->isReady()) {
                        ...
                        if (isset($checktypes[$checktype]) && $checktypes[$checktype]['status'] <= 0) { // Проверка отключена
            ...
            foreach ($finishedPool as $ind => $rContext) {
                if (isset($rContext) && $rContext->getPlugin()) {
                        ...
                        foreach ($result as $record) {
                            foreach ($record as $field) {
                                ...
                                if ($value && ($type == 'url' || $type == 'hidden') && (/*$params['recursive'] || */ $recursive) && ($level <= 20) && ($contact_source = array_search(strtr(parse_url($value, PHP_URL_HOST), array('www.' => '')), $contact_urls)) !== false && !in_array($value, $params['url'])/* && ($params['recursive'] || in_array($contact_source,$params['sources']))*/) {
                                    ...
                                    $new_params['sources'] = /*array($contact_source.'_url'); //*/
                                        array_intersect($new_params['sources'], array($contact_source, $contact_source . '_url'));
                                ...
                                if ($value && (strpos(strtoupper($name), 'BIRTHDATE') !== false || strpos(strtoupper($name), 'BIRTHDAY') !== false) && (strlen($value) == 10) && (sizeof($result) < 10 || $rContext->getCheckType() == 'ok_phoneapp' || $rContext->getCheckType() == 'ok_emailapp') && isset($record['name']) && strpos($record['name']->getValue(), ' ') && (array_search(mb_strtoupper($record['name']->getValue() . ' ' . $value), $h_params) === false) /*&& !isset($params['person']['date'])*/) {
                                ...
                                if ($value && !preg_match("/\*/", $value) && (strtoupper($name) == 'VIN') && (sizeof($result) < 10) && (array_search($value, $h_params) === false) /*!isset($params['car']['vin'])*/) {
                                    ...
                                    if ($source_name != 'eaisto' && $source_name != 'ЕАИСТО') {
                                        $new_params['sources'][] = 'eaisto';
                                    }
                                    //                                        /*if ($source_name!='RSA')*/ $new_params['sources'][] = 'rsa_policy';
                                    if ($source_name != 'carinfo') {
                                        $new_params['sources'][] = 'carinfo';
                                    }
                                    if ($source_name != 'alfastrah') {
                                        $new_params['sources'][] = 'alfastrah';
                                    }
                                    if ($source_name != 'elpts') {
                                        $new_params['sources'][] = 'elpts';
                                    }
                                    $new_params['sources'] = array_intersect($params['sources'], $new_params['sources']);
                                    $new = true;
                                }
                                ...
                                if ($value && (strtoupper($name) == 'IPNUMBER' || strtoupper($name) == 'CASENUMBER' || strtoupper($name) == 'DOCNUMBER') && preg_match("/([0-9]+[\-\/][0-9]+[\-\/][0-9]+)-ИП/", $value) && sizeof($result) < 10 && !in_array($value, $params['fssp_ip'])) {
                                    ...
                                    $new_params['sources'] = array();
                                    if ($source_name != 'fssp' && $source_name != 'fsspsite' && $source_name != 'gisgmp') {
                                        $new_params['sources'][] = 'fssp';
                                    }
                                    if ($source_name != 'gisgmp' && !isset($record['close_date'])) {
