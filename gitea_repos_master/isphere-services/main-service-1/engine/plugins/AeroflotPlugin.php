...
*/
        ...
        $params['lang'] = 'ru';
        if (isset($initData['phone']))
            $params['data']['phone'] = '+'.$initData['phone']; // input:"check_type=aeroflot_phone,name=phone,type=string,required"
        else
            $params['data']['email'] = $initData['email']; // input:"check_type=aeroflot_email,name=email,type=string,format=email,required"
...
//            file_put_contents('./logs/aeroflot/aeroflot_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
...
*/
            ...
            if ($content) file_put_contents('./logs/aeroflot/aeroflot_err_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
        } else {
