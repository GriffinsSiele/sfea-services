...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('callapp', $initData['phone']) && ($db->llen('callapp_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('callapp', $initData['phone'])){
                ...
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('callapp', $initData['phone']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
...
//        file_put_contents('./logs/callapp/callapp_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/callapp/callapp_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/callapp/callapp_err_'.time().'.txt',$content);
