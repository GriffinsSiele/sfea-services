...
class DomclickPlugin implements PluginInterface
{
    ...
    {
        return 'Поиск телефона в Домклик';
    }
...
//            $params['id'] = $reqId;
            ...
            try {
                ...
                $result = $exchange->publish($swapData['param'],'domclick'/*,AMQP_NOPARAM,['expiration' => 60]*/);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('domclick', $swapData['param'])){
                $content = $db->hget('domclick', $swapData['param']);
                $res = json_decode($content, true);
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('domclick', $swapData['param']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
...
//        file_put_contents('./logs/domclick/domclick_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/domclick/domclick_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/domclick/domclick_err_'.time().'.txt',$content);
