...
//            $url = 'http://172.16.1.3:8090/passport?series='.$initData['passport_series'].'&number='.$initData['passport_number'];
//            $url = 'http://fmsdb-master.isphere-go-modules/passport?series='.$initData['passport_series'].'&number='.$initData['passport_number'];
            $url = 'http://172.16.0.69/passport?series='.$initData['passport_series'].'&number='.$initData['passport_number'];
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_TIMEOUT, 5);
        }
...
//        $rContext->setSwapData($swapData);
        ...
        $error = false; //$content?false:curl_error($rContext->getCurlHandler());

        file_put_contents('./logs/fms/fms_'.time().'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content."\n\n".$swapData['iteration']);
...
//                    $error = 'Список недействительных паспортов недоступен';
            ...
            if ($start!==false) {
                ...
                $data['Result'] = new ResultDataField('string','Result', strtr(trim($content),array('C'=>'С')), 'Результат', 'Результат проверки паспорта');
...
//                        $error = trim($content);
                    ...
                    file_put_contents('./logs/fms/fms_err_'.time().'.html',$content);
                    if (isset($swapData['session']))
                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3,statuscode='unexpected' WHERE id=" . $swapData['session']->id);
                    unset($swapData['session']);
                } elseif (strpos($content,'internal error') || strpos($content,'nginx') || strpos($content,'Failed')) {
                    file_put_contents('./logs/fms/fms_err_'.time().'.html',$content);
                    if (isset($swapData['session']))
                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3,statuscode='error' WHERE id=" . $swapData['session']->id);
                    unset($swapData['session']);
                } elseif ($content) {
                    file_put_contents('./logs/fms/fms_err_'.time().'.html',$content);
...
*/
        ...
        if(!isset($swapData['db']) && $swapData['iteration']>=10) {
            $swapData['db']=true;
        }
