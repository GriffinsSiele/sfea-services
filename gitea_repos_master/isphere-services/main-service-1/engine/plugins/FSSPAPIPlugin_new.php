...
//            $rContext->setError('Указаны не все обязательные параметры (номер ИП)');
        ...
        if (!isset($swapData['task'])) {
            ...
            if ($checktype=='person') {
                $params['region']=isset($initData['region_id'])?$initData['region_id']:'null';
...
//                    $record['doc_issuer'] = new ResultDataField('string','DocIssuer', $doc_issuer, 'Орган', 'Орган, выдавший исполнительный документ');
                    ...
                    if (sizeof($subject)>1)
                        $record['total'] = new ResultDataField('string','Total', strtr(substr($subject[1],0,strpos($subject[1],' ')),',','.'), 'Сумма', 'Сумма задолженности');
                    foreach($subjects as $subject) {
                        ...
                        if (isset($name[$subject_b[0]])) {
                            ...
                            if(sizeof($subject_b)>1)
                                $record[$n.'Total'] = new ResultDataField('string',$n.'Total', strtr(substr($subject_b[1],0,strpos($subject_b[1],' ')),',','.'), $t, $t.' исполнителя');
                        }
                    }
