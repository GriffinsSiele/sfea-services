...
//        return 'Поиск задолженностей в ГИС ГМП';
    ...
    public function prepareRequest(&$rContext)
    {
        ...
        if(($checktype=='taxes') && !isset($initData['inn'])){
            $rContext->setFinished();
//            $rContext->setError('Указаны не все обязательные параметры (ИНН)');
            return false;
        }

        if(($checktype=='fines') && !isset($initData['driver_number']) && !isset($initData['ctc'])){
            $rContext->setFinished();
//            $rContext->setError('Указаны не все обязательные параметры (водительское удостоверение или св-во о регистрации ТС)');
            return false;
        }

        if(($checktype=='fssp') && (!isset($initData['last_name']) || !isset($initData['first_name']) || !isset($initData['date']))
          /* && (!isset($initData['passport_series']) || !isset($initData['passport_number'])) && !isset($initData['inn'])*/){
            $rContext->setFinished();
...
//            $rContext->setError('Указаны не все обязательные параметры (паспорт или ИНН)');
        ...
        if(($checktype=='ip') && !preg_match("/([0-9]+[\-\/][0-9]+[\-\/][0-9]+)-ИП/", $initData['fssp_ip'], $matches)){
            $rContext->setFinished();
...
*/
        ...
        if (!isset($swapData['uid'])) {
            ...
            if ($checktype=='fssp' && isset($initData['last_name']) && isset($initData['first_name']) && isset($initData['date'])) {
                $data = 'FIO='.urlencode($initData['last_name'].' '.$initData['first_name'].(isset($initData['patronymic'])?' '.$initData['patronymic']:'')).'&birthday='.date('d.m.Y',strtotime($initData['date'])).'&region='.(isset($initData['region_id'])?$initData['region_id']:'');
            } elseif ($checktype=='fssp' && isset($initData['passport_number'])) {
                $data = 'passport='.$initData['passport_series'].$initData['passport_number'];
            } elseif (($checktype=='fssp' || $checktype=='taxes') && isset($initData['inn'])) {
                $data = 'inn='.$initData['inn'];
            } elseif ($checktype=='fines' && isset($initData['driver_number'])) {
                $data = 'regnum=&license='.urlencode($initData['driver_number']);
            } elseif ($checktype=='fines' && isset($initData['ctc'])) {
                $data = 'regnum='.urlencode($initData['ctc']).'&license=';
            } elseif ($checktype=='ip' && isset($initData['fssp_ip'])) {
...
//            print "Proxy: ".$swapData['session']->proxy."\n";
    ...
    public function computeRequest(&$rContext)
    {
        ...
        if (!isset($swapData['uid'])) {
//            if ($content) file_put_contents('./logs/gisgmp/oplatagosuslug_start_'.time().'.html',$content);
            $res = json_decode($content,true);
            if (isset($res['uid'])) {
                $swapData['uid'] = $res['uid'];
                $rContext->setSleep(5);
            } elseif (isset($res['status']) && ($res['status']==403 || !$res['status'])) {
                $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3,code='expired' WHERE id=" . $swapData['session']->id);
                unset($swapData['session']);
            } elseif ($content) {
                file_put_contents('./logs/gisgmp/oplatagosuslug_start_err_'.time().'.html',$content);
...
//            if ($content) file_put_contents('./logs/gisgmp/oplatagosuslug_get_'.time().'.html',$content);
            $res = json_decode($content,true);
            if (isset($res['status']) && $res['status']) {
                if ($res['status']=='success') {
                    ...
                    file_put_contents('./logs/gisgmp/oplatagosuslug_get_err_'.time().'.html',$content);
                    $error = "Ошибка при выполнении запроса";
                }
            }
        } else {
//            if (!empty(trim($content))) file_put_contents('./logs/gisgmp/oplatagosuslug_'.time().'.html',$content);
            ...
            if(preg_match_all("/<div class=\"panel-heading\"[^>]+>.*?<\/article>/sim", $content, $matches)){
                foreach($matches[0] as $key => $part){
                    if(preg_match("/<pre>Индекс ([0-9]+),([^<]+)<\/pre>/sim", $part, $mmmatches)){
                        $res[$key]['index'] = trim($mmmatches[1]);
                        $res[$key]['debttype'] = trim($mmmatches[2]);
                    }
                    ...
                    if(preg_match_all("/<span class=\"dt\">(.*?)<\/span><span class=\"dd\"><pre>([^<]+)/sim", $part, $mmmatches)){
                        foreach($mmmatches[1] as $k => $val){
                            $res[$key][trim(strip_tags($val))] = trim(strip_tags($mmmatches[2][$k]));
                        }
                    }
                ...
                file_put_contents('./logs/gisgmp/oplatagosuslug_err_'.time().'.html',$content);
...
//                            }
                        ...
                        if (isset($debt['Документ'])) {
                            $debt['Документ'] = strtr(html_entity_decode($debt['Документ']),array(' '=>' '));
                            ...
                            elseif (preg_match("/паспорт РФ\, серия ([\d\*А-Я]+) номер ([\d\*А-Я]+)/",$debt['Документ'],$matches) && (strpos(mb_strtoupper($debt['purpose']),'ГИБДД')!==false))
