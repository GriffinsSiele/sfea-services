...
//            $params['phone'] = $initData['phone'];
            ...
            try {
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('hlr', $initData['phone']) && ($db->llen('hlr_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//        file_put_contents('./logs/hlr/hlr_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/hlr/hlr_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/hlr/hlr_err_'.time().'.txt',$content);
