...
//            $params['phone'] = $initData['phone'];
            ...
            try {
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('icq', $initData['phone']) && ($db->llen('icq_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//        file_put_contents('./logs/icq/icq_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/icq/icq_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/icq/icq_err_'.time().'.txt',$content);
