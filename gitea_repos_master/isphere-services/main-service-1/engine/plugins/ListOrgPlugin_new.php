...
{
    private $names = array (
                           'телефон' => array('phone', 'Телефон', 'Телефон'),
                           'инн/кпп' => array('inn', 'ИНН', 'ИНН'),
                           'кпп' => array('kpp', 'КПП', 'КПП'),
                           'руководитель' => array('head', 'Руководитель', 'Руководитель'),
                           'юр.адрес' => array('address', 'Адрес', 'Адрес'),
    );
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query("UPDATE isphere.session s SET request_id=".$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=23 ORDER BY lasttime limit 1");
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=23 AND request_id=".$reqId." ORDER BY lasttime limit 1");
    ...
    public function prepareRequest(&$rContext)
    {
        ...
        if(!isset($initData['phone']) && !isset($initData['email']) && !isset($initData['inn']) && !isset($initData['ogrn'])) {
            $rContext->setFinished();
            $rContext->setError('Указаны не все обязательные параметры (телефон или email или ИНН или ОГРН)');
        ...
        if(!$error) {
            ...
            if (preg_match("/вы не робот/", $content, $matches)) {
                if (isset($swapData['session']))
                    $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=5,statuscode='invalidcaptcha' WHERE id=" . $swapData['session']->id);
                unset($swapData['session']);
                $rContext->setSleep(1);
            } elseif (!isset($swapData['url'])) {
                file_put_contents('./logs/listorg/search_'.time().'.html',$content);
                $resultData = new ResultDataList();

                if(preg_match("/Найдено 0 организаций/sim", strip_tags($content), $matches)){
                    $rContext->setResultData($resultData);
                    $rContext->setFinished();
                } elseif(preg_match("/<div class=\'org_list\'>(.*?)<\/div>/sim", $content, $matches)){
                    $parts = preg_split("/<\/p>/",strtr($matches[1],array('&nbsp;'=>' ')));
...
//                                    if ($field[0]=='phone') $text = normal_phone($text);
                                    ...
                                    file_put_contents('./logs/fields/listorg_'.time().'_'.strtr($title,array('/'=>'_')), $title."\n".$text);
                                }
                            }
                        }
                    ...
                    if (isset($swapData['session']))
                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=5,statuscode='badanswer' WHERE id=" . $swapData['session']->id);
                    unset($swapData['session']);
                } elseif (preg_match("/403 Forbidden/", $content)) {
                    if (isset($swapData['session']))
                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=5,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
                    unset($swapData['session']);
                } elseif ($content) {
                    file_put_contents('./logs/listorg/search_err_'.time().'.html',$content);
