...
class MicrosoftPlugin implements PluginInterface
{
    ...
    public function getTitle($checktype = '')
    {
        $title = array(
            '' => 'Поиск в Microsoft',
            'microsoft_phone' => 'Microsoft - поиск по номеру телефона',
            'microsoft_email' => 'Microsoft - поиск по email',
            'microsoft_nick' => 'Microsoft - поиск по псевдониму',
        );
        return isset($title[$checktype])?$title[$checktype]:$title[''];
//        return 'Поиск в Microsoft';
    }
...
//            $rContext->setError('Указаны не все обязательные параметры (псевдоним)');
        ...
        if($checktype=='nick' && preg_match("/^live\:/",$initData['nick'])) {
            $rContext->setFinished();
            return false;
        }
...
//            $params['id'] = $reqId;
            ...
            } elseif ($checktype=='nick') {
                $swapData['param'] = $initData['nick'];
            } else {
                $rContext->setFinished();
            ...
            try {
                ...
                $db->auth($keydb['auth']);
//                if($db->hexists('micro_result', $swapData['param']))
//                    $db->hdel('micro_result', $swapData['param']);
                if ($db->llen('micro_queue')>20) {
                    $db->close();
                    $rContext->setFinished();
                ...
                if(!$db->hexists('micro_result', $swapData['param']))
                    $db->rpush('micro_queue', $swapData['param']);
                $swapData['db'] = $db;
//                $rContext->setSleep(1);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('micro_result', $swapData['param'])){
                $content = $db->hget('micro_result', $swapData['param']);
                ...
                if($swapData['iteration']>30){
                    $db->hdel('micro_queue', $swapData['param']);
                    $db->close();
                    $rContext->setFinished();
...
//        file_put_contents('./logs/microsoft/'.$initData['checktype'].'_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                    $row = $res;
                ...
                file_put_contents('./logs/microsoft/'.$initData['checktype'].'_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/microsoft/'.$initData['checktype'].'_err_'.time().'.txt',$content);
