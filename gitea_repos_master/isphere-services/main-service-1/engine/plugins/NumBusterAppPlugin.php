...
class NumBusterAppPlugin implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query("UPDATE isphere.session s SET request_id=".$reqId." WHERE request_id IS NULL AND (sessionstatusid=2 OR statuscode='limitexceed') AND sourceid=12 AND token>'' AND unix_timestamp(now())-unix_timestamp(lasttime)>15 AND (substr(data,1,1)<>'r' OR unix_timestamp(now())-unix_timestamp(lasttime)>3600) ORDER BY lasttime limit 1");
        $result = $mysqli->query("SELECT id,cookies,server,starttime,lasttime,statuscode,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=12 AND request_id=".$reqId." ORDER BY lasttime limit 1");
        if($result) {
            ...
            if ($row) {
                ...
                $mysqli->query("UPDATE isphere.session SET lasttime=now(),used=ifnull(used,0)+1,statuscode='used',request_id=NULL WHERE request_id=".$reqId." AND id=".$sessionData->id);
//                $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3 WHERE used>=1 AND id=".$sessionData->id);

                if (!$row->proxyid) {
                    $result = $mysqli->query("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM isphere.proxy WHERE enabled=1 AND status=1 AND proxygroup=1 AND id NOT IN (SELECT proxyid FROM session WHERE sourceid=12 AND proxyid IS NOT NULL) ORDER BY lasttime limit 1");
                    if ($result) {
                        $row = $result->fetch_object();
                        if ($row) {
                            $sessionData->proxyid = $row->proxyid;
...
*/
        ...
        $swapData['test'] = false;//(substr($swapData['session']->lasttime,0,13)<date('Y-m-d H') && date('H')>='09');
        $rContext->setSwapData($swapData);
...
//        $locale = 'ru';
        ...
        if (substr($swapData['session']->lasttime,0,10)<date('Y-m-d')) $swapData['session']->host='';
...
//            if ($swapData['iteration']>=3 && rand(0,1))
                $host = '7d7992e49365310ec5e997241c6312bd.com';
            $phone = $swapData['test']?/*'78005552550'*/'78005555550':$initData['phone'];
...
//            $source = 'GET'.$host.'/api/v13/call/outgoing'.'access_token='.$access_token.'&cnonce='.$cnonce.'&locale='.$locale.'&phone='.$phone.'&timestamp='.$timestamp;
            $source = 'GET'.$host.'/api/v13/call/incoming'.'access_token='.$access_token.'&cnonce='.$cnonce.'&locale='.$locale.'&phone='.$phone.'&timestamp='.$timestamp;
...
//        curl_setopt($ch, CURLINFO_HEADER_OUT, true);
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, ($swapData['session']->status=='success' || rand(0,4)==0)?20:5);
    ...
    public function computeRequest(&$rContext)
    {
        ...
        if (!$content) {
            if ($swapData['iteration']>=3) $error = curl_error($rContext->getCurlHandler());
            $http_code = curl_getinfo($rContext->getCurlHandler(),CURLINFO_HTTP_CODE);
            file_put_contents('./logs/numbuster/numbuster_empty_'.$swapData['iteration'].'_'.time().'.txt',$http_code."\n".curl_error($rContext->getCurlHandler()));
            if ($http_code>=500) {
                if ($swapData['iteration']>=2) $error = 'Внутренняя ошибка источника';
                ...
                $mysqli->query("INSERT INTO session12 SELECT * FROM session WHERE successtime<DATE_SUB(now(),INTERVAL 6 HOUR) AND id=" . $swapData['session']->id);
                $mysqli->query("UPDATE isphere.session SET sessionstatusid=4,endtime=now() WHERE id IN (SELECT id FROM session12) AND id=" . $swapData['session']->id);
            }
            unset($swapData['session']);
/*
        } elseif(!isset($swapData['v6'])) {
//            file_put_contents('./logs/numbuster/numbuster_old_'.($swapData['test']?'test_':'').$swapData['iteration'].'_'.time().'.txt',$content);
            ...
            if($res && isset($res['phones'])){
                ...
                file_put_contents('./logs/numbuster/numbuster_err_'.$swapData['iteration'].'_'.time().'.txt',$content);
...
//            file_put_contents('./logs/numbuster/numbuster_host_'.$swapData['iteration'].'_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
            $start = strpos($content,'{');
            $content = trim(substr($content,$start,strlen($content)-$start+1));
            $res = json_decode($content, true);
            if($res && isset($res['status']) && $res['status']=='success' && isset($res['data'])){
                $host = strtr($res['data'],array('https://'=>'','http://'=>''));
                $swapData['session']->host = $host;
                $swapData['session']->lasttime = date('Y-m-d H:i:s');
                $mysqli->query("UPDATE isphere.session SET lasttime=now(),server='$host' WHERE id=" . $swapData['session']->id);
            }
        } else {
//            file_put_contents('./logs/numbuster/numbuster_'.($swapData['test']?'test_':'').$swapData['iteration'].'_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
            $start = strpos($content,'{');
            $content = trim(substr($content,$start,strlen($content)-$start+1));
            $res = json_decode($content, true);
            if($res && isset($res['status']) && $res['status']=='success' && is_array($res['data'])){
                $res = $res['data'];
            }
            if($res && isset($res['metrics'])){
                ...
                $data = array();
                $name = '';
                $profile = isset($res['name']) && is_array($res['name'])?$res['name']:(isset($res['averageProfile'])?$res['averageProfile']:(isset($res['profile'])?$res['profile']:array()));
                if (isset($profile['firstName'])) {
                    if ($profile['firstName']=='Please Update App') {
                        $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='needupdate' WHERE id=" . $swapData['session']->id);
                        unset($swapData['session']);
                        $rContext->setSwapData($swapData);
                        return false;
                    } elseif ($profile['firstName']=='Covers up info') {
                        $name = 'Информация скрыта';
                    } else {
                        $data['first_name'] = new ResultDataField('string','first_name',trim($profile['firstName']),'Имя','Имя');
                        $name = trim($profile['firstName']);
                    }
                }
                if (isset($profile['lastName']) && trim($profile['lastName'])) {
                    $data['last_name'] = new ResultDataField('string','last_name',trim($profile['lastName']),'Фамилия','Фамилия');
                    $name = trim($name . ' ' . trim($profile['lastName']));
                }
...
*/
                ...
                if (isset($res['common']['leftRequests']) && $res['common']['leftRequests']<=1) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(str_to_date('".date('Y-m-d 03:00:00',time()-3*60*60)."', '%Y-%m-%d %H:%i:%s'),interval 1 day),sessionstatusid=6,statuscode='limitexceed' WHERE sessionstatusid=2 AND id=" . $swapData['session']->id);
                    unset($swapData['session']);
                } elseif ($swapData['test']) {
                    $swapData['session']->lasttime = date('Y-m-d H:i:s');
                    $swapData['test'] = false;
                    if ($name!=/*'Тинькоф Линия'*/'Сбербанка Линия') {
                    //    file_put_contents('./logs/numbuster/dead/'.$swapData['session']->id.'_'.time().'.txt', $content);
                        $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 month),sessionstatusid=6,statuscode='garbage' WHERE id=" . $swapData['session']->id);
                        unset($swapData['session']);
                    }
                } else {
                    if (sizeof($data))
                ...
                $mysqli->query("UPDATE isphere.session SET success=ifnull(success,0)+1,statuscode='success',successtime=now() WHERE id=".$swapData['session']->id);
                unset($swapData['session']);
            } elseif($res && isset($res['message']) && $res['message']=="Unknown error") {
                $error = "Внутренняя ошибка источника";
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='unknown' WHERE id=" . $swapData['session']->id);
                unset($swapData['session']);
            } else {
                file_put_contents('./logs/numbuster/numbuster_err_'.$swapData['iteration'].'_'.time().'.txt',$content);
                if ($swapData['iteration']>2) {
                ...
                if (strpos($content,'414 URI Too Long')) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='uritoolong' WHERE id=" . $swapData['session']->id);
                    $mysqli->query("INSERT INTO session12 SELECT * FROM session WHERE id=" . $swapData['session']->id);
                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=4,endtime=now() WHERE id=" . $swapData['session']->id);
                } else {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 10 minute),sessionstatusid=6,statuscode='invalidanswer' WHERE id=" . $swapData['session']->id);
                }
                unset($swapData['session']);
