...
class NumBusterPlugin implements PluginInterface
{
...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                if(!$db->hexists('numbuster', $initData['phone'])) {
                    if ($db->llen('numbuster_queue')>20) {
                        $db->close();
                        unset($swapData['db']);
                    ...
                    $db->rpush('numbuster_queue', $initData['phone']);
                }                
                $swapData['db'] = $db;
//                    $rContext->setSleep(1);
//                    $rContext->setSwapData($swapData);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('numbuster', $initData['phone'])){
                $content = $db->hget('numbuster', $initData['phone']);
                $res = json_decode($content, true);
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('numbuster', $initData['phone']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
                if($swapData['iteration']>30){
                    $db->hdel('numbuster_queue', $initData['phone']);
                    $db->close();
                    $rContext->setFinished();
...
//        file_put_contents('./logs/numbuster/numbuster_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/numbuster/numbuster_err_'.time().'.txt',$content);
