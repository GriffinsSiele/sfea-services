...
class NumBusterProPlugin implements PluginInterface
{
    ...
    {
        return 'Поиск телефона в NumBuster Pro';
    }
...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                if(!$db->hexists('numbuster-pro', $initData['phone'])) {
                    if ($db->llen('numbuster-pro_queue')>20) {
                        $db->close();
                        unset($swapData['db']);
                    ...
                    $db->rpush('numbuster-pro_queue', $initData['phone']);
                }                
                $swapData['db'] = $db;
//                    $rContext->setSleep(1);
//                    $rContext->setSwapData($swapData);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('numbuster-pro', $initData['phone'])){
                $content = $db->hget('numbuster-pro', $initData['phone']);
                $res = json_decode($content, true);
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('numbuster-pro', $initData['phone']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
                if($swapData['iteration']>20){
                    $db->hdel('numbuster-pro_queue', $initData['phone']);
                    $db->close();
                    $rContext->setFinished();
...
//                        $db->rpush('numbuster-pro_queue', $initData['phone']);
        ...
        file_put_contents('./logs/numbusterpro/numbusterpro_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                    $row = $res;
                foreach ($res['records'] as $row) if (is_array($row)) {
                    ...
                    foreach($row as $field) if (is_array($field) && isset($field['value'])) {
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/numbusterpro/numbusterpro_err_'.time().'.txt',$content);
                if (!strpos($res['message'],'время ожидания'))
                    $error = $res['message'];
            } else {
                file_put_contents('./logs/numbusterpro/numbusterpro_err_'.time().'.txt',$content);
