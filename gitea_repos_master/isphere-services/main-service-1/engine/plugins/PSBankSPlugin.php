...
class PSBankSPlugin implements PluginInterface
{
    private $aes;
...
//            $rContext->setSleep(3);
        ...
        if (!isset($swapData['operationId'])) {
            ...
            $params = array(
                'bank_code' => $this->encrypt('100000000010'),
...
//                file_put_contents('./logs/sovcombank/psbank_id_'.time().'.txt',$content);
                if (isset($res['data']['operationId'])) {
                    $swapData['operationId'] = $res['data']['operationId'];
                    $rContext->setSwapData($swapData);
                    $rContext->setSleep(1);
                } else {
                    file_put_contents('./logs/sovcombank/psbank_id_err_'.time().'.txt',$content);
                    $error = "Ошибка при отправке запроса";
                }
            } elseif($res && isset($swapData['operationId'])) {
                file_put_contents('./logs/sovcombank/psbank_'.time().'.txt',$content);
...
//                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(current_date(),interval 1 day),sessionstatusid=6,statuscode='exceeded' WHERE id=" . $swapData['session']->id);
                ...
                file_put_contents('./logs/sovcombank/psbank_err_'.time().'.txt',$content);
