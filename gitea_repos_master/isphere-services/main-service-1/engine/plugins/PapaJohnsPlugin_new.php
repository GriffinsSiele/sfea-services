...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('papajohns', $initData['phone']) && ($db->llen('papajohns_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('papajohns', $initData['phone'])){
                ...
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('papajohns', $initData['phone']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
...
//                        $db->rpush('papajohns_queue', $initData['phone']);
        ...
        file_put_contents('./logs/papajohns/papajohns_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/papajohns/papajohns_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/papajohns/papajohns_err_'.time().'.txt',$content);
