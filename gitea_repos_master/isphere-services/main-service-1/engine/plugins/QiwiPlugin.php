...
class QiwiPlugin implements PluginInterface
{
    ...
    {
        return 'Qiwi - проверка телефона на наличие электронного кошелька';
    }
...
*/
        ...
        $header = array(
          ...
          'Referer: https://1.elecsnet.ru/NotebookFront/services/0mhp/default.aspx?merchantId=23987',
...
//            file_put_contents('./logs/elecsnet/qiwi_'.time().'.txt',$content);
            ...
            if($res && isset($res['message']) && !strpos($res['message'],'невозможно')){
                ...
                if (isset($res['isSuccess'])){
                    if ($res['isSuccess']) {
                        $data['result'] = new ResultDataField('string','result', 'По телефону '.$initData['phone'].' найден электронный кошелек Qiwi', 'Результат', 'Результат');
                ...
                if ($content) {
                    file_put_contents('./logs/elecsnet/qiwi_err_'.time().'.txt',$content);
