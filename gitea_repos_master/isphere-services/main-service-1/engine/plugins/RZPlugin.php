...
//        array('host' => 'capmonster.i-sphere.local', 'key' => 'afb26cbb248d650ea8b8d88822984242', 'minscore' => 0.3, 'maxtime' => 20, 'maxinvalid' => 10),
//        array('host' => 'api.capmonster.cloud', 'key' => 'afb26cbb248d650ea8b8d88822984242', 'minscore' => 0.3, 'maxtime' => 20, 'maxinvalid' => 5),
//        array('host' => 'api.anti-captcha.com', 'key' => '63def0a149a147e0d13e409cc8318fc3', 'minscore' => 0.3, 'maxtime' => 20, 'maxinvalid' => 30),
//        array('host' => 'rucaptcha.com', 'key' => 'd167c71a9278312f184f17caa4e71050', 'minscore' => 0.3, 'maxtime' => 20, 'maxinvalid' => 5),
        array('host' => 'api.nextcaptcha.com', 'key' => 'next_ee8fed6e0853b731e900b14869215f7741', 'minscore' => 0.3, 'maxtime' => 20, 'maxinvalid' => 3),
    );
...
/*
        if (!isset($swapData['captcha_token'])) {
            $token = neuro_token('v3',$this->googlekey,'search_notary');
            if (strlen($token)>30) {
                $swapData['captcha_token'] = $token;
                $swapData['captcha_service'] = 'queue';
                $swapData['captcha_id'] = 0;
//                echo "Queue token: ".substr($token,0,5)."...".substr($token,strlen($token)-5,5)."\n";
            } else {
//                echo "Queue token not ready\n";
            }
        }
*/
        ...
        if (!isset($swapData['captcha_token']) && !isset($swapData['captcha_id'.$swapData['num']])) {
            if ($swapData['iteration']<10 || $swapData['iteration']%4) {
                $swapData['captcha_session'] = $this->getSessionData($this->search_sourceid);
                if (!$swapData['captcha_session'] && $swapData['iteration']%10==0)
                    $mysqli->query("INSERT INTO isphere.session (used,endtime,sourceid,sessionstatusid,statuscode,captcha_service) VALUES (1,now(),".($this->search_sourceid).",4,'needmore','')");
            }
...
*/
        $res = json_decode($content, true);
//        if (!empty(trim($content))) file_put_contents('./logs/rz/rz_'./*(isset($swapData['key'])?'':'key_').*/time().'.txt',$content);
/*
...
//                                    $obj++;
                        ...
                        $data['objects'] = new ResultDataField('string', 'objects', /*$obj*/ sizeof($item['properties']), 'Всего объектов залога', 'Всего объектов залога');
                    }
            ...
            if (isset($swapData['captcha_session'])) {
                ...
                $mysqli->query("INSERT INTO isphere.session (used,endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (1,now(),now(),".$this->search_sourceid.",3,'success','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
//                echo "Captcha ID ".$swapData['captcha_id']." from ".$swapData['captcha_service']." reported as good with result $res\n";
            ...
            if (strpos($res['message'],'token') || strpos($res['message'],'капча') || strpos($res['message'],'CAPTCHA')) {
                if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                    $mysqli->query("INSERT INTO isphere.session (used,endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (1,now(),now(),".$this->search_sourceid.",4,'invalidcaptcha','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
...
//                $mysqli->query("UPDATE isphere.session SET statuscode='captcha' WHERE id=" . $swapData['session']->id);
                ...
                if (!isset($swapData['row']) && isset($swapData['session']) && $swapData['iteration']%3==0) {
                ...
                file_put_contents('./logs/rz/rz_err_'.time().'.txt',$content);
                ...
                if (!$swapData['session']->nocaptcha)
                    $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3,statuscode='message' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
...
//                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
                ...
                if (isset($swapData['captcha_session'])) {
                    ...
                    $mysqli->query("INSERT INTO isphere.session (used,endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (1,now(),now(),".$this->search_sourceid.",4,'forbidden','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
...
//                unset($swapData['key']);
            ...
            if ($content) file_put_contents('./logs/rz/rz_err_'.time().'.txt',$content);
            if ($content) {
                if (!$swapData['session']->nocaptcha)
