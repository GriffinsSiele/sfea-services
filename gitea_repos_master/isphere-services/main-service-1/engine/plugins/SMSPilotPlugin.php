...
//            file_put_contents('./logs/smspilot/'.(!isset($swapData['id'])?'send':'check').'_'.time().'.txt', $content."\n\n".$swapData['iteration']);
            $ares = json_decode($content, true);

            if ($ares && !isset($swapData['id']) && isset($ares['send'][0]['server_id']) && intval($ares['send'][0]['server_id'])) {
                $swapData['id'] = intval($ares['send'][0]['server_id']);
            } elseif ($ares && isset($swapData['id']) && isset($ares['check'][0]['status']) && (intval($ares['check'][0]['status'])==0 || intval($ares['check'][0]['status'])==1)) {
                $swapData['iteration']--;
                ...
                $data['hlr_status'] = new ResultDataField('string','HLRStatus', intval($ares['check'][0]['status'])>0?'Доступен':'Недоступен', 'Статус', 'Статус абонента');
                ...
                file_put_contents('./logs/smspilot/'.(!isset($swapData['id'])?'send':'get').'_err_'.time().'.txt', $content."\n\n".$swapData['iteration']);
