...
class SberbankPlugin implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $result = $mysqli->query("SELECT id,cookies,server,starttime,lasttime FROM isphere.session WHERE sessionstatusid=2 AND sourceid=7 AND sourceaccessid IN (select sourceaccessid from sourceaccess WHERE userid IS NULL OR userid=(SELECT COALESCE(MasterUserId,id) FROM SystemUsers WHERE id=$userId)) ORDER BY lasttime limit 1");
...
//            $rContext->setError('Поиск производится только по российским телефонам');
        ...
        $phone = '('.substr($phone,1,3).') '.substr($phone,4,3).'-'.substr($phone,7,2).'-'.substr($phone,9,2);
        ...
        $form = $path.'payments/payment.do';
        $params = '?form=RurPayment&receiverSubType=ourCard';
        ...
        if(isset($swapData['post']) && isset($swapData['code'])){
            ...
            if(isset($swapData['post'])){
                $post = $swapData['post'];
                $post['sellAmount']='0.01';
//                $post['currency']='RUB';
                $post['buyAmountCurrency']=$post['currency'];
                $post['receiverSubType']='ourPhone';
                $post['externalPhoneNumber']=$phone;
                $post['exactAmount']='charge-off-field-exact';
                $post['externalCardCurrency']=$post['currency'];
                $post['ground']='';
                $post['operation']='button.save';
    ...
    public function computeRequest(&$rContext)
    {
        ...
        if(!$curl_error) {
            ...
            if (!isset($swapData['post'])) {
                file_put_contents('./logs/sberbank/sberbank_'.time().'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
		$content = iconv('windows-1251','utf-8',$content);

                if(preg_match("/<form name=\"EditPaymentForm\" .*?<\/form>/sim", $content, $matches)){
                    $post = array();
                    $form = $matches[0];
...
//                    $error = 'Ошибка при передаче запроса';
                ...
                $zelonow = time();
                file_put_contents('./logs/sberbank/sberbank_code_'.$zelonow.'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
	        $content = iconv('windows-1251','utf-8',$content);
                unset($swapData['code']);
...
//                        return true;
                ...
                $zelonow = time();
                file_put_contents('./logs/sberbank/sberbank_result_'.$zelonow.'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
		$content = iconv('windows-1251','utf-8',$content);
                $resultData = new ResultDataList();
                $data = array();
                        ...
                        foreach($matches[1] as $i => $label){
                            ...
                            if ($label=='ФИО') {
                                ...
                                if (isset($initData['first_name'])) {
                                    ...
                                    if ($this->str_uprus($initData['first_name'] . (isset($initData['patronymic']) ? ' '.$this->str_uprus($initData['patronymic']) :''))==$first_name) {
                            ...
                            if ($label=='Номер карты получателя') {
                                $card = strtr($matches[2][$i],array('•'=>'*','&bull;'=>'*'));
                                $data['card'] = new ResultDataField('string','card', $card, $label, $label);
                            }
                        }
                    }
                    $resultData->addResult($data);
                } elseif(preg_match("/\"itemDiv\">([^<]+)/", $content, $matches)){
                    $matches[1] = trim(html_entity_decode($matches[1]));
                    if (strpos($matches[1],'невозможно') || strpos($matches[1],'не подключен')) {
                        $resultData = new ResultDataList();
                        $rContext->setResultData($resultData);
                        $rContext->setFinished();
...
/*
                        ...
                        return true;
*/
                    } elseif (strpos($matches[1],'попытку') || strpos($matches[1],'лимит')) {
                        if (isset($swapData['session']))
                            $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 hour),sessionstatusid=6 WHERE id=" . $swapData['session']->id);
//                            $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3 WHERE id=" . $swapData['session']->id);
                        unset($swapData['session']);
                        ...
                        $rContext->setSwapData($swapData);
                        return true;
                    } elseif (strpos($matches[1],'Не удается')===false) {
                        $error = trim($matches[1]);
                    }
                } elseif(!preg_match("/\"\/PhizIC\/logoff.do\"/", $content)) {
                    if (isset($swapData['session']))
