...
//        array('host' => 'api.anti-captcha.com', 'key' => '63def0a149a147e0d13e409cc8318fc3'),
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query("UPDATE isphere.session s SET request_id=".$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=24 AND captcha>'' ORDER BY lasttime limit 1");
...
/*
        if (!isset($swapData['captcha_token']) && !isset($swapData['captcha_id'.$swapData['num']])) {
            $token = neuro_token('emobiletracker.com');
            if (strlen($token)>30) {
