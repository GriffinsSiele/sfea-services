...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('truecaller', $initData['phone']) && ($db->llen('truecaller_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//            $params = $swapData['params'];
            ...
            if($db->hexists('truecaller', $initData['phone'])){
                ...
                if($res && isset($res['status']) && strtoupper($res['status'])=='ERROR'){
                    $db->hdel('truecaller', $initData['phone']);
                }
                $db->close();
                unset($swapData['db']);
            }else{
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/truecaller/truecaller_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/truecaller/truecaller_err_'.time().'.txt',$content);
