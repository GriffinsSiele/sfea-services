...
//        return 'Поиск в Twitter';
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query("UPDATE session s SET request_id=$reqId WHERE sessionstatusid=2 AND sourceid=50 AND lasttime<DATE_SUB(now(), INTERVAL 10 SECOND) ORDER BY lasttime limit 1");
        ...
        if($result)
        {
            ...
            if ($row)
            {
                ...
                if (!$row->proxyid) {
//                    $result = $mysqli->query("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM isphere.proxy WHERE enabled=1 AND status=1 AND country='ru' AND (rotation>0 OR (SELECT COUNT(*) FROM session WHERE proxyid=proxy.id AND sourceid=50 AND sessionstatusid IN (1,2,6,7))<1) ORDER BY lasttime limit 1");
...
//                $initData['phone']='7'.substr($initData['phone'],1);
        ...
        if (isset($initData['url'])) {
            if (strpos($initData['url'],'twitter.com/')===false) {
                $rContext->setFinished();
                return false;
            }
...
//        curl_setopt($ch, CURLINFO_HEADER_OUT, true);
    ...
    public function computeRequest(&$rContext)
    {
        ...
        if(!$curl_error) {
            ...
            if (!isset($swapData['path'])) {
//                file_put_contents('./logs/twitter/twitter_'.time().'.html',/*curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".*/$content);

                if (strpos($content,'send_password_reset') || strpos($content,'verify_user_info') || strpos($content,'свою учетную') || strpos($content,'нашли несколько') || strpos($content,'нашли более') || strpos($content,'не можем найти') || strpos($content,"couldn't find") || strpos($content,'tidak dapat menemukan')) {
...
//                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='limit' WHERE id=" . $swapData['session']->id);
                    ...
                    unset($swapData['session']);
                    file_put_contents('./logs/twitter/twitter_err_proxy_'.time().'.txt',$content);
                } elseif (!$content) {
                    ...
                    file_put_contents('./logs/twitter/twitter_err_'.time().'.html',$content);
