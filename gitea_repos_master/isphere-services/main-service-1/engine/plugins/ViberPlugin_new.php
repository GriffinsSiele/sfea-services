...
//            $params = $swapData['params'];
            ...
            if($db->hexists('viber', $initData['phone'])){
                ...
                if($swapData['iteration']>=20){
                    ...
                    if ($swapData['iteration']>=10 && $swapData['iteration']%5==0) {
//                        $db->rpush('viber_queue', $initData['phone']);
                        $db->close();
                        unset($swapData['db']);
                    }
        ...
        file_put_contents('./logs/viber/viber_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
                ...
                foreach ($res['records'] as $row) if (is_array($row)) {
                    ...
                    foreach($row as $field) if (is_array($field) && isset($field['value'])) {
                        if ($field['field']=='Photo') {
                            file_put_contents('./logs/viber/'.$initData['phone'].'.jpg',base64_decode(substr($field['value'],22)));
                        }
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/viber/viber_err_'.time().'.txt',$content);
                if (!strpos($res['message'],'database'))
                    $error = $res['message'];
            } else {
                file_put_contents('./logs/viber/viber_err_'.time().'.txt',$content);
