...
class ViberWinPlugin implements PluginInterface
{
...
//            $error = /*($swapData['iteration']>3) && */ curl_error($rContext->getCurlHandler());
            ...
            file_put_contents('./logs/viber/viber_'.(isset($swapData['path'])?$swapData['path'].'_':'').time().'.txt',$content);
...
*/
            if($res && isset($res['status']) && $res['status']=='success' && isset($res['viberstatus']) && ($res['viberstatus']!='error' || $swapData['iteration']>10)){
                $resultData = new ResultDataList();
                if ($res['viberstatus']==1) {
                    $data = array();
...
*/
            ...
            } elseif($res && isset($res['data']) && $res['data']=="in queue already" && $swapData['iteration']<=5) {
                $rContext->setSleep(10);
                return true;
            } elseif($res && isset($res['data']) && $res['data']=="unconventional phone") {
                $error = "Некорректный номер телефона";
            } elseif($res && isset($res['data']) && $res['data']) {
                file_put_contents('./logs/viber/viber_err_'.(isset($swapData['path'])?$swapData['path'].'_':'').time().'.txt',$content);
            } else {
                file_put_contents('./logs/viber/viber_err_'.(isset($swapData['path'])?$swapData['path'].'_':'').time().'.txt',$content);
