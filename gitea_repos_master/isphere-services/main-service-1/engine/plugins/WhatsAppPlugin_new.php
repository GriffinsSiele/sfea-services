...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                if($db->hexists('whatsapp', $initData['phone']))
                    $db->hdel('whatsapp', $initData['phone']);
                if (!$db->hexists('whatsapp', $initData['phone']) && ($db->llen('whatsapp_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//                        $db->rpush('whatsapp_queue', $initData['phone']);
        ...
        file_put_contents('./logs/whatsapp/whatsapp_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                    $row = $res;
                foreach ($res['records'] as $row) if (is_array($row)) {
                    ...
                    foreach($row as $field) if (is_array($field) && isset($field['value'])) {
                        if ($field['field']=='FullPhoto') {
                            file_put_contents('./logs/whatsapp/'.$initData['phone'].'.jpg',file_get_contents($field['value']));
                        }
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/whatsapp/whatsapp_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/whatsapp/whatsapp_err_'.time().'.txt',$content);
