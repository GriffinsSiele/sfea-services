...
//            $initData['phone']='7'.substr($initData['phone'],1);
    ...
    public function computeRequest(&$rContext)
    {
        ...
        if (!$content) {
            ...
            file_put_contents('./logs/whatsapp/whatsapp_'.(isset($initData['phone'])?$initData['phone'].'_':'').time().'.txt',$content);
...
*/
            if($res && isset($res['status']) && $res['status']=='success'){
                ...
                $data = array();
                if(strpos($res['data'],'https')===0 || strpos($res['data'],'default')===0){
                    $data['phone'] = new ResultDataField('string','phone',$initData['phone'],'Телефон','Телефон');
                    if (isset($res['lasttime']))
                        $data['lasttime'] = new ResultDataField('string','lasttime',$res['lasttime'],'Время последней активности','Время последней активности');
                    if (strpos($res['data'],'https')===0) {
                        if (isset($res['base64']) && $res['base64']!='error')
            ...
            } elseif($res && isset($res['data']) && ($res['data']=="unconventional phone" || $res['data']=="invalid phone")) {
                $error = "Некорректный номер телефона";
            } elseif($res && isset($res['data']) && $res['data']) {
                file_put_contents('./logs/whatsapp/whatsapp_err_'.(isset($initData['phone'])?$initData['phone'].'_':'').time().'.txt',$content);
            } else {
                file_put_contents('./logs/whatsapp/whatsapp_err_'.(isset($initData['phone'])?$initData['phone'].'_':'').time().'.txt',$content);
