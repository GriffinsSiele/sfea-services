...
//                $db->connect($keydb['server'],6379,$keydb['connect_timeout'],NULL,100,$keydb['read_timeout']);
                ...
                $db->auth($keydb['auth']);
                if (!$db->hexists('yamap', $initData['phone']) && ($db->llen('yamap_queue')>20)) {
                    $db->close();
                    unset($swapData['db']);
...
//        file_put_contents('./logs/yamap/yamap_'.time().'.txt',$content);
        if (!$content) {
                $rContext->setFinished();
...
//                        if ($field['field']=='Name')
                ...
                file_put_contents('./logs/yamap/yamap_err_'.time().'.txt',$content);
                $error = $res['message'];
            } else {
                file_put_contents('./logs/yamap/yamap_err_'.time().'.txt',$content);
