...
class YaMoneyPlugin implements PluginInterface
{
    ...
    {
        return 'Яндекс - проверка телефона на наличие электронного кошелька';
    }
...
*/
        ...
        $header = array(
          ...
          'Referer: https://1.elecsnet.ru/NotebookFront/services/0mhp/default.aspx?merchantId=47',
          'X-Requested-With: XMLHttpRequest',
        );
        $params = array(
            "merchantId" => 47,
            "paymentTool" => 44,
            "merchantFields[1]" => $initData['phone'],
        );
...
//            file_put_contents('./logs/elecsnet/yamoney_'.time().'.txt',$content);

            $res = json_decode($content, true);               
            if($res && isset($res['message']) && !strpos($res['message'],'невозможно')){
                $resultData = new ResultDataList();
                if (isset($res['isSuccess'])){
                    if ($res['isSuccess'] || strpos($res['message'],'анонимного') || strpos($res['message'],'заблокирован')) {
                        $data['result'] = new ResultDataField('string','result', 'По телефону '.$initData['phone'].' найден электронный кошелек Яндекс.Деньги', 'Результат', 'Результат');
                        $data['result_code'] = new ResultDataField('string','result_code', 'FOUND', 'Код результата', 'Код результата');
                        if (!strpos($res['message'],'заблокирован'))
                            $data['identified'] = new ResultDataField('string','identified', $res['isSuccess']?'Да':'Нет', 'Идентифицированный', 'Идентифицированный');
                        $data['locked'] = new ResultDataField('string','locked', strpos($res['message'],'заблокирован')?'Да':'Нет', 'Заблокирован', 'Заблокирован');
                ...
                if ($content) {
                    file_put_contents('./logs/elecsnet/yamoney_err_'.time().'.txt',$content);
