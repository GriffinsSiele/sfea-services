...
//              print_r($result);
		...
		echo "<td>";
		     ...
		     if(isset($request->CarReq)){
		           ...
		           foreach($prequest as $key => $val){
		                if ($val && !is_array($val)) echo $key.": ".$val."<br />";
			   }
		     }
...
//		$response = simplexml_load_string($result['response']);
		echo "<td>";
                if ($request) {
		    echo '<a href="showresult.php?id='.$result['id'].'" target=_blank>Просмотр</a><br/><a href="showresult.php?id='.$result['id'].'&mode=pdf" target=_blank>PDF</a>&nbsp;<a href="showresult.php?id='.$result['id'].'&mode=xml" target=_blank>XML</a>';
                } else {
      ...
      echo '<a href="history.php?'.$querystr.'&maxid='.($maxid?$maxid:'').'"> << </a> ';
      if ($sqlRes && ($sqlRes->num_rows==$limit))
          echo '<a href="history.php?'.$querystr.'&minid='.$minid.'"> >> </a>';
