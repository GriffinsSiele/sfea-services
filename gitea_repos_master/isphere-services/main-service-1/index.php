...
include ('engine/RequestManager.php');
...
if($userId && ($result = mysqli_query($mysqli, "SELECT sum(user_id=$userId) processing,COUNT(*) total_processing FROM RequestNew WHERE created_at>date_sub(now(),interval 5 minute) AND status=0"))){
    ...
    if ($processing>=($userId==4169?50:($userId==3677?10:30))) {
        echo '
        <error>
            <code>429</code>
...
if ($userId==2430) $total_timeout = 30;
if ($userId==4431) $total_timeout = 55;
...
/*
        if ($status && ($clientId==264 || $clientId==265)) {
...
*/
        if ($status /*&& ($clientId==264 || $clientId==265)*/) {
            ...
            if (curl_errno($ch)) {
                $error = curl_error($ch);
                file_put_contents('./logs/isto-'.date('Y-m-d').'.log',date('Y-m-d H:i:s')." id=$reqId processtime=$process_time error $error\n",FILE_APPEND);
            } else {
                file_put_contents('./logs/isto-'.date('Y-m-d').'.log',date('Y-m-d H:i:s')." id=$reqId processtime=$process_time success\n",FILE_APPEND);
            }
            curl_close($ch);
        }
        return $response;
