...
//      $mysqli = mysqli_connect ($database['server'],$database['login'],$database['password'], $database['name']);
      ...
      if (isset($_REQUEST['action']) && isset($_REQUEST['id'])) {
          if ($_REQUEST['action']=='delete') {
              $mysqli->query("DELETE FROM session WHERE id=".intval($_REQUEST['id']));
          } elseif ($_REQUEST['action']=='report' && isset($_REQUEST['captcha_id'])) {
              $apikey = 'd167c71a9278312f184f17caa4e71050'; //rucaptcha
              $ch = curl_init();
              curl_setopt($ch, CURLOPT_URL, "http://rucaptcha.com/res.php?key=$apikey&action=reportbad&id=".intval($_REQUEST['captcha_id']));
              curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
              $answer = curl_exec($ch);
              curl_close($ch);
              if ($answer=='OK_REPORT_RECORDED')
                  $mysqli->query("UPDATE session SET captcha_reporttime=now() WHERE id=".intval($_REQUEST['id']));
          }
      }
...
//              print_r($result);
                ...
                if ($result['captcha_service']=='rucaptcha.com' && !$result['captcha_reporttime'] && substr($result['captcha'],0,6)!='ERROR_')
