    ...
    function init_<xsl:value-of select="../../@request_id"/>_<xsl:value-of select="$record_number"/>_<xsl:value-of select="./FieldName"/>(){     
        ...
        Map_<xsl:value-of select="../../@request_id"/>_<xsl:value-of select="$record_number"/>_<xsl:value-of select="./FieldName"/>.geoObjects.add(Col);
				...
				<xsl:if test="@status=1">
                                Результаты получены ___servicename___ <xsl:value-of select="concat(substring(@datetime,9,2),'.',substring(@datetime,6,2),'.',substring(@datetime,1,4),' в ',substring(@datetime,12,8))"/><br/>
				<img src="https://metrics.i-sphere.ru/watch/12345?u=___login___"  style="position:absolute; left:-9999px;" alt="" />
				</xsl:if>
			</body>
		</html>
	</xsl:template>
