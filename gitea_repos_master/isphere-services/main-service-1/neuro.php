...
function nn_decode($image, $source, $provider='nnetworks')
{
    ...
    do {
        ...
        if ($result) {
            ...
            file_put_contents('./logs/nn-'.date('Y-m-d').'.log',date('Y-m-d H:i:s')." id=$reqId source=$source timeout=".$nn_trytimeout[$tries]." errors=$nn_error {$error['message']}\n",FILE_APPEND);
        }
    } while (!$result && ++$tries<sizeof($nn_trytimeout));
    $process_time = number_format(microtime(true)-$start,2,'.','');
    file_put_contents('./logs/nn-'.date('Y-m-d').'.log',date('Y-m-d H:i:s')." id=$reqId source=$source processtime=$process_time $result\n",FILE_APPEND);
