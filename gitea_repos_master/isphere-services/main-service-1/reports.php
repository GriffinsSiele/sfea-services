...
//      }
      ...
      echo '<option value="seconds"'.($type=='seconds'?' selected':'').'>По секундам</option>';
      ...
      if ($user_level<0) {
          ...
          echo '<option value="statuses"'.($type=='statuses'?' selected':'').'>По статусам</option>';
          echo '<option value="reqtypes"'.($type=='reqtypes'?' selected':'').'>По типам запросов</option>';
          echo '<option value="process"'.($type=='process'?' selected':'').'>По длительности</option>';
      }
      echo '</select>';
/*
      if ($user_level<0) {
...
//          "params" => "start_param",
          ...
          "statuses" => "req.status",
          "reqtypes" => "req.type",
          "process" => "CEILING(r.process_time) process",
      );
...
} elseif (/*$user_level>=0 && */!(isset($_REQUEST['source']) && $_REQUEST['source']) && !(isset($_REQUEST['checktype']) && $_REQUEST['checktype']) && in_array($_REQUEST['client_id'],array('','6','15','19','25','221','230','292','303')) && date('01.m.Y',strtotime($_REQUEST['from']))!=date('01.m.Y',strtotime($_REQUEST['to']))) {
      echo "Получение статистики временно ограничено одним календарным месяцем. Приносим извинения за неудобства.<br/>Если вам нужна статистика за более длительный период, напишите нам в онлайн-чат или на e-mail <a href=\"mailto:support@i-sphere.ru\">support@i-sphere.ru</a>";
} else {
      $field = $fields[$type];
...
//              $conditions .= ' AND created_at <= str_to_date(\''.date('Y-m-d H:i:s',strtotime($_REQUEST['to'])).'\', \'%Y-%m-%d %H:%i:%s\')';
      ...
      if(isset($_REQUEST['source']) && $_REQUEST['source']){
          $res_conditions .= ' AND r.source_name=\''.mysqli_real_escape_string($mysqli,$_REQUEST['source']).'\'';
      }
      if(isset($_REQUEST['checktype']) && $_REQUEST['checktype']){
          $res_conditions .= ' AND r.checktype=\''.mysqli_real_escape_string($mysqli,$_REQUEST['checktype']).'\'';
      }
      ...
      if($user_level<0 && isset($_REQUEST['status']) && $_REQUEST['status']!=''){
          $rep_conditions .= ' AND req.status = '.intval($_REQUEST['status']);
      }
      if($user_level<0 && isset($_REQUEST['reqtype']) && ($_REQUEST['reqtype']=='api' || $_REQUEST['reqtype']=='bulk' || $_REQUEST['reqtype']=='check')){
          $rep_conditions .= ' AND req.type '.($_REQUEST['reqtype']=='api'?'IS NULL':($_REQUEST['reqtype']=='bulk'?' = "bulk"':'LIKE "check%"'));
      }
      ...
      $addfields = ($type=='dates'||$type=='months'||strpos($conditions,'created_date')?'created_date,':'')./*(strpos($conditions,'created_at')?'created_at,':'').*//*($type=='sources'?'source_name,':'').($type=='clients'?'client_id,':'').($type=='users'?'user_id,':'').*/($type=='checktypes'||strpos($res_conditions,'checktype')?'checktype,':'');
      $addgroups = $addfields;
...
//      $mysqli->query($sql);
      ...
      if ($viewtype=="sources" || $viewtype=="checktypes") {
          $srcfields .= ', SUM(r.found_count>0) hit';
      }
      if ($type!='process') {
          $srcfields .= ', AVG(r.process_time) avgprocess';
      }
      if ($type=="users") {
          $field2 .= "$u.id user_id,";
          $group2 .= ",$u.id";
      ...
      if (($user_level<0 && $type!='checktypes' && $type!='sources') || strpos($rep_conditions,'req.')) {
          $r = 'req';
          $rid = 'id';
          $conditions = strtr($conditions,array('r.'=>'req.'));
          $r_to_req = array(
              'r.created_date' => 'req.created_date',
              'r.created_at' => 'req.created_at',
              'r.hour' => 'hour(req.created_at) as hour',
              'r.minute' => 'minute(req.created_at) as minute',
              'r.process_time' => 'TIMESTAMPDIFF(SECOND,req.created_at,req.processed_at)+1',
          );
          if ($viewtype!='checktypes' && $viewtype!='sources') {
              $field = strtr($field,$r_to_req);
              $srcfields = strtr($srcfields,$r_to_req);
          }
          if ($type=="ips") $group2 .= ',req.ip';
          if ($type=="statuses") $group2 .= ',req.status';
          if ($type=="reqtypes") $group2 .= ',req.type';
          $join2 .= ($viewtype!='checktypes' && $viewtype!='sources'?'RIGHT ':'').'JOIN RequestNew req ON r.request_id=req.id';
      }
      if ($type=="sources" || $type=="checktypes") {
          $field .= ",".$fields["params"];
          $group2 .= ',2';
      }
...
SQL;
          $field2 .= $type=='users' || $type=='clients' || $user_area<=1 || (isset($_REQUEST['user_id']) && $_REQUEST['user_id']) || (isset($_REQUEST['client_id']) && $_REQUEST['client_id']!="") ? 'IFNULL((SELECT MIN(price) FROM UserSourcePrice WHERE (user_id=u.id OR user_id=m.id) AND source_name=r.source_name),IFNULL(u.DefaultPrice,m.DefaultPrice)) price,' : '';
          $group2 .= $field2!='' ? ',price' : '';
      }
...
$field,
$field2
COUNT(DISTINCT $r.$rid) reqcount,
COUNT(r.request_id) rescount,
...
*/
      ...
      while($result = $sqlRes->fetch_assoc()){
                if ($viewtype=="sources" || $viewtype=="checktypes") {
                    $result["successrate"] = $result["rescount"]>=10?$result["success"]/$result["rescount"]*100:"";
                    $result["hitrate"] = $result["success"]>=10?$result["hit"]/$result["success"]*100:"";
                }
//              print_r($result);
                if ($i==0) {
                    $first = $result;
...
*/
                ...
                foreach ($result as $key => $val) {
                    if (!isset($hide[$viewtype][$key]) && isset($title[$key])) {
                        if (isset($total[$key]) && !preg_match("/rate$/",$key)) {
                            $total[$key]+=(preg_match("/^avg/",$key)?$result["rescount"]:1)*$val;
                        }
                        if ($key=="status" && isset($statuses[$val])) $val = $statuses[$val];
                        if ($key=="type" && !$val) $val = 'api';
                        echo "<td ".(is_numeric($val)?"class=\"right\"":"").">";
                        if ($key=="avgprocess") $val = number_format($val,1,',','');
                        if (($key=="price" || $key=="total" || preg_match("/rate$/",$key)) && strlen($val)) $val = number_format($val,2,',','');
                        $params = false;
                        if ($key=="created_date") {
                            $params = $_REQUEST;
                            $params['from']=$val;
                        ...
                        if ($key=="minute" && isset($_REQUEST['from']) && strtotime($_REQUEST['from']) && ((isset($_REQUEST['to']) && strtotime($_REQUEST['to']) && date('d.m.Y H',strtotime($_REQUEST['from']))==date('d.m.Y H',strtotime($_REQUEST['to']))) || ((!isset($_REQUEST['to']) || !$_REQUEST['to']) && date('d.m.Y H',strtotime($_REQUEST['from']))==date('d.m.Y H')))) {
                            ...
                            $params['type']=(!isset($_REQUEST['source']) || !$_REQUEST['source']) && (!isset($_REQUEST['checktype']) || !$_REQUEST['checktype'])?'sources':(!isset($_REQUEST['user_id']) || !$_REQUEST['user_id']?'users':'seconds');
                        }
...
//                        } 
                        ...
                        if ($params && ($params['type']=='hours' || $params['type']=='minutes') && isset($params['from']) && strtotime($params['from']) && ((isset($params['to']) && strtotime($params['to']) && date('d.m.Y H',strtotime($params['from']))==date('d.m.Y H',strtotime($params['to'])-1)) || (!isset($params['to']) && date('d.m.Y H',strtotime($params['from']))==date('d.m.Y H')))) {
                            ...
                            echo '<a href="reports.php?'.http_build_query($params).'">';
                        echo $val;
                        if ($params) echo '</a>';
                        echo "</td>";
                    }
...
*/
      ...
      if ($i == 0) {
          echo "Нет данных";
                ...
                if ((isset($_REQUEST['source']) && $_REQUEST['source']) || (isset($_REQUEST['checktype']) && $_REQUEST['checktype'])) {
                    $total["successrate"] = $total["rescount"]>=10?$total["success"]/$total["rescount"]*100:"";
                    $total["hitrate"] = $total["success"]>=10?$total["hit"]/$total["success"]*100:"";
                }
                foreach ($first as $key => $val) {
                    if (!isset($hide[$viewtype][$key]) && isset($title[$key])) {
                        ...
                        if (isset($total[$key])) {
                            $val = $total[$key];
                            if (substr($key,0,3)=="avg" && $total["rescount"]) $val = $val/$total["rescount"];
                            if (is_numeric($val)) {
                                if ($key=="avgprocess") $val = number_format($val,1,',','');
                                if ($key=="total" || preg_match("/rate$/",$key)) $val = number_format($val,2,',','');
                            }
                        }
                        echo "<td class=\"right\"><b>".$val."</b></td>";
                    }
                }
/*
                if ((isset($_REQUEST['source']) && $_REQUEST['source']) || (isset($_REQUEST['checktype']) && $_REQUEST['checktype'])) {
                ...
                } elseif ($viewtype=="sources" || $viewtype=="checktypes") {
