...
//      $mysqli = mysqli_connect ($database['server'],$database['login'],$database['password'],$database['name']);
      ...
      if ($id) {
          ...
          if (!$result || !mysqli_result($result, 0)){
            ...
            if(resource_exists('/opt/xml/'.$titles[0].'/'.$titles[1].'/'.$titles[2].'_res.xml')){
                  $theResult = resource_get_contents('/opt/xml/'.$titles[0].'/'.$titles[1].'/'.$titles[2].'_res.xml');
                  if (!$theResult) {
                      usleep(10);
                      $theResult = resource_get_contents('/opt/xml/'.$titles[0].'/'.$titles[1].'/'.$titles[2].'_res.xml');
                  }
...
//                    http_response_code(404);
            ...
            if (isset($_REQUEST['mode']) && $_REQUEST['mode']=='xml') {
                ...
                $doc = xml_transform(strtr($theResult,array('request>'=>'Request>')), isset($_REQUEST['mode']) && $_REQUEST['mode']=='pdf' ? 'isphere_view_pdf_new.xslt' : 'isphere_view_new.xslt');
                if ($doc){
