...
$check_sources = [
  'fssp' => ['ФССП', 1, 1, 0],
  'fsspapi' => ['ФССП API', 1, 1, 1],
  'rsa_bsostate' => ['РСА полис', 1, 1, 1],
];

if (!isset($_REQUEST['fsspip'])) {
    $_REQUEST['fsspip'] = '';
}
...
<h1>Поиск по дополнительным данным</h1><hr/><a href="admin.php">Назад</a><br/><br/>
<form id="checkform" method="POST">
    <table>
        <tr>
            <td>Номер ИП в ФССП</td>
            <td>
                <input type="text" name="fsspip" value="<?php echo $_REQUEST['fsspip']; ?>" maxlength="100" />
            </td>
        </tr>
        <tr>
            <td>Полис ОСАГО</td>
            <td>
                <input type="text" name="osago" value="<?php echo $_REQUEST['osago']; ?>" maxlength="100" />
            </td>
        </tr>
        <!--tr>
...
.(!isset($_REQUEST['request_id']) || !$_REQUEST['request_id'] ? '' : "
        ...
        <requestType>checkother</requestType>
        ...
        <OtherReq>'
.(!isset($_REQUEST['fsspip']) || !$_REQUEST['fsspip'] ? '' : "
            <fsspip>{$_REQUEST['fsspip']}</fsspip>"
