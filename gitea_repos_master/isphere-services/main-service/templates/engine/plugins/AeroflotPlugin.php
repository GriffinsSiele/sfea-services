...
class AeroflotPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в Aeroflot', 'aeroflot_phone' => 'Aeroflot - проверка телефона на наличие пользователя', 'aeroflot_email' => 'Aeroflot - проверка email на наличие пользователя'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $reqId = $params['_reqId'];
        $sessionData = null;
        $connection->executeQuery("UPDATE session s SET request_id={$reqId} WHERE sessionstatusid=2 AND sourceid=38 AND unix_timestamp(now())-unix_timestamp(lasttime)>1 ORDER BY lasttime limit 1");
        $result = $connection->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sourceid=38 AND request_id={$reqId} ORDER BY lasttime limit 1");
        if ($result) {
            ...
            if ($row) {
                ...
                $connection->executeQuery("UPDATE session SET lasttime=now(),used=ifnull(used,0)+1,statuscode='used',request_id=NULL WHERE id=".$sessionData->id);
                //                $connection->executeQuery("UPDATE proxy SET lasttime=now() WHERE id=".$row['proxyid']);
                if (!$row['proxyid']) {
                    ...
                    if ($result) {
                        ...
                        if ($row) {
                            ...
                            $sessionData->proxy_auth = \strlen($row['proxy_auth']) > 1 ? $row['proxy_auth'] : false;
                            //                            $connection->executeQuery("UPDATE proxy SET lasttime=now() WHERE id=".$row['proxyid']);
                            $connection->executeQuery('UPDATE session SET proxyid='.$row['proxyid'].' WHERE id='.$sessionData->id);
                        }
                    }
                }
            }
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $header = ['Accept: application/json', 'Origin: https://www.aeroflot.ru', 'Referer: https://www.aeroflot.ru/', 'Content-Type: application/json', 'Authorization: Bearer null', 'X-IBM-Client-Id: 52965ca1-f60e-46e3-834d-604e023600f2', 'X-IBM-Client-Secret: rU0gE3yP1wV0dY6nJ8kY8pD6pI5dF7xP5nH5nR4cH3sC0rK2rR'];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (\strpos($error, 'timed out') || \strpos($error, 'connection')) {
            $error = false;
            //                $connection->executeQuery("UPDATE session SET unlocktime=date_add(now(),interval 10 minute),sessionstatusid=6,statuscode='connectionerror' WHERE id=" . $swapData['session']->id);
        }
        if (!$error) {
            $content = \curl_multi_getcontent($rContext->getCurlHandler());
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/aeroflot/aeroflot_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
            ...
            //                if ($arr[0]=='www.aeroflot.ru')
                        ...
                        $rContext->setSwapData($swapData);
                        $connection->executeQuery("UPDATE session SET cookies='$new_cookies' WHERE id=" . $swapData['session']->id);
            */
        }
        if (isset($res) && \is_array($res) && isset($res['isSuccess']) && $res['isSuccess']) {
            $resultData = new ResultDataList();
            $mysqli->executeQuery("UPDATE session SET success=ifnull(success,0)+1,statuscode='success' WHERE id=".$swapData['session']->id);
            ...
            /*
                        ...
                        $connection->executeQuery("UPDATE session SET sessionstatusid=3,statuscode='failed',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
                    } elseif (strpos($content,'Access Denied')) {
                        $connection->executeQuery("UPDATE session SET sessionstatusid=3,statuscode='denied',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
                    } elseif (strpos($content,'403 Forbidden')) {
                        $connection->executeQuery("UPDATE session SET sessionstatusid=3,statuscode='forbidden',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
                    } elseif (strpos($content,'заблокирован')) {
                        $connection->executeQuery("UPDATE session SET sessionstatusid=3,statuscode='blocked',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
            */
            ...
            if ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/aeroflot/aeroflot_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
        } else {
            $mysqli->executeQuery("UPDATE session SET unlocktime=date_add(now(),interval 2 minute),sessionstatusid=6,statuscode='error' WHERE id=".$swapData['session']->id);
            //            $connection->executeQuery("UPDATE session SET sessionstatusid=3,statuscode='error',endtime=now() WHERE id=" . $swapData['session']->id);
            if ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/aeroflot/aeroflot_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
        }
