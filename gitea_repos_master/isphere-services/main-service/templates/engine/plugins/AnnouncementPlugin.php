...
class AnnouncementPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск по архиву объявлений', 'announcement_phone' => 'Архив объявлений - поиск по телефону'];
...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/announcement/announcement_'.time().'.txt',$content);
        if (!$content) {
            $db->close();
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/announcement/announcement_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/announcement/announcement_err_'.\time().'.txt', $content);
