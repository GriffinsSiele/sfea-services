...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
        ...
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/avito/avito_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/avito/avito_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/avito/avito_err_'.\time().'.txt', $content);
