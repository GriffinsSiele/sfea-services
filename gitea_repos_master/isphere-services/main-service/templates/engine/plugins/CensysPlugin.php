...
class CensysPlugin implements PluginInterface
{
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/censys/censys_'.time().'.json',$content);
        ...
        if ($res && isset($res['result']['ip'])) {
            ...
            if (isset($res['location']['coordinates']['latitude']) && isset($res['location']['coordinates']['longitude']) && $res['location']['coordinates']['latitude'] && $res['location']['coordinates']['longitude']) {
                $map = [['coords' => [(float) $res['location']['coordinates']['latitude'], (float) $res['location']['coordinates']['longitude']], 'text' => $locationtext]];
            ...
            if (isset($res['dns']['names']) && \count($res['dns']['names'])) {
            ...
            */
            ...
            if (isset($res['services']) && \count($res['services'])) {
                $data = [];
            ...
            if (false !== \strpos($res['error'], 'don\'t know')) {
                $resultData = new ResultDataList();
                $rContext->setResultData($resultData);
                $rContext->setFinished();
            } else {
