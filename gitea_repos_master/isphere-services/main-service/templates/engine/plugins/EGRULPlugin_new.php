...
class EGRULPlugin_new implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId.' WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=9 AND unix_timestamp(now())-unix_timestamp(lasttime)>=3 ORDER BY lasttime limit 1');
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            if (0 == \strlen($content) || false !== \strpos($content, 'Checking your browser before accessing') || false !== \strpos($content, 'Lame Page')) {
                if ($content) {
                    \file_put_contents('./logs/egrul/egrul_err_'.\time().'.html', $content);
                }
                ...
                \file_put_contents('./logs/egrul/egrul_captcha_'.\time().'.html', $content);
                ...
                \file_put_contents('./logs/egrul/egrul_locked_'.\time().'.html', $content);
                ...
                \file_put_contents('./logs/egrul/egrul_search_'.\time().'.html', $content);
                ...
                if (\preg_match("/(найдено [\d]+.*?)<form id=\"form-filters\"/sim", $content, $matches)) {
                    ...
                    if ($found > 100) {
                        ...
                        foreach ($parts as $i => $dataPart) {
                            \file_put_contents('./logs/egrul/egrul_search_part'.$i.'_'.\time().'.html', $dataPart);
                            $data = [];
                ...
                \file_put_contents('./logs/egrul/egrul_'.\time().'.html', $content);
                $resultData = new ResultDataList();
