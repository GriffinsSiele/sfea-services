...
class EMTPlugin1 implements PluginInterface
{
...
//        array('host' => 'api.capmonster.cloud', 'key' => 'afb26cbb248d650ea8b8d88822984242'),
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=24 AND (statuscode<>'used' OR lasttime<from_unixtime(unix_timestamp(now())-60)) ORDER BY lasttime limit 1");
            ...
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/emt/captcha_err_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
