...
class EMTPlugin_new implements PluginInterface
{
...
//        array('host' => 'api.anti-captcha.com', 'key' => '63def0a149a147e0d13e409cc8318fc3'),
    ...
    public function getSessionData($sourceid = 24)
    {
        global $mysqli;
        global $reqId;
        $sessionData = null;

        $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=$sourceid AND captcha>'' AND unix_timestamp(now())-unix_timestamp(captchatime)<110 AND (statuscode<>'used' OR lasttime<from_unixtime(unix_timestamp(now())-60)) ORDER BY lasttime limit 1");
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=24 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
...
*/
    ...
    public function computeRequest(array $params, &$rContext)
    {
            ...
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \file_put_contents('./logs/emt/captcha_err_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
        ...
        if (!$error) {
            $content = \curl_multi_getcontent($rContext->getCurlHandler());
            \file_put_contents('./logs/emt/emt_'.\time().'.html', $content);
            ...
            if (\preg_match('/<strong>   Name:([^<]+)</', $content, $matches)) {
                ...
                \file_put_contents('./logs/emt/emt_err_'.\time().'.html', $content);
                if (!$content) {
