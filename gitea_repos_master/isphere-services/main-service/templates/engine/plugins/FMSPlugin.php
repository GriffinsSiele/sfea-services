...
class FMSPlugin implements PluginInterface
{
    ...
    public function getSessionData(array $params, $sourceid = 1)
    {
        ...
        $sessionData = null;
        if ($sourceid) {
            $mysqli->executeStatement("UPDATE session s SET lasttime=now(),request_id={$reqId} WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid={$sourceid} ORDER BY lasttime limit 1");
            ...
            $result = $mysqli->executeQuery("SELECT 0 id,'' cookies,now() starttime,now() lasttime,'' captcha,'' token,id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM proxy WHERE status=1 AND proxygroup=1 ORDER BY lasttime limit 1");
        }
        if ($result) {
            ...
            if ($row) {
                ...
                if ($sourceid) {
                    $mysqli->executeStatement("UPDATE session SET lasttime=now(),used=ifnull(used,0)+1,statuscode='used',request_id=NULL".($row['captcha'] && 0 ? ',endtime=now(),sessionstatusid=3' : '').' WHERE id='.$sessionData->id);
                } else {
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if (!isset($swapData['db'])) {
            $params = ['sid' => '2000', 'form_name' => 'form', 'DOC_SERIE' => $initData['passport_series'], 'DOC_NUMBER' => $initData['passport_number'], 'captcha-input' => $swapData['session']->code];
            $url = 'http://сервисы.гувм.мвд.рф/info-service.htm';
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fms/fms_'.time().'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content."\n\n".$swapData['iteration']);
        if (!$error) {
            if (isset($swapData['db'])) {
                /*
                                if ($content) {
            ...
            $start = \strpos($content, '<div class="m_b35">');
            // Ищем результат запроса
            if (false !== $start) {
                $start += 19;
                ...
                if (isset($swapData['session'])) {
                    $mysqli->executeStatement("UPDATE session SET statuscode='success',success=ifnull(success,0)+1 WHERE statuscode='used' AND id=".$swapData['session']->id);
                }
            } else {
                ...
                if (false !== $start) {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fms/fms_err_'.\time().'.html', $content);
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='unexpected' WHERE id=".$swapData['session']->id);
                    }
                } elseif (\strpos($content, 'internal error') || \strpos($content, 'nginx')) {
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fms/fms_err_'.\time().'.html', $content);
                //                    if (isset($swapData['session']))
                //                        $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='invalidanswer' WHERE id=" . $swapData['session']->id);
                } elseif ($content) {
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fms/fms_err_'.\time().'.html', $content);
