...
class FSSPAppPlugin implements PluginInterface
{
    ...
    public function getSessionData(array $params, $generate = 1)
    {
        ...
        /*
                if ($generate) {
                    $result = $mysqli->query("SELECT COUNT(*) count FROM session WHERE sessionstatusid=2 AND sourceid=13 AND unix_timestamp(now())-unix_timestamp(lasttime)>5");
                    if($result) {
        ...
        */
        $mysqli->executeStatement('UPDATE session s SET request_id='.$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=13 AND token>'' AND unix_timestamp(now())-unix_timestamp(lasttime)>5 ORDER BY lasttime limit 1");
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sourceid=13 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
        if ($result) {
            $row = $result->fetchAssociative();
            if ($row) {
                ...
                /*
                                if ($generate) {
                                    ...
                                    $mysqli->query("UPDATE session SET endtime=now(),statuscode='exhausted',sessionstatusid=3 WHERE used>=10 AND id=".$row['id']);
                                }
                */
            }
        }
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fsspapp/fssp_'.\time().'_'.$swapData['iteration'].'.txt', $content);
        ...
        if (isset($data['data']['captcha'])) {
            if (isset($swapData['session']) && $swapData['session']->id) {
                $mysqli->executeStatement("UPDATE session SET endtime=now(),statuscode='exhausted',sessionstatusid=3 WHERE id=".$swapData['session']->id);
            }
            unset($swapData['session']);
        } elseif (isset($data['error_name']) && 'Большое количество запросов' == $data['error_name']) {
            if (isset($swapData['session']) && $swapData['session']->id) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='limit' WHERE id=".$swapData['session']->id);
            }
            //                $mysqli->query("UPDATE session SET endtime=now(),statuscode='exhausted',sessionstatusid=3 WHERE id=" . $swapData['session']->id);
            unset($swapData['session']);
        } elseif (isset($data['error_name']) && 'Успешно' == $data['error_name'] && isset($data['data']['list'])) {
            if (isset($swapData['session']) && $swapData['session']->id) {
            ...
            foreach ($data['list'] as $element) {
                if (isset($element['name'])) {
                    ...
                    if (\preg_match('/^(.*?) (\\d{2}\\.\\d{2}\\.\\d{4})(.*?)$/', $element['name'], $matches)) {
                        $debtor = \trim($matches[1]);
                        $debtor_birthdate = \trim($matches[2]);
                        $debtor_birthplace = \trim($matches[3]);
                    } elseif (\strpos($element['name'], ',') > 12) {
                        $debtor = \trim(\substr($element['name'], 0, \strpos($element['name'], ',') - 11));
                        $debtor_birthdate = \substr($element['name'], \strpos($element['name'], ',') - 10, 10);
                        $debtor_birthplace = \trim(\substr($element['name'], \strpos($element['name'], ',') + 1));
                    } else {
                        $debtor = \trim($element['name']);
                        $debtor_birthdate = '';
                        $debtor_birthplace = '';
                    }
                    ...
                    if (\preg_match('/^(.*?) \\d{6},/', $department, $matches)) {
                        $department_address = \trim(\substr($department, \strlen($matches[1]) + 1));
                        $department = $matches[1];
                    }
                    $bailiff = \explode('<br>', $element['bailiff']);
                    if (\preg_match('/^([А-Яа-яЁё\\-\\.\\s]+) [0-9\\+\\-\\(\\)\\s]+$/', $bailiff[0], $matches)) {
                        $bailiff_name = \trim($matches[1]);
                        $bailiff[0] = \substr($bailiff[0], \strlen($matches[1]) + 1);
                    } else {
                        $bailiff_name = \trim($bailiff[0]);
                        \array_shift($bailiff);
                    }
                    $record['name'] = new ResultDataField('string', 'Debtor', \strtr($debtor, ["\u{a0}" => '']), 'Должник', 'Должник');
                    if ($debtor_birthdate) {
                    ...
                    $record['case_date'] = new ResultDataField('string', 'CaseDate', \substr(\trim($case[1]), 0, 10), 'Дата ИП', 'Дата исполнительного производства');
                    if (\strlen(\trim($case[1])) > 11) {
                        $record['summary_case_num'] = new ResultDataField('string', 'SummaryCaseNumber', \substr(\trim($case[1]), 11), 'Номер сводного ИП', 'Номер сводного исполнительного производства');
                    }
                    $record['doc_text'] = new ResultDataField('string', 'DocText', \trim(\strip_tags($doc)), 'Реквизиты документа', 'Реквизиты исполнительного документа');
                    $record['doc_type'] = new ResultDataField('string', 'DocType', \trim(\strip_tags(\substr($doc, 0, \strpos($doc, ' от ')))), 'Вид документа', 'Вид исполнительного документа');
                    $doc = \trim(\substr($doc, \strpos($doc, ' от ') + 6));
                    if (\preg_match('/^([0-9\\.]+) /', $doc, $matches)) {
                        ...
                        $doc = \trim(\substr($doc, \strlen($matches[1]) + 1));
                    }
                    ...
                    $doc = \trim(\strtr($doc, ['Постановление о взыскании исполнительского сбора' => '']));
                    if ($doc) {
                    ...
                    */
                    ...
                    foreach ($subjects as $subject) {
                        ...
                        $name = ['Исполнительский сбор' => ['Bailiff', 'Сбор', 'Сумма сбора'], 'Задолженность по расходам' => ['Costs', 'Расходы', 'Сумма расходов'], 'Штраф СПИ' => ['Fine', 'Штраф', 'Сумма штрафа']];
                    ...
                    foreach ($bailiff as $i => $bailiff_phone) {
                        if (false !== \strpos($bailiff_phone, '<')) {
                            break;
                        }
                        if ($bailiff_phone && (0 == $i || $bailiff_phone != $bailiff[$i - 1])) {
            ...
            return true;
        } elseif (\strpos($content, 'Exception') || \strpos($content, '404 Not Found') || \strpos($content, '502 Bad Gateway') || false !== \strpos($content, 'Ошибка')) {
            if ($swapData['iteration'] >= 3) {
                $error = 'Внутренняя ошибка источника';
            }
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fsspapp/fssp_err_'.\time().'_'.$swapData['iteration'].'.txt', $content);
        } elseif (\strpos($content, '403 Forbidden')) {
            if (isset($swapData['session']) && $swapData['session']->id) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 1 hour),sessionstatusid=6,statuscode='forbidden' WHERE id=".$swapData['session']->id);
            }
            unset($swapData['session']);
        } elseif (!$content) {
            if (isset($swapData['session']) && $swapData['session']->id) {
