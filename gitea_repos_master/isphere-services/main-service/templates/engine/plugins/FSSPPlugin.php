...
class FSSPPlugin implements PluginInterface
{
    ...
    {
        $name = ['' => 'fssp', 'fssp_person' => 'fssp', 'fssp_suspect' => 'fssp_suspect', 'fssp_org' => 'fssp', 'fssp_ip' => 'fssp'];
    ...
    {
        $title = ['' => 'ФССП - поиск исполнительных производств', 'fssp_person' => 'ФССП - исполнительные производства (сайт)', 'fssp_suspect' => 'ФССП - розыск подозреваемых в совершении преступлений', 'fssp_org' => 'ФССП - исполнительные производства по организации', 'fssp_ip' => 'ФССП - информация об исполнительном производстве'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $sessionData = null;
        if (0 == \rand(0, 9)) {
            $mysqli->executeStatement("DELETE FROM session WHERE sourceid=3 AND cookies='' AND sessionstatusid IN (2,6) ORDER BY lasttime limit 1");
        }
        //        $mysqli->query("DELETE FROM session WHERE sessionstatusid=2 AND sourceid=3 AND cookies='' ORDER BY lasttime limit 1");
        $mysqli->executeStatement('UPDATE session s SET request_id='.$reqId." WHERE sourceid=3 AND request_id IS NULL AND sessionstatusid=2 AND (statuscode<>'used' OR lasttime<from_unixtime(unix_timestamp(now())-600)) AND unix_timestamp(now())-unix_timestamp(lasttime)>30 ORDER BY lasttime limit 1");
        //        $mysqli->query("UPDATE session s SET endtime=now(),sessionstatusid=5,statuscode='expired' WHERE sourceid=3 AND request_id IS NULL AND sessionstatusid IN (1,2) AND unix_timestamp(now())-unix_timestamp(starttime)>=15");
        //        $mysqli->query("UPDATE session s SET request_id=".$reqId." WHERE sourceid=3 AND request_id IS NULL AND sessionstatusid=2 AND unix_timestamp(now())-unix_timestamp(starttime)<15 ORDER BY starttime limit 1");
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sourceid=3 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->executeStatement("UPDATE session SET statuscode='used',lasttime=now(),used=ifnull(used,0)+1,captcha='',request_id=NULL WHERE id=".$sessionData->id);
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='used',lasttime=now(),used=ifnull(used,0)+1,request_id=NULL WHERE id=".$sessionData->id);
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='exhausted' WHERE used>=4 AND id=".$sessionData->id);
                if (!$row['proxyid']) {
                    if (\rand() % 3) {
                        $proxygroup = 1;
                        $multiple = false;
                    ...
                    $result = $mysqli->executeQuery("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth, (SELECT count(*) FROM session s WHERE sourceid=3 AND proxyid=proxy.id) scnt FROM proxy WHERE enabled=1 AND status=1 AND proxygroup={$proxygroup} AND id NOT IN (SELECT proxyid FROM session s WHERE sourceid=3 AND proxyid IS NOT NULL AND ((sessionstatusid=6 AND lasttime>from_unixtime(unix_timestamp(now())-{$lastlocked}))))".($multiple ? '' : ' HAVING scnt=0').' ORDER BY scnt, lasttime limit 1');
                    //                    $result = $mysqli->query("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM proxy WHERE enabled=1 AND status=1 AND country='ru' AND (rotation>0 OR id NOT IN (SELECT proxyid FROM session WHERE sourceid=3 AND proxyid IS NOT NULL AND sessionstatusid IN (1,2,6))) ORDER BY lasttime limit 1");
                    //                    $result = $mysqli->query("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM proxy WHERE enabled=1 AND status=1 AND country='ru' ORDER BY lasttime limit 1");
                    if ($result) {
                        $row = $result->fetchAssociative();
                        if ($row) {
                            $sessionData->proxyid = $row['proxyid'];
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $hosts = ['suspect' => 'is.fssp.gov.ru', 'person' => 'is-node1.fssp.gov.ru', 'org' => 'is-node2.fssp.gov.ru', 'ip' => 'is-node3.fssp.gov.ru'];
        $url = 'https://'.$hosts[$checktype];
        //        $url = 'https://is-node1.fssp.gov.ru';
        if (false && !isset($swapData['captcha_image'])) {
            $params = ['callback' => $swapData['callback'], '_' => $swapData['_']++];
            $url .= '/refresh_visual_captcha/?'.\http_build_query($params);
            $header = ['Accept: */*', 'Accept-Language: ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3', 'Accept-Encoding: gzip, deflate, br', 'DNT: 1', 'Connection: keep-alive', 'Referer: https://fssp.gov.ru/', 'Cookie: '.$swapData['session']->cookies, 'Sec-Fetch-Dest: script', 'Sec-Fetch-Mode: no-cors', 'Sec-Fetch-Site: same-site'];
            if ($this->tlsproxy) {
                $params = [
                    'Url' => $url,
                    'Method' => 'GET',
        ...
        //            echo "\n";
            ...
            if ('suspect' == $checktype) {
                $params = ['is[extended]' => 1, 'system' => 'suspect_info', 'is[all]' => '', 'is[suspect_fio]' => $initData['last_name'].' '.$initData['first_name'].(isset($initData['patronymic']) ? ' '.$initData['patronymic'] : ''), 'is[suspect_birth_date]' => \date('d.m.Y', \strtotime($initData['date'])), 'is[suspect_birth_place]' => '', 'is[init_agency_name]' => '', 'is[init_agency_contact]' => '', 'is[search_agency_name]' => '', 'is[code_clause]' => '', 'is[or]' => 0];
                if (isset($initData['region_id'])) {
                    $params['is[region_id][]'] = $initData['region_id'];
                }
            } else {
                $params = ['system' => 'ip', 'is[extended]' => 1, 'nocache' => 1, 'is[variant]' => 0, 'is[region_id][0]' => isset($initData['region_id']) ? $initData['region_id'] : '-1', 'is[last_name]' => '', 'is[first_name]' => '', 'is[drtr_name]' => '', 'is[ip_number]' => '', 'is[patronymic]' => '', 'is[date]' => '', 'is[address]' => '', 'is[id_number]' => '', 'is[id_type][0]' => '', 'is[id_issuer]' => '', 'is[inn]' => ''];
            ...
            /*
                for( $j=1; $j <= 4; $j++){
                    ...
                    if( $j > 3 ){
                              \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/cookies/fssp_error.txt', '4', FILE_APPEND);
                              return true;
                    }
                    else{
                    ...
                    $rContext->setSwapData($swapData);
            */
            $header = ['Accept: */*', 'Accept-Language: ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3', 'DNT: 1', 'Connection: keep-alive', 'Referer: https://fssp.gov.ru/', 'Sec-Fetch-Dest: script', 'Sec-Fetch-Mode: no-cors', 'Sec-Fetch-Site: same-site'];
            if ($this->tlsproxy) {
                $params = [
                    'Url' => $url,
                    'Method' => 'GET',
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        /*
                if (!isset($swapData['captcha_image']) && strlen($content)>1000) {
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/'.$initData['checktype'].'_captcha_image_'.$swapData['iteration'].'_'.time().'.txt',$content);
                    ...
                    if ($res && isset($res['image'])) {
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/captcha_'.time().'.jpg', $captcha);
                        $swapData['iteration']--;
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/'.$initData['checktype'].'_captcha_image_err_'.$swapData['iteration'].'_'.time().'.txt',$content);
                        $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='invalidimage' WHERE id=" . $swapData['session']->id);
        ...
        */
            ...
            if (!isset($swapData['captcha_id'])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service']]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/'.$initData['checktype'].'_captcha_err_'.$swapData['iteration'].'_'.\time().'.txt', $content."\r\n".$this->captcha_service[$swapData['captcha_service']]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
        ...
        if ($this->tlsproxy) {
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/raw_'.time().'.txt', $raw_content."\n\n".$swapData['iteration']);
            if (!isset($res['success']) || !$res['success'] || !isset($res['payload']['status']) || 200 != $res['payload']['status']) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/raw_err_'.\time().'.txt', $raw_content."\n\n".$swapData['iteration']);
            }
            if (isset($res['payload']['headers']['Set-Cookie'])) {
                ...
                $rContext->setSwapData($swapData);
                $mysqli->executeStatement("UPDATE session SET cookies='{$new_cookies}' WHERE id=".$swapData['session']->id);
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/fssp_'.time().'.cookies',$new_cookies);
            }
            $full_content = isset($res['payload']['text']) ? $res['payload']['text'] : '';
            // curl_multi_getcontent($rContext->getCurlHandler());
        }
        ...
        //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/'.$initData['checktype'].'_'.time().'.htm', $content."\n\n".$swapData['iteration']);
        } elseif ($full_content) {
            $content = $full_content;
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/'.$initData['checktype'].'_err_'.\time().'.htm', $content."\n\n".$swapData['iteration']);
        } else {
            $content = $full_content;
            if (isset($swapData['session'])) {
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='empty' WHERE id=" . $swapData['session']->id);
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 60 minute),sessionstatusid=6,statuscode='empty' WHERE id=".$swapData['session']->id);
                $mysqli->executeStatement('UPDATE session SET proxyid=NULL WHERE sourceid=3 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' AND id<>'.$swapData['session']->id.' ORDER BY lasttime LIMIT 3');
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        }
        if (\preg_match('/connection refused/', $content)) {
            if (isset($swapData['session'])) {
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='refused' WHERE id=" . $swapData['session']->id);
                //                $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 6 hour),sessionstatusid=6,statuscode='refused' WHERE id=" . $swapData['session']->id);
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='refused' WHERE id=".$swapData['session']->id);
                $mysqli->executeStatement('UPDATE session SET proxyid=NULL WHERE sourceid=3 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' AND id<>'.$swapData['session']->id.' ORDER BY lasttime LIMIT 3');
                //                $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval ".($swapData['session']->proxyid<100?"1 minute":"3 hour")."),sessionstatusid=6,statuscode='refused' WHERE sourceid=3 AND proxyid=" . $swapData['session']->proxyid . " ORDER BY lasttime LIMIT 10");
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        }
        if (\preg_match('/Timeout exceeded/', $content)) {
            if (isset($swapData['session'])) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='timeout' WHERE id=".$swapData['session']->id);
                $mysqli->executeStatement('UPDATE session SET proxyid=NULL WHERE sourceid=3 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' AND id<>'.$swapData['session']->id.' ORDER BY lasttime LIMIT 3');
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        ...
        if (\preg_match('/превысили лимит/', $content)) {
            if (isset($swapData['session'])) {
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='limit' WHERE id=" . $swapData['session']->id);
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='limit' WHERE id=".$swapData['session']->id);
                $mysqli->executeStatement('UPDATE session SET proxyid=NULL WHERE sourceid=3 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' AND id<>'.$swapData['session']->id.' ORDER BY lasttime LIMIT 3');
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        ...
        if (\preg_match('/502 Bad Gateway/', $content)) {
            if (isset($swapData['session'])) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='bad' WHERE id=".$swapData['session']->id);
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        }
        if (\preg_match('/403 Forbidden/', $content)) {
            if (isset($swapData['session'])) {
                //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
                //                $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 6 hour),sessionstatusid=6,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 10 minute),sessionstatusid=6,statuscode='forbidden' WHERE id=".$swapData['session']->id);
                $mysqli->executeStatement('UPDATE session SET proxyid=NULL WHERE sourceid=3 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' AND id<>'.$swapData['session']->id.' ORDER BY lasttime LIMIT 3');
                //                $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval ".($swapData['session']->proxyid<100?"1 minute":"3 hour")."),sessionstatusid=6,statuscode='forbidden' WHERE sourceid=3 AND proxyid=" . $swapData['session']->proxyid . " ORDER BY lasttime LIMIT 10");
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        }
        if (\preg_match('/503 Service Temporarily Unavailable/', $content)) {
            if (isset($swapData['session'])) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='unavailable' WHERE id=".$swapData['session']->id);
            }
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
        }
        if (\preg_match('/уже обрабатывается/', $content)) {
            ...
            /*
                        if (isset($swapData['session']))
            //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
                            $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='processing' WHERE id=" . $swapData['session']->id);
            */
            unset($swapData['session']);
            $rContext->setSwapData($swapData);
            $rContext->setSleep(5);
        ...
        if (\preg_match('/<div id="captcha-popup"/', $content)) {
            /*
                        if (isset($swapData['session'])) {
            //                $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='exhausted' WHERE used>1 AND id=" . $swapData['session']->id);
            ...
            */
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_service'])) {
                //                $mysqli->query("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),'".date('Y-m-d H:i:s',$swapData['captcha_time'])."',3,4,'invalidcaptcha','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
            }
            if (isset($swapData['captcha_image']) && isset($swapData['captcha_value'])) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/captcha/bad/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
            }
            ...
            if (false !== $start) {
                ...
                if (false !== $finish) {
                    ...
                    unset($swapData['captcha_id']);
                    $captcha = \base64_decode($captcha);
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/captcha_'.\time().'.jpg', $captcha);
                    //                    if (rand(0,1)) {
                    $value = neuro_post($captcha, 'fsspsitedecode');
                    ...
                    //                    }
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement("UPDATE session SET statuscode='captcha' WHERE id=".$swapData['session']->id);
                    }
                }
            }
            $rContext->setSwapData($swapData);
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_service'])) {
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),'".\date('Y-m-d H:i:s', $swapData['captcha_time'])."',3,3,'success','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
            }
            if (isset($swapData['captcha_image']) && isset($swapData['captcha_value'])) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fssp/captcha/good/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
            }
            $mysqli->executeStatement("UPDATE session SET statuscode='success',success=ifnull(success,0)+1 WHERE id=".$swapData['session']->id);
            ...
            foreach ($records[1] as $record) {
                if ('fssp_suspect' == $initData['checktype'] && \preg_match_all('/<td[^>]*>(.*?)<\\/td>/msu', $record, $cols)) {
                    ...
                    if (isset($close[1])) {
                        ...
                        //                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('logs/fssp/reasons.csv',"\"".trim(html_entity_decode($close[1]))."\"\n",FILE_APPEND);
                    ...
                    if ($subject[0]) {
                        ...
                        //                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('logs/fssp/subjects.csv',"\"".trim(html_entity_decode($subject[0]))."\"\n",FILE_APPEND);
                    }
                    if (\count($subject) > 1) {
                    ...
                    foreach ($subjects as $subject) {
                        ...
                        $name = ['Общая сумма задолженности' => ['', false, 'Сумма задолженности'], 'Исполнительский сбор' => ['Bailiff', 'Сбор исполнителя', 'Сумма сбора исполнителя'], 'Задолженность по расходам' => ['Costs', 'Расходы исполнителя', 'Сумма расходов исполнителя'], 'Штраф СПИ' => ['Fine', 'Штраф', 'Сумма штрафа']];
