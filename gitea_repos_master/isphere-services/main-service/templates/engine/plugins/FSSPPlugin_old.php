...
class FSSPPlugin_old implements PluginInterface
{
        ...
        if (\preg_match('/<div id="captcha-popup"/', $content)) {
            ...
            //                    file_put_contents('./logs/fssp/captcha_'.time().'.jpg', $captcha);
                                ...
                                if ($value && substr($value,0,5)<>'ERROR') {
                                    $swapData['session']->code = $value;
                                    $swapData['iteration']--;
                                    $rContext->setSwapData($swapData);
                                    $rContext->setSleep(2);
                                    return true;
                                } else {
                                    $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=3,statuscode='exhausted' WHERE id=" . $swapData['session']->id);
                                    unset($swapData['session']);
                                }
                            }
                        }
