...
class GIBDDPlugin_new implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sessionstatusid=2 AND sourceid=21 ORDER BY lasttime limit 1");
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->query("UPDATE isphere.session SET sessionstatusid=3,endtime=now(),lasttime=now(),used=ifnull(used,0)+1,statuscode='used' WHERE id=".$sessionData->id);
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if (\in_array($checktype, ['history', 'aiusdtp', 'wanted', 'restricted', 'diagnostic']) && !isset($initData['vin']) && !isset($initData['bodynum'])) {
        ...
        */
        ...
        if (!isset($swapData['captcha_token'])) {
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                $swapData['captcha_service'.$swapData['num']] = (int) (($swapData['iteration'] - 1) / 4) % \count($this->captcha_service);
                $rContext->setSwapData($swapData);
...
*/
    ...
    public function computeRequest(array $params, &$rContext)
    {
            ...
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \file_put_contents('./logs/gibdd/'.$initData['checktype'].'_captcha_err_'.$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']);
                    } else {
                        \file_put_contents('./logs/gibdd/'.$initData['checktype'].'_captcha_err_'.$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']);
                    }
                } else {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    ...
                    if (!$content) {
                    } elseif (isset($res['status']) && 'ready' !== $res['status'] && \time() - $swapData['captcha_time'.$swapData['num']] < 30) {
                    } else {
        ...
        \file_put_contents('./logs/gibdd/'.$initData['checktype'].'_'.(isset($swapData['pic']) && \count($swapData['pics']) ? 'pic'.$swapData['pics'][0]['i'].'_' : '').$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content);
        ...
        if (\is_array($res) && \array_key_exists('RequestResult', $res) && \is_array($res['RequestResult'])) {
            ...
            if (\array_key_exists('vehicle', $res)) {
                ...
                if (\array_key_exists('vin', $rec)) {
                    $rec['vin'] = \strtr($rec['vin'], [' ' => '', 'I' => '1', 'O' => '0', 'Q' => '0', 'А' => 'A', 'В' => 'B', 'С' => 'C', 'Е' => 'E', 'Н' => 'H', 'К' => 'K', 'М' => 'M', 'О' => '0', 'Р' => 'P', 'Т' => 'T', 'У' => 'Y', 'Х' => 'X', 'а' => 'a', 'с' => 'c', 'е' => 'e', 'к' => 'k', 'м' => 'm', 'о' => '0', 'р' => 'p', 'т' => 't', 'у' => 'y', 'х' => 'x']);
                    $data['VIN'] = new ResultDataField('string', 'VIN', $rec['vin'], 'VIN', 'VIN');
                }
                if (\array_key_exists('chassisNumber', $rec)) {
                    $rec['chassisNumber'] = \strtr($rec['chassisNumber'], [' ' => '', 'А' => 'A', 'В' => 'B', 'С' => 'C', 'Е' => 'E', 'Н' => 'H', 'К' => 'K', 'М' => 'M', 'О' => '0', 'Р' => 'P', 'Т' => 'T', 'У' => 'Y', 'Х' => 'X', 'а' => 'a', 'с' => 'c', 'е' => 'e', 'к' => 'k', 'м' => 'm', 'о' => '0', 'р' => 'p', 'т' => 't', 'у' => 'y', 'х' => 'x']);
                    $data['Chassis'] = new ResultDataField('string', 'Chassis', $rec['chassisNumber'], 'Шасси', 'Шасси');
                }
                if (\array_key_exists('bodyNumber', $rec)) {
                    $rec['bodyNumber'] = \strtr($rec['bodyNumber'], [' ' => '', 'А' => 'A', 'В' => 'B', 'С' => 'C', 'Е' => 'E', 'Н' => 'H', 'К' => 'K', 'М' => 'M', 'О' => '0', 'Р' => 'P', 'Т' => 'T', 'У' => 'Y', 'Х' => 'X', 'а' => 'a', 'с' => 'c', 'е' => 'e', 'к' => 'k', 'м' => 'm', 'о' => '0', 'р' => 'p', 'т' => 't', 'у' => 'y', 'х' => 'x']);
            ...
            if (\array_key_exists('ownershipPeriods', $res)) {
                ...
                $data['Owners'] = new ResultDataField('string', 'Owners', \count($rec), 'Кол-во владельцев', 'Кол-во владельцев');
...
*/
                    ...
                    if (isset($owner['lastOperation'])) {
                        $ownerdata['lastoperation'] = new ResultDataField('string', 'LastOperation', $operationtypes[$owner['lastOperation']], 'Последнее действие', 'Последнее действие');
                    }
            ...
            if (\array_key_exists('Accidents', $res) && \is_array($res['Accidents'])) {
                foreach ($res['Accidents'] as $rec) {
                    ...
                    if (\array_key_exists('DamagePoints', $rec) && \is_array($rec['DamagePoints']) && \count($rec['DamagePoints'])) {
                        $pointname = [
            ...
            if (isset($res['doc']) && \is_array($res['doc'])) {
                ...
                $data['Result'] = new ResultDataField('string', 'Result', \count($doc) ? 'В/у найдено, дата выдачи соответствует номеру' : 'В/у не найдено или неверно указана дата выдачи', 'Результат', 'Результат');
                $data['ResultCode'] = new ResultDataField('string', 'ResultCode', \count($doc) ? 'FOUND' : 'NOT_FOUND', 'Код результата', 'Код результата');
                //                if (isset($doc['num']))
                ...
                if (isset($doc['st_kart'])) {
                    $data['DocStatus'] = new ResultDataField('string', 'DocStatus', ('T' != $doc['st_kart'] && 'Т' != $doc['st_kart']) ? 'Недействителен' : 'Действует', 'Статус документа', 'Статус документа');
                }
            ...
            //                return false;
                ...
                $data['Result'] = new ResultDataField('string', 'Result', isset($res['code']) && (200 == $res['code']) ? 'Проверка выполнена успешно, серия и номер свидетельства о регистрации ТС соответствует госномеру' : (isset($res['message']) ? $res['message'] : 'Ошибка при выполнении запроса'), 'Результат проверки', 'Результат проверки');
                ...
                if ('FOUND' == $res_code && \array_key_exists('data', $res) && \is_array($res['data'])) {
                    $data['fines'] = new ResultDataField('string', 'Fines', \count($res['data']), 'Неоплаченных штрафов', 'Неоплаченных штрафов');
                }
            ...
            --$swapData['iteration'];
            $rContext->setSwapData($swapData);
        } elseif (\is_array($res) && \array_key_exists('status', $res) && (200 == $res['status'] || 404 == $res['status'])) {
            $resultData = new ResultDataList();
            $rContext->setResultData($resultData);
            $rContext->setFinished();
            ...
            */
            ...
            //                $swapData['iteration'] = 100;
                ...
                \file_put_contents('./logs/gibdd/'.$initData['checktype'].'_err_'.$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $fullcontent);
