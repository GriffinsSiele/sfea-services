...
class GISGMPPlugin implements PluginInterface
{
    ...
    public function getSessionData(array $params)
    {
        ...
        $result = $mysqli->executeQuery(
            "SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sessionstatusid=2 AND (SELECT status FROM proxy WHERE id=s.proxyid)=1 AND sourceid=34 AND unix_timestamp(now())-unix_timestamp(lasttime)>1 ORDER BY lasttime limit 1"
        );
        if ($result) {
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if ('fines' == $checktype && !isset($initData['driver_number']) && !isset($initData['ctc'])) {
            $rContext->setFinished();
            $rContext->setError(
                'Указаны не все обязательные параметры (водительское удостоверение или св-во о регистрации ТС)'
            );
        ...
        if ('fssp' == $checktype && (!isset($initData['last_name']) || !isset($initData['first_name']) || !isset($initData['date']))) {
            $rContext->setFinished();
            $rContext->setError('Указаны не все обязательные параметры (ФИО+ДР или паспорт или ИНН)');
        ...
        if (isset($initData['last_name']) && isset($initData['first_name']) && \preg_match(
            '/[^А-Яа-яЁё\\s\\-\\.]/ui',
            $initData['last_name'].' '.$initData['first_name'].(isset($initData['patronymic']) ? ' '.$initData['patronymic'] : '')
        )) {
            $rContext->setFinished();
        ...
        */
        ...
        $articles = ['taxes' => 'ufns', 'fines' => 'gibddfine', 'fssp' => 'ufssp', 'ip' => 'ufssp'];
        ...
        if (!isset($swapData['uid'])) {
            ...
            if ('fssp' == $checktype && isset($initData['last_name']) && isset($initData['first_name']) && isset($initData['date'])) {
                $data = 'FIO='.\urlencode(
                    ...
                    'd.m.Y',
                    \strtotime($initData['date'])
                ).'&region='.(isset($initData['region_id']) ? $initData['region_id'] : '');
            ...
            $params = ['data' => $data, 'article' => $articles[$checktype], '_stoken' => $swapData['session']->token];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        $res = false;
        if (!isset($swapData['uid'])) {
            //            if ($content) \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/gisgmp/oplatagosuslug_start_'.time().'.html',$content);
            ...
            if (isset($res['uid'])) {
                ...
                $mysqli->executeStatement(
                    "UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='expired' WHERE id=".$swapData['session']->id
                );
                unset($swapData['session']);
            } elseif ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                    './logs/gisgmp/oplatagosuslug_start_err_'.\time().'.html',
                    $content
                );
                $error = 'Ошибка при отправке запроса';
            } else {
                $mysqli->executeStatement(
                    "UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='empty' WHERE id=".$swapData['session']->id
                );
                unset($swapData['session']);
            }
        } elseif (!isset($swapData['url'])) {
            //            if ($content) \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/gisgmp/oplatagosuslug_get_'.time().'.html',$content);
            $res = \json_decode($content, true);
            if (isset($res['status']) && $res['status']) {
                if ('success' == $res['status']) {
                    ...
                    if ($swapData['iteration'] > 3) {
                        ...
                        $mysqli->executeStatement(
                            "UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='fail' WHERE id=".$swapData['session']->id
                        );
                        unset($swapData['session']);
                        unset($swapData['uid']);
                    } else {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                        './logs/gisgmp/oplatagosuslug_get_err_'.\time().'.html',
                        $content
                    );
            ...
            //            if (!empty(trim($content))) \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/gisgmp/oplatagosuslug_'.time().'.html',$content);
            if (\preg_match_all('/<div class="panel-heading"[^>]+>.*?<\\/article>/sim', $content, $matches)) {
                foreach ($matches[0] as $key => $part) {
                    ...
                    if (\preg_match_all(
                        '/<input type="hidden" name="([^"]+)" value="([^"]+)"/sim',
                        $part,
                        $mmmatches
                    ) || \preg_match_all(
                    ...
                    if (\preg_match_all(
                        '/<span class="dt">(.*?)<\\/span><span class="dd"><pre>([^<]+)/sim',
                        $part,
                        $mmmatches
                    )) {
                        foreach ($mmmatches[1] as $k => $val) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                    './logs/gisgmp/oplatagosuslug_err_'.\time().'.html',
                    $content
                );
                ...
                if (!isset($res['error']) || !$res['error']) {
                    foreach ($res as $debt) {
                        ...
                        if (isset($debt['debttype'])) {
                            $data['DebtType'] = new ResultDataField(
                                'string',
                                'DebtType',
                                $debt['debttype'],
                                'Тип задолженности',
                        ...
                        if (isset($debt['purpose']) && $debt['purpose']) {
                            $data['Subject'] = new ResultDataField(
                                'string',
                                'Subject',
                                $debt['purpose'],
                        ...
                        if (isset($debt['amount'])) {
                            ...
                            $data['Total'] = new ResultDataField(
                                'float',
                                'Total',
                                $debt['origin_charge_amount'],
                                'Сумма задолженности',
                                'Сумма задолженности'
                            );
                        }
                        if (isset($debt['docno'])) {
                            $data['DocNumber'] = new ResultDataField(
                                'string',
                                'DocNumber',
                                $debt['docno'],
                                'Номер документа',
                        ...
                        if (isset($debt['docdate'])) {
                            $data['DocDate'] = new ResultDataField(
                                'string',
                                'DocDate',
                                $debt['docdate'],
                                'Дата документа',
                        ...
                        if (isset($debt['primalDocument']) && $debt['primalDocument']) {
                            ...
                            //                            } else {
                            $data['PrimalDoc'] = new ResultDataField(
                                'string',
                                'PrimalDoc',
                                $debt['primalDocument'],
                                'Первичный документ',
                        ...
                        if (isset($debt['payerName']) && $debt['payerName']) {
                            ...
                            $data['Name'] = new ResultDataField(
                                'string', 'Name', $debt['ФИО плательщика'], 'ФИО', 'ФИО'
                            );
                        } elseif (isset($debt['purpose']) && \preg_match(
                            '/в отношении ([А-Яа-яЁё\\s\\.\\-\\*]+)$/',
                            $debt['purpose'],
                            $matches
                        ) && $matches[1]) {
                        ...
                        if (isset($debt['Документ'])) {
                            ...
                            if (\preg_match('/ИНН ([\\d\\*]+)/', $debt['Документ'], $matches)) {
                                ...
                                '/паспорт РФ\\, серия ([\\d\\*]+) номер ([\\d\\*]+)/',
                                $debt['Документ'],
                                $matches
                            )) {
                                $data['Passport'] = new ResultDataField(
                                    'string',
                                    'Passport',
                                    $matches[1].$matches[2],
                                    'Паспорт',
                                ...
                                '/паспорт РФ\\, серия ([\\d\\*А-Я]+) номер ([\\d\\*А-Я]+)/',
                                $debt['Документ'],
                                $matches
                            ) && false !== \strpos(\mb_strtoupper($debt['purpose']), 'ГИБДД')) {
                                $data['DriverLicense'] = new ResultDataField(
                                    'string',
                                    'DriverLicense',
                                    $matches[1].$matches[2],
                                ...
                                '/Водительское удостоверение ([\\s\\d\\*А-Я]+)/',
                                $debt['Документ'],
                                $matches
                            )) {
                                $data['DriverLicense'] = new ResultDataField(
                                ...
                                $data['Document'] = new ResultDataField(
                                    'string',
                                    'Document',
                                    $debt['Документ'],
                                    'Документ',
                        ...
                        if (isset($debt['supplierbillid']) && $debt['supplierbillid']) {
                            $data['BillId'] = new ResultDataField(
                                'string',
                                'BillId',
                                $debt['supplierbillid'],
                        ...
                        if (isset($debt['payeeName']) && $debt['payeeName']) {
                            $data['PayeeName'] = new ResultDataField(
                                'string',
                                'PayeeName',
                                $debt['payeeName'],
                                'Получатель',
                        ...
                        if (isset($debt['payeeInn']) && $debt['payeeInn']) {
                            $data['PayeeINN'] = new ResultDataField(
                                'string',
                                'PayeeINN',
                                $debt['payeeInn'],
                                'ИНН получателя',
                        ...
                        if (isset($debt['Основание платежа']) && $debt['Основание платежа']) {
                            $data['PaymentBasis'] = new ResultDataField(
                                'string',
                                'PaymentBasis',
                                $debt['Основание платежа'],
                                'Основание платежа',
                                'Основание платежа'
                            );
                        }
