...
class GetContactAppPlugin_new implements PluginInterface
{
...
//        array('host' => 'api.capmonster.cloud', 'key' => 'afb26cbb248d650ea8b8d88822984242'),
    ...
    public function getSessionData($usecaptcha = true)
    {
        ...
        $forcecaptcha = false;
        if ($usecaptcha) {
            $result = $mysqli->query("SELECT COUNT(*) count FROM isphere.session WHERE request_id IS NULL AND sessionstatusid=2 AND (statuscode<>'used' OR lasttime<from_unixtime(unix_timestamp(now())-600)) AND lasttime<from_unixtime(unix_timestamp(now())-10) AND sourceid=55 AND captcha>'' AND statuscode<>'success'");
            if ($result) {
        ...
        $uselocked = false;
        if (!$usecaptcha) {
            $result = $mysqli->query('SELECT COUNT(*) count FROM isphere.session WHERE sessionstatusid=2 AND sourceid=55 AND lasttime<from_unixtime(unix_timestamp(now())-600)');
            if ($result) {
                $row = $result->fetch_object();
        ...
        try {
            $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId." WHERE request_id IS NULL AND ((sessionstatusid=2 AND (statuscode<>'used' OR unix_timestamp(now())-unix_timestamp(lasttime)>600)".($forcecaptcha ? " AND captcha>'' AND statuscode<>'success'" : " AND captcha=''".($usecaptcha ? '' : " AND statuscode='success'")).')'.($uselocked ? " OR (sessionstatusid=6 AND statuscode='tagslimitexceed')" : '').") AND unix_timestamp(now())-unix_timestamp(lasttime)>10 AND sourceid=55 AND token>'' ORDER BY lasttime limit 1");
        } catch (Exception $e) {
            return $sessionData;
        }
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,enckey,device,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=55 AND request_id=".$reqId);
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->query("UPDATE isphere.session SET lasttime=now(),used=ifnull(used,0)+1,used_ext=ifnull(used_ext,0)+1,sessionstatusid=2,statuscode='used',request_id=NULL WHERE id=".$sessionData->id);
                ...
                if (!$row->proxyid) {
                    ...
                    $result = $mysqli->query("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth, (SELECT count(*) FROM session s WHERE sourceid=55 AND proxyid=proxy.id) scnt FROM isphere.proxy WHERE enabled=1 AND status=1 AND proxygroup=$proxygroup AND id NOT IN (SELECT proxyid FROM session s WHERE sourceid=55 AND proxyid IS NOT NULL AND ((sessionstatusid=6 AND lasttime>from_unixtime(unix_timestamp(now())-$lastlocked)))) ORDER BY scnt, lasttime limit 1"/* "lasttime>from_unixtime(unix_timestamp(now())-$lastused) OR ". */);
                    if ($result) {
                        $row = $result->fetch_object();
                        if ($row) {
                            $sessionData->proxyid = $row->proxyid;
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            $swapData['session'] = $this->getSessionData($swapData['iteration'] % 2);
            if (!$swapData['session']) {
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (\strpos($error, 'timed out') || \strpos($error, 'connection')) {
            $error = false;
            //                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 30 minute),sessionstatusid=6 WHERE id=" . $swapData['session']->id);
        }
            ...
            if (!isset($swapData['captcha_id'])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service']]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \file_put_contents('./logs/getcontact/'.$initData['checktype'].'_captcha_err_'.$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service']]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
        ...
        if (!$error && isset($swapData['captcha_value'])) {
            ...
            \file_put_contents('./logs/getcontact/getcontact_verify_'.$swapData['iteration'].'_'.\time().'.txt', $captcha_value."\n\n".$content);
            unset($swapData['captcha_value']);
            if (isset($res['meta']['httpStatusCode']) && 200 == $res['meta']['httpStatusCode']) {
                ...
                unset($swapData['captcha_image']);
                if ($swapData['session']->code) {
                    $mysqli->query("UPDATE isphere.session SET statuscode='success',successtime=now() WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                ...
                \file_put_contents('./logs/getcontact/getcontact_captcha_verify_'.$swapData['iteration'].'_'.\time().'.jpg', \base64_decode($res['result']['image']));
                if ($swapData['session']->code) {
                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=4,statuscode='invalidcaptcha',captchaimage='".$res['result']['image']."' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                ...
                if (\strpos($res['meta']['errorMessage'], 'failed')) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='failed' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                    \file_put_contents('./logs/getcontact/getcontact_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                } elseif (\strpos($res['meta']['errorMessage'], 'not authorized')) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='notauthorized' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                    \file_put_contents('./logs/getcontact/getcontact_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                ...
                $error = 'Внутренняя ошибка источника';
                \file_put_contents('./logs/getcontact/getcontact_err_'.$initData['phone'].'_'.$swapData['iteration'].'_verify_'.\time().'.html', $content);
                unset($swapData['session']);
                ...
                \file_put_contents('./logs/getcontact/getcontact_err_'.$initData['phone'].'_'.$swapData['iteration'].'_verify_'.\time().'.html', $content);
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 30 minute),sessionstatusid=6,statuscode='error',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                unset($swapData['session']);
            } else {
                \file_put_contents('./logs/getcontact/getcontact_empty_'.$initData['phone'].'_'.$swapData['iteration'].'_verify_'.\time().'.html', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $full_content."\n".$swapData['session']->id);
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 30 minute),sessionstatusid=6,statuscode='empty',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                $mysqli->query('UPDATE isphere.session SET proxyid=NULL WHERE sourceid=55 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' ORDER BY lasttime LIMIT '.($swapData['session']->proxyid < 100 ? '3' : '10'));
                //                $mysqli->query("UPDATE isphere.session SET used_ext=0,success_ext=0,proxy_id=NULL WHERE success_ext=0 AND used_ext>=3 AND sessionstatusid=6 AND id=" . $swapData['session']->id);
                //                $mysqli->query("UPDATE isphere.session SET used_ext=1,success_ext=0 WHERE success_ext>0 AND sessionstatusid=6 AND id=" . $swapData['session']->id);
            ...
            if (isset($swapData['captcha_service'])) {
                \file_put_contents('./logs/getcontact/getcontact_'.$this->captcha_service[$swapData['captcha_service']]['host'].'_'.$report.'_'.$swapData['iteration'].'_'.\time().'.txt', $captcha_value);
            }
            if ($report && isset($swapData['captcha_id']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                ...
                */
                $mysqli->query('INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),55,'.('bad' == $report ? 4 : 3).",'".('bad' == $report ? 'invalidcaptcha' : 'success')."','".$captcha_value."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                //                echo "Captcha ID ".$swapData['captcha_id']." reported as ".$report." with result $res\n";
            }
            unset($swapData['captcha_id']);
        } elseif (!$error && isset($swapData['tags'])) {
            \file_put_contents('./logs/getcontact/getcontact_tags_'.$swapData['iteration'].'_'.\time().'.txt', $content);
            ...
            //                $mysqli->query("UPDATE isphere.session SET success_ext=ifnull(success_ext,0)+1 WHERE id=" . $swapData['session']->id);
                ...
                if (false !== \strpos($res['meta']['errorMessage'], 'No result found')) {
                    ...
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='failed' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                    \file_put_contents('./logs/getcontact/getcontact_tags_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                } elseif (\strpos($res['meta']['errorMessage'], 'not authorized')) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='notauthorized' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                    \file_put_contents('./logs/getcontact/getcontact_tags_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                    ...
                    //                    else
                    //                        $swapData['captcha_value'] = 'fedcba';
                    //                    file_put_contents('./logs/getcontact/getcontact_captcha_tags_'.$swapData['iteration'].'_'.time().'.jpg',base64_decode($res['result']['image']));
                    //                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 day),sessionstatusid=6,statuscode='validation' WHERE id=" . $swapData['session']->id);
                    //                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=7,captchaimage='".$res['result']['image']."' WHERE sessionstatusid=2 AND id=" . $swapData['session']->id);
                    //                    unset($swapData['session']);
                    //                    $retry = true;
                } else {
                ...
                //                $error = 'Ошибка при выполнении запроса';
                \file_put_contents('./logs/getcontact/getcontact_tags_err_'.$swapData['iteration'].'_'.\time().'.txt', $content);
            }
            if ((isset($res['subscriptionInfo']['usage']['numberDetail']['remainingCount']) && !$res['subscriptionInfo']['usage']['numberDetail']['remainingCount']) ||
                (isset($res['result']['subscriptionInfo']['usage']['numberDetail']['remainingCount']) && !$res['result']['subscriptionInfo']['usage']['numberDetail']['remainingCount'])) {
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 day),sessionstatusid=6,statuscode='tagslimitexceed',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
            }
            if (!$retry) {
            ...
            \file_put_contents('./logs/getcontact/getcontact_search_'.$swapData['iteration'].'_'.\time().'.txt', $content);
            if (isset($res['meta']['errorMessage']) && \strlen(\trim($res['meta']['errorMessage']))) {
                ...
                if (false !== \strpos($res['meta']['errorMessage'], 'Number is invalid') || \strpos($res['meta']['errorMessage'], 'this country')) {
                    $resultData = new ResultDataList();
                    $mysqli->query("UPDATE isphere.session SET used=0,success=0,success_ext=ifnull(success_ext,0)+1,statuscode='success',successtime=now() WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    $rContext->setResultData($resultData);
                    $rContext->setFinished();
                ...
                //                } elseif (strpos($res['meta']['errorMessage'],"be visible") || strpos($res['meta']['errorMessage'],"быть невидимым")) {
                    ...
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(str_to_date('".\date('Y-m-01 03:00:00', \time() - 3 * 60 * 60)."', '%Y-%m-%d %H:%i:%s'),interval 1 month),sessionstatusid=6,statuscode='limitexceed',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                    \file_put_contents('./logs/getcontact/getcontact_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                } elseif (\strpos($res['meta']['errorMessage'], 'failed')) {
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 year),sessionstatusid=6,statuscode='failed' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    ...
                    //                    else
                    //                        $swapData['captcha_value'] = 'fedcba';
                    \file_put_contents('./logs/getcontact/getcontact_captcha_search_'.$swapData['iteration'].'_'.\time().'.jpg', \base64_decode($res['result']['image']));
                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=7,captchaimage='".$res['result']['image']."' WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    $mysqli->query("UPDATE isphere.session SET captcha_id=NULL WHERE sessionstatusid=7 AND statuscode='used' AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                } elseif (\strpos($res['meta']['errorMessage'], '500000')) {
                    $error = 'Внутренняя ошибка источника';
                    \file_put_contents('./logs/getcontact/getcontact_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                    ...
                    //                    $error = trim($res['meta']['errorMessage']);
                    \file_put_contents('./logs/getcontact/getcontact_msg_'.$swapData['iteration'].'_'.\time().'.txt', $content);
                    $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 day),sessionstatusid=6,statuscode='unknown',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                    unset($swapData['session']);
                }
                ...
                $error = 'Внутренняя ошибка источника';
                \file_put_contents('./logs/getcontact/getcontact_err_'.$initData['phone'].'_'.$swapData['iteration'].'_'.\time().'.html', $content);
                unset($swapData['session']);
            } elseif (\strlen($content)) {
                if ($swapData['iteration'] >= 3) {
                    $error = 'Некорректный ответ источника';
                ...
                \file_put_contents('./logs/getcontact/getcontact_empty_'.$initData['phone'].'_'.$swapData['iteration'].'_'.\time().'.html', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $full_content."\n".$swapData['session']->id);
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 30 minute),sessionstatusid=6,statuscode='empty',captcha='',captcha_id=NULL WHERE sessionstatusid=2 AND id=".$swapData['session']->id);
                $mysqli->query('UPDATE isphere.session SET proxyid=NULL WHERE sourceid=55 AND sessionstatusid=2 AND proxyid='.$swapData['session']->proxyid.' ORDER BY lasttime LIMIT 10');
                //                $mysqli->query("UPDATE isphere.session SET used_ext=0,success_ext=0,proxy_id=NULL WHERE success_ext=0 AND used_ext>=3 AND sessionstatusid=6 AND id=" . $swapData['session']->id);
                //                $mysqli->query("UPDATE isphere.session SET used_ext=1,success_ext=0 WHERE success_ext>0 AND sessionstatusid=6 AND id=" . $swapData['session']->id);
                (!isset($swapData['empty'])) ? $swapData['empty'] = 1 : $swapData['empty']++;
                unset($swapData['session']);
