...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
                ...
                if ($db->llen('getcontact_queue') > 20) {
                    $db->close();
                    $rContext->setFinished();
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/getcontact/getcontact_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/getcontact/getcontact_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/getcontact/getcontact_err_'.\time().'.txt', $content);
