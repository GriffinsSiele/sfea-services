...
class GooglePlusPlugin implements PluginInterface
{
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        // //////////////////////////////////////////////////////////////////////////////////////////////////
        $swapData['session'] = $this->getSessionData($params, isset($swapData['url']) ? 43 : 17);
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$content) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/googleplus/contrib_'.time().'.html',$content);
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/googleplus/googleplus_'.time().'.txt',$content);
            $res = \json_decode($content, true);
            if ($res && isset($res['resourceName'])) {
                $data = [];
                if (isset($res['metadata']['sources']) && \count($res['metadata']['sources']) > 1) {
                    if (isset($res['phoneNumbers'])) {
                        foreach ($res['phoneNumbers'] as $i => $rec) {
                            $data['phone'.($i ? $i + 1 : '')] = new ResultDataField('phone', 'Phone'.($i ? $i + 1 : ''), \strtr($rec['value'], ['+' => '']), 'Телефон '.($i ? $i + 1 : ''), 'Телефон '.($i ? $i + 1 : ''));
                        }
                    }
                    if (isset($res['emailAddresses'])) {
                        foreach ($res['emailAddresses'] as $i => $rec) {
                            $data['email'.($i ? $i + 1 : '')] = new ResultDataField('email', 'Email'.($i ? $i + 1 : ''), $rec['value'], 'Email '.($i ? $i + 1 : ''), 'Email '.($i ? $i + 1 : ''));
                        }
                    }
                    ...
                    if (isset($res['photos'])) {
                        foreach ($res['photos'] as $i => $rec) {
                            if (isset($rec['metadata']['source']['type']) && ('PROFILE' == $rec['metadata']['source']['type'] || 'DOMAIN_PROFILE' == $rec['metadata']['source']['type']) && !\strpos($rec['url'], '/AAAAAAAAAA')) {
                                $data['photo'.($i ? $i + 1 : '')] = new ResultDataField('image', 'Photo'.($i ? $i + 1 : ''), \strtr($rec['url'], ['/s100/' => '/', '=s100' => '']), 'Фото '.($i ? $i + 1 : ''), 'Фото '.($i ? $i + 1 : ''));
                            }
                        }
                    }
                ...
                if (\strpos($res['error']['message'], 'access token')) {
                    $mysqli->executeStatement('UPDATE session SET endtime=now(),sessionstatusid=5 WHERE id='.$swapData['session']->id);
