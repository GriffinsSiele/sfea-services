...
class GosuslugiPlugin_new implements PluginInterface
{
...
//        array('host' => 'rucaptcha.com', 'key' => 'd167c71a9278312f184f17caa4e71050'),
    ...
    public function getSessionData($nocaptcha)
    {
        ...
        if (0 == \rand(0, 9)) {
            $mysqli->query("DELETE FROM isphere.session WHERE sessionstatusid=2 AND sourceid=48 AND cookies='' ORDER BY lasttime limit 1");
        }

        try {
            $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId." WHERE sessionstatusid=2 AND sourceid=48 AND cookies>'' AND (statuscode<>'used' OR lasttime<from_unixtime(unix_timestamp(now())-600)) AND lasttime<from_unixtime(unix_timestamp(now())-10) AND captcha>'' ORDER BY lasttime limit 1");
        } catch (Exception $e) {
            return $sessionData;
        }
        ...
        if ($result && 0 == $result->num_rows && $nocaptcha) {
            $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId." WHERE sessionstatusid IN (2,7) AND sourceid=48 AND cookies>'' AND (statuscode<>'used' OR unix_timestamp(now())-unix_timestamp(lasttime)>600) AND lasttime<from_unixtime(unix_timestamp(now())-10) ORDER BY lasttime limit 1");
            $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,'' captcha,'' captcha_service,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=48 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
            $nocaptcha = true;
        } else {
            $nocaptcha = false;
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->query('UPDATE isphere.session SET '.($nocaptcha ? "captcha='',captcha_id=NULL," : '')."lasttime=now(),used=ifnull(used,0)+1,sessionstatusid=2,statuscode='used',request_id=NULL WHERE id=".$sessionData->id);
                $mysqli->query("UPDATE isphere.session SET lasttime=now(),used=ifnull(used,0)+1,sessionstatusid=2,statuscode='used',request_id=NULL WHERE statuscode<>'used' AND id=".$sessionData->id);
        ...
        */
        ...
        //            echo "\n";
            ...
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                $swapData['captcha_service'.$swapData['num']] = (int) (($swapData['iteration'] - 1) / 3) % \count($this->captcha_service);
...
//                    'captchaResponse' => $swapData['captcha_token'], //.($swapData['iteration']<3?'0':''),
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (empty(\trim($content))) {
            ...
            //            $mysqli->query("UPDATE isphere.session SET proxyid=NULL,used=NULL,success=NULL WHERE proxyid<100 AND statuscode='used' AND id=".$swapData['session']->id);
            //            $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='empty',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
            ...
            /*
                        ...
                        $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='techwork',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                        unset($swapData['session']);
            */
        } else {
            $cookies = str_cookies($swapData['session']->cookies);
        ...
        if (isset($swapData['captcha_session']) && !isset($swapData['captcha_image'])) {
            if (/* strlen($content)>4500 && strlen($content)<25000 && */ 'PNG' == \substr($content, 1, 3) && 'END' == \substr($content, \strlen($content) - 7, 3)) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_captcha_'.\time().'.jpg', $content);
                ...
                if (!empty(\trim($content))) {
                    \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_bad_captcha_'.\time().'.jpg', $content);
                }
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='badimage',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                unset($swapData['session']);
                $rContext->setSleep(1);
            }
            $rContext->setSwapData($swapData);
        ...
        if (!isset($swapData['captcha_session'])) {
            if (!empty(\trim($content))) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_session_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
            if (\is_array($res) && isset($res['captchaType']) && 'esiacaptcha' == $res['captchaType'] && isset($res['captchaSession'])) {
                $swapData['captcha_session'] = $res['captchaSession'];
            } elseif (\strpos($content, 'технические работы')) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_session_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
                ...
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='error',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                unset($swapData['session']);
                ...
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='unavailable',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                unset($swapData['session']);
            } elseif (!empty(\trim($content))) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_session_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='badsession',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                unset($swapData['session']);
            }
        } elseif (!isset($swapData['verify_token'])) {
            if (!empty(\trim($content))) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_verify_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
            if (\is_array($res) && isset($res['verify_token'])) {
                ...
                $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 5 minute),sessionstatusid=6,statuscode='invalidanswer',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                unset($swapData['session']);
            }
        } elseif (!isset($swapData['request_id'])) {
            if (!empty(\trim($content))) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_find_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
            if (isset($res['message']) && \strpos($res['message'], 'verify.token.is.invalid')) {
                if (!$swapData['session']->nocaptcha) {
                    ...
                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=7,lasttime=now(),successtime=now(),success=ifnull(success,0)+1,statuscode='success',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                    if (isset($swapData['captcha_service']) && isset($swapData['captcha_id'])) {
                        //                        $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),48,4,'invalidcaptcha','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                    }
                    \file_put_contents('./logs/gosuslugi/captcha/bad/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
                }
                unset($swapData['verify_token']);
                unset($swapData['captcha_session']);
                unset($swapData['captcha_image']);
                unset($swapData['captcha_value']);
            //                unset($swapData['session']);
            } elseif (\is_array($res) && ('phone' == $checktype || 'email' == $checktype || (isset($res['message']) && (\strpos($res['message'], 'not.found') || \strpos($res['message'], 'не найден'))))) {
                ...
                //                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=7,lasttime=now(),successtime=now(),success=ifnull(success,0)+1,statuscode='success',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                    ...
                    //                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=7,lasttime=now(),successtime=now(),success=ifnull(success,0)+1,statuscode='success',captchaimage='' WHERE statuscode='used' AND id=".$swapData['session']->id);
                    if (isset($swapData['captcha_service']) && isset($swapData['captcha_id'])) {
                        //                        $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),48,3,'success','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                    }
                    \file_put_contents('./logs/gosuslugi/captcha/good/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
                }
            ...
            if (!empty(\trim($content))) {
                \file_put_contents('./logs/gosuslugi/'.$swapData['mode'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
            if (\is_array($res)) {
                $resultData = new ResultDataList();
                $data = [];
