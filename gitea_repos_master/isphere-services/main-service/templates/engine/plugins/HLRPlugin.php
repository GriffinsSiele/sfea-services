...
class HLRPlugin implements PluginInterface
{
...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
                ...
                $db->rpush('hlr_queue', $initData['phone']);
                $swapData['db'] = $db;
                $swapData['sent'] = true;
                //                $rContext->setSleep(1);
                ...
                if ($swapData['iteration'] >= 10 && !isset($swapData['sent'])) {
                    $rContext->setFinished();
                    $rContext->setError('Сервис временно недоступен');
                }
                $rContext->setSwapData($swapData);
        ...
        try {
            ...
            if ($db->hexists('hlr', $initData['phone'])) {
                $content = $db->hget('hlr', $initData['phone']);
                $db->close();
                unset($swapData['db']);
            } else {
                if ($swapData['iteration'] > 300) {
                    $db->hdel('hlr_queue', $initData['phone']);
                    $db->close();
                    $rContext->setFinished();
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/hlr/hlr_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/hlr/hlr_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/hlr/hlr_err_'.\time().'.txt', $content);
