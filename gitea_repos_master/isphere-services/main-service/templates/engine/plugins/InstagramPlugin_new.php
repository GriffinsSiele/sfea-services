...
class InstagramPlugin_new implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query('UPDATE isphere.session s SET request_id='.$reqId.' WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=19 AND unix_timestamp(now())-unix_timestamp(lasttime)>10 ORDER BY lasttime limit 1');
...
//                'Accept-Encoding: deflate',
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$curl_error) {
            ...
            if (!isset($swapData['path'])) {
                \file_put_contents('./logs/instagram/instagram_login_'.\time().'.txt', $content);
                ...
                if (isset($res['user'])) {
                    ...
                    if (\strpos($error, 'there was a problem') || \strpos($error, 'an error occured')) {
                        $error = false;
                        //                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=4,statuscode='error' WHERE id=" . $swapData['session']->id);
                    ...
                    \file_put_contents('./logs/instagram/instagram_login_err_'.\time().'.txt', $content);
                    ...
                    $mysqli->query('UPDATE isphere.proxy SET successtime=now(),success=ifnull(success,0)+1 WHERE id='.$swapData['session']->proxyid);
                    unset($swapData['session']);
                    \file_put_contents('./logs/instagram/instagram_login_err_spam_'.\time().'.txt', $content);
                    ...
                    $mysqli->query('UPDATE isphere.proxy SET successtime=now(),success=ifnull(success,0)+1 WHERE id='.$swapData['session']->proxyid);
                    unset($swapData['session']);
                    \file_put_contents('./logs/instagram/instagram_login_err_ip_'.\time().'.txt', $content);
                    ...
                    $mysqli->query('UPDATE isphere.proxy SET successtime=now(),success=ifnull(success,0)+1 WHERE id='.$swapData['session']->proxyid);
                    unset($swapData['session']);
                    \file_put_contents('./logs/instagram/instagram_login_err_wait_'.\time().'.txt', $content);
                } elseif (isset($res['message'])) {
                    $error = $res['message'];
                    $mysqli->query('UPDATE isphere.proxy SET successtime=now(),success=ifnull(success,0)+1 WHERE id='.$swapData['session']->proxyid);
                    \file_put_contents('./logs/instagram/instagram_login_err_'.\time().'.txt', $content);
                } elseif (\strpos($content, 'something went wrong')) {
                    $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=4,statuscode='wrong' WHERE id=".$swapData['session']->id);
                    $mysqli->query('UPDATE isphere.proxy SET successtime=now(),success=ifnull(success,0)+1 WHERE id='.$swapData['session']->proxyid);
                    unset($swapData['session']);
                    \file_put_contents('./logs/instagram/instagram_login_err_'.\time().'.txt', $content);
                } else {
                    if (!$content || \strpos($content, 'page could not be loaded')) {
                        //                        $mysqli->query("UPDATE isphere.session SET endtime=now(),sessionstatusid=4,statuscode='notloaded' WHERE id=" . $swapData['session']->id);
                        $mysqli->query("UPDATE isphere.session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='notloaded' WHERE id=".$swapData['session']->id);
                        unset($swapData['session']);
                    } else {
                ...
                \file_put_contents('./logs/instagram/instagram_'.\time().'.html', $content);
                ...
                if (\preg_match("/window\._sharedData = (.*?);<\/script>/", $content, $matches)) {
                    ...
                    \file_put_contents('./logs/instagram/instagram_err_'.\time().'.html', $content);
                }
            }
        }
        $rContext->setSwapData($swapData);
