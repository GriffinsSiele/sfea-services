...
class ListOrgPlugin_new implements PluginInterface
{
    private $names = [
                           'телефон' => ['phone', 'Телефон', 'Телефон'],
                           'инн/кпп' => ['inn', 'ИНН', 'ИНН'],
                           'кпп' => ['kpp', 'КПП', 'КПП'],
                           'руководитель' => ['head', 'Руководитель', 'Руководитель'],
                           'юр.адрес' => ['address', 'Адрес', 'Адрес'],
    ];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            if (\preg_match('/вы не робот/', $content, $matches)) {
                ...
                \file_put_contents('./logs/listorg/search_'.\time().'.html', $content);
                ...
                if (\preg_match("/<div class=\'org_list\'>(.*?)<\/div>/sim", $content, $matches)) {
                        ...
                        if (\preg_match_all("/<i>([^<]+)<\/i>: (.*?)<br>/sim", $dataPart.'<br>', $matches)) {
                            foreach ($matches[1] as $key => $val) {
                                ...
                                if (isset($this->names[$title])) {
                                    ...
                                    \file_put_contents('./logs/fields/listorg_'.\time().'_'.\strtr($title, ['/' => '_']), $title."\n".$text);
                                }
                            }
                        }
                    ...
                    \file_put_contents('./logs/listorg/search_err_'.\time().'.html', $content);
