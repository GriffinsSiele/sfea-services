...
class NamesPlugin_new implements PluginInterface
{
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            \file_put_contents('./logs/names/names_'.\time().'.txt', $content);
            ...
            if ($res && isset($res['data'])) {
                ...
                foreach ($res['data'] as $row) {
                    ...
                    if (isset($row['firstName'])) {
                        $data['firstname'] = new ResultDataField('string', 'FirstName', \iconv('windows-1251', 'utf-8', \iconv('utf-8', 'windows-1251//IGNORE', \strtr($row['firstName'], [\chr(0) => ' ']))), 'Имя', 'Имя');
                        $name = \iconv('windows-1251', 'utf-8', \iconv('utf-8', 'windows-1251//IGNORE', \strtr($row['firstName'], [\chr(0) => ' '])));
                    }
                    if (isset($row['lastName'])) {
                        $data['lastname'] = new ResultDataField('string', 'LastName', \iconv('windows-1251', 'utf-8', \iconv('utf-8', 'windows-1251//IGNORE', \strtr($row['lastName'], [\chr(0) => ' ']))), 'Фамилия', 'Фамилия');
                        $name = \trim($name.' '.\iconv('windows-1251', 'utf-8', \iconv('utf-8', 'windows-1251//IGNORE', \strtr($row['lastName'], [\chr(0) => ' ']))));
                    }
                    if ($name) {
