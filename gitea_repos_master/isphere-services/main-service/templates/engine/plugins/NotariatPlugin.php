...
{
    private $names = ['Fio' => ['ФИО'], 'BirthDate' => ['Дата рождения'], 'DeathDate' => ['Дата смерти'], 'DeathActDate' => ['Дата свидетельства о смерти'], 'DeathActNumber' => ['Номер свидетельства о смерти'], 'Address' => ['Адрес'], 'CaseNumber' => ['Номер наследственного дела'], 'CaseDate' => ['Дата наследственного дела'], 'CaseCloseDate' => ['Дата завершения наследственного дела'], 'NotaryName' => ['Нотариус'], 'ChamberName' => ['Нотариальная палата'], 'DistrictName' => ['Местонахождение'], 'ContactAddress' => ['Адрес нотариуса'], 'ContactPhone' => ['Телефон нотариуса']];
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        $url = $page.'eis-proxy';
        $params = ['name' => $initData['last_name'].' '.$initData['first_name'].(isset($initData['patronymic']) ? ' '.$initData['patronymic'] : ''), 'birth_date' => \date('Ymd', \strtotime($initData['date'])), 'death_date' => 'NULL'];
        $header = ['Accept: application/json, text/javascript, */*; q=0.01', 'Content-Type: application/json', 'Origin: '.$host, 'Referer: '.$page, 'X-CSRFToken: '.$swapData['session']->token, 'X-Requested-With: XMLHttpRequest'];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if ($content) {
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/notariat/search_'.time().'.txt',$content);
            ...
            if (\is_array($res) && isset($res['records']) && \is_array($res['records'])) {
                ...
                if (\strpos($content, 'временно') || \strpos($content, 'ведутся работы')) {
                    ...
                    if ($content) {
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/notariat/err_'.\time().'.txt', $content);
                    }
