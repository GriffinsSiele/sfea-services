...
class OKAppRPlugin_new implements PluginInterface
{
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['db'])) {
            ...
            $connection_params = [
                'host' => '172.16.1.25'.(3 + $swapData['iteration'] % 2),
                'port' => 5672,
                'vhost' => '/',
                'login' => 'default_user_FbMUOGaBu35fZ6YXHA3',
                'password' => 'Fh0tqYCJH5Wl15pMl0uOoI3k1jUnGFTf',
            ];
        ...
        \file_put_contents('./logs/ok/okapp_'.\time().'.txt', $content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                foreach ($res['records'] as $row) {
                    if (\is_array($row)) {
                        ...
                        foreach ($row as $field) {
                            if (\is_array($field)) {
                                //                        if ($field['field']=='Name')
                                $data[$field['field']] = new ResultDataField('bool' == $field['type'] ? 'integer' : $field['type']/* .($field['type']=='url'?':recursive':'') */, $field['field'], $field['value'], $field['title'], $field['description']);
                            }
                        }
                        if (\count($data)) {
                ...
                \file_put_contents('./logs/ok/okapp_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \file_put_contents('./logs/ok/okapp_err_'.\time().'.txt', $content);
