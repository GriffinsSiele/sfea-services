...
class OKBotPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск учетной записи в OK', 'ok_phonecheck' => 'OK - проверка телефона на наличие пользователя', 'ok_emailcheck' => 'OK - проверка email на наличие пользователя', 'ok_urlcheck' => 'OK - проверка наличия профиля'];
...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/okbot_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/okbot_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/okbot_err_'.\time().'.txt', $content);
