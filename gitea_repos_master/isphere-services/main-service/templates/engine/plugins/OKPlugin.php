...
class OKPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в ОК', 'ok_person' => 'ОК - поиск профилей по имени и дате рождения', 'ok_phone' => 'ОК - поиск по номеру телефона', 'ok_email' => 'ОК - поиск по email', 'ok_url' => 'ОК - профиль пользователя'];

        return isset($title[$checktype]) ? $title[$checktype] : $title[''];
        //        return 'Поиск в OK';
    }

    public function getSessionData(array $params, $sourceid = 6)
    {
        /** @var \Doctrine\DBAL\Connection $mysqli */
        $mysqli = $params['_connection'];
        $reqId = $params['_reqId'];
        $sessionData = null;
        $mysqli->executeStatement('UPDATE session s SET request_id='.$reqId." WHERE request_id IS NULL AND sessionstatusid=2 AND (s.proxyid IS NULL OR (SELECT status FROM proxy WHERE id=s.proxyid)=1) AND sourceid={$sourceid} AND unix_timestamp(now())-unix_timestamp(lasttime)>2 ORDER BY lasttime limit 1");
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if ('url' == $checktype && !isset($swapData['found'])) {
            $swapData['found'] = true;
            $swapData['data'] = [];
        }
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            $swapData['session'] = $this->getSessionData($params, 'person' == $checktype || 'url' == $checktype && isset($swapData['found']) ? 6 : 14);
            if (!$swapData['session']) {
                if (isset($swapData['iteration']) && $swapData['iteration'] >= 30) {
                    if ('url' == $checktype && !isset($swapData['data'])) {
                        $swapData['found'] = true;
                        $swapData['data'] = [];
                        $rContext->setSwapData($swapData);
        ...
        } elseif ('url' == $checktype && !isset($swapData['found']) && !isset($swapData['locked']) && !isset($swapData['captcha'])) {
        ...
        //            print "POST: ".http_build_query($params)."\n";
            ...
            $url .= '?'.\http_build_query($params);
        } elseif ('url' == $checktype && isset($swapData['data'])) {
            $url = $initData['url'].(isset($swapData['about']) ? '/about' : '');
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/'.$initData['checktype'].'_'.time().'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
        if (!$content) {
            ...
            if ($curlError) {
                ...
                if (\strpos($curlError, 'timed out') || \strpos($curlError, 'refused') || \strpos($curlError, 'reset by peer')) {
                    if (isset($swapData['session']) && $swapData['session']->id) {
                        $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='connectionerror' WHERE id=".$swapData['session']->id);
            ...
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/'.$initData['checktype'].'_err_'.\time().'.html', $curlError."\n".\curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            if (isset($swapData['session']) && $swapData['session']->id) {
                $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 15 minute),sessionstatusid=6,statuscode='empty' WHERE id=".$swapData['session']->id);
                unset($swapData['session']);
                $rContext->setSwapData($swapData);
            }
        ...
        if ($content) {
            if (('person' == $checktype || 'text' == $checktype || 'url' == $checktype) && \preg_match('/<a href="\\/dk\\?st\\.cmd=anonymMain"/', $content)) {
                if (isset($swapData['session'])) {
                    $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='notlogged' WHERE id=".$swapData['session']->id);
                    unset($swapData['session']);
                }
            } elseif ('person' == $checktype || 'text' == $checktype) {
                $resultData = new ResultDataList();
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_search_'.time().'.html',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
                $res = false;
                if (\preg_match('/ results="({[^"]+)"/', $content, $matches)) {
                    //                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_search_'.time().'.txt',html_entity_decode($matches[1]));
                ...
                if (0 == $found) {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_search_err_'.\time().'.html', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
                    $error = 'Не удалось выполнить поиск';
                    if (isset($swapData['session'])) {
                        //                        $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='searcherror' WHERE id=" . $swapData['session']->id);
                        unset($swapData['session']);
                    }
                }
            } elseif (!isset($swapData['hash'])) {
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_start_'.time().'.html',$content);
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_recovery_'.time().'.html',$content);
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_header_'.time().'.html',$content);
                if (\strpos($content, 'ChooseCodeDestination')) {
                    ...
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement('UPDATE session SET unlocktime=date_add(now(),interval 6 hour),sessionstatusid=6 WHERE id='.$swapData['session']->id);
                ...
                //                    $error = 'Не удалось выполнить поиск';
                    ...
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='recoveryerror' WHERE id=".$swapData['session']->id);
                        unset($swapData['session']);
                    }
                //                    $error = 'Не удалось выполнить поиск';
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_err_'.\time().'.html', $content);
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_locked_'.time().'.html',$content);
                //                $swapData['captcha']++;
                $resultData = new ResultDataList();
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/okcaptcha_'.time().'.jpg',$content);
                $error = 'Ошибка распознавания капчи';
            } elseif ('url' == $checktype && isset($swapData['data']) && !isset($swapData['about'])) {
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_profile_'.time().'.html',$content);
                $data = $swapData['data'];
                if (\preg_match('/"AuthLoginPopup"/', $content)) {
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='loggedout' WHERE sourceid=6 AND id=".$swapData['session']->id);
                        unset($swapData['session']);
                    }
                } else {
                                        ...
                                        if(preg_match_all("/<a href=\"[^\"]+\" class=\"mctc_navMenuSec\">([^<]+)<span class=\"navMenuCount\">([^<]+)<\/span>/", $content, $matches)){
                                            foreach( $matches[1] as $key => $val ){
                                                ...
                                                if(isset($this->names[$title])){
                                                    ...
                                                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fields/ok'.time().'_'.$title , $title."\n".$text);
                                                }
                                            }
                                        }
                    */
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_about_'.time().'.html',$content);
                $data = $swapData['data'];
                ...
                if (\preg_match_all('/<a class="mctc_navMenuSec[^>]+>([^<]+)<span class="navMenuCount">([^<]+)<\\/span><\\/a>/', $content, $matches)) {
                    foreach ($matches[1] as $key => $val) {
                        ...
                        if (isset($this->names[$title])) {
                            ...
                            ++$counter;
                            $data['other'.$counter['other']] = new ResultDataField('string', 'other'.$counter['other'], $text, $title, $title);
                            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fields/ok'.\time().'_'.$title, $title."\n".$text);
                        }
                    }
                }
                if (\preg_match_all('/<span class="user-profile_i_t_inner">([^<]+)<\\/span><\\/div><\\/div><div class="user-profile_i_val[^>]+>(.*?)<\\/div>/', $content, $matches)) {
                    foreach ($matches[1] as $key => $val) {
                        $title = \trim(\str_replace(':', '', $val));
                        ...
                        if (isset($this->names[$title])) {
                            ...
                            $counter[$field[0]] = isset($counter[$field[0]]) ? $counter[$field[0]] + 1 : 0;
                            if ('birthday' == $field[0]) {
                                $repl = [' ' => '.', 'января' => '01', 'февраля' => '02', 'марта' => '03', 'апреля' => '04', 'мая' => '05', 'июня' => '06', 'июля' => '07', 'августа' => '08', 'сентября' => '09', 'октября' => '10', 'ноября' => '11', 'декабря' => '12'];
                                if (\strpos($text, '(')) {
                ...
                if (\preg_match_all('/<a class="o user-profile_i_relation-t ellip" href="([^\\?]+)[^>]+><span>([^<]+)<\\/span><div [^>]+><\\/div><\\/a><div class="ellip lstp-t">([^<]+)<\\/div>/', $content, $matches)) {
                    foreach ($matches[1] as $key => $val) {
                        ...
                        if (isset($this->names[$title])) {
                            ...
                            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fields/ok'.\time().'_rel_'.$title, $title."\n".$text);
                        }
                    }
                }
                $resultData = new ResultDataList();
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_login_'.time().'.html',$content);
                $resultData = new ResultDataList();
                if (\preg_match('/восстановления/', $content) || \preg_match('/можно восстановить/', $content) || \preg_match('/можно было бы восстановить/', $content)) {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_login_err_'.\time().'.html', $content);
                    $error = 'Некорректный ответ сервиса';
                    if (isset($swapData['session'])) {
                        $mysqli->executeStatement("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='loginerror' WHERE id=".$swapData['session']->id);
                        unset($swapData['session']);
                    }
                }
            } else {
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/ok/ok_result_'.time().'.html',$content);
                if (!\preg_match('/anonymPasswordRecoveryNew/', $content)) {
                    if ('url' == $checktype && $swapData['iteration'] > 10) {
                        $swapData['data'] = [];
                        unset($swapData['session']);
                    } else {
                        if (isset($swapData['session'])) {
