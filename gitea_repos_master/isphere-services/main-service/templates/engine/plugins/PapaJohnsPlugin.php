...
class PapaJohnsPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в Papa Johns', 'papajohns_phone' => 'Papa Johns - проверка телефона на наличие пользователя'];
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/papajohns/papajohns_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/papajohns/papajohns_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/papajohns/papajohns_err_'.\time().'.txt', $content);
