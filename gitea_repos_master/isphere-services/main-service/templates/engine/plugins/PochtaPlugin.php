...
class PochtaPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в Почте России', 'pochta_phone' => 'Почта России - проверка телефона на наличие пользователя'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $reqId = $params['_reqId'];
        $sessionData = null;
        $mysqli->executeStatement("UPDATE session s SET request_id={$reqId} WHERE sessionstatusid=2 AND sourceid=56 AND unix_timestamp(now())-unix_timestamp(lasttime)>2 ORDER BY lasttime limit 1");
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sourceid=56 AND request_id={$reqId} ORDER BY lasttime limit 1");
        if ($result) {
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $header = ['Origin: https://www.pochta.ru', 'Referer: https://www.pochta.ru/parcels'];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (\strpos($error, 'timed out') || \strpos($error, 'connection')) {
            $error = false;
            //                $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 10 minute),sessionstatusid=6,statuscode='connectionerror' WHERE id=" . $swapData['session']->id);
        }
        if (!$error) {
            $content = \curl_multi_getcontent($rContext->getCurlHandler());
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/pochta/pochta_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
        ...
        if (empty(\trim($content))) {
            ...
            $rContext->setSwapData($swapData);
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/pochta/pochta_'.time().'.cookies',$new_cookies);
            $mysqli->executeStatement("UPDATE session SET cookies='{$new_cookies}' WHERE id=".$swapData['session']->id);
        }
        if (\strpos($content, 'Ошибка 404') || isset($res) && \is_array($res) && 0 == \count($res)) {
            ...
            /*
                        ...
                        $mysqli->query("UPDATE session SET sessionstatusid=3,statuscode='forbidden',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
                    } elseif (strpos($content,'заблокирован')) {
                        $mysqli->query("UPDATE session SET sessionstatusid=3,statuscode='blocked',endtime=now() WHERE id=" . $swapData['session']->id);
            //            $rContext->setSleep(1);
            */
        } elseif (isset($res) && \is_array($res) && (isset($res['error']) || isset($res['humanReadableMessage']))) {
            $error = 'Внутренняя ошибка источника';
            if ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/pochta/pochta_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
        } elseif (\strpos($content, 'Делаем сайт')) {
            $error = 'Сервис временно недоступен';
        } else {
            $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 2 minute),sessionstatusid=6,statuscode='error' WHERE id=".$swapData['session']->id);
            //            $mysqli->query("UPDATE session SET sessionstatusid=3,statuscode='error',endtime=now() WHERE id=" . $swapData['session']->id);
            if ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/pochta/pochta_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
        }
