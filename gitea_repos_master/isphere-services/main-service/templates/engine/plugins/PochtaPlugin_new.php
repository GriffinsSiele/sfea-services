...
class PochtaPlugin_new implements PluginInterface
{
    ...
    public function getSessionData()
    {
        ...
        $mysqli->query("UPDATE isphere.session s SET request_id=$reqId WHERE sessionstatusid=2 AND sourceid=56 AND unix_timestamp(now())-unix_timestamp(lasttime)>2 ORDER BY lasttime limit 1");
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            \file_put_contents('./logs/pochta/pochta_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
        ...
        if (\strpos($content, 'Ошибка 404') || (isset($res) && \is_array($res) && 0 == \count($res))) {
            ...
            */
            ...
            if ($content) {
                \file_put_contents('./logs/pochta/pochta_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            }
