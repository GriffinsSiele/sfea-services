...
class RSAPlugin_new implements PluginInterface
{
...
//        array('host' => 'api.capmonster.cloud', 'key' => 'afb26cbb248d650ea8b8d88822984242'),
    ...
    public function getSessionData()
    {
        ...
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sessionstatusid=2 AND sourceid=44 ORDER BY lasttime limit 1");
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if (/* $userId==3481 && */ 'policy' == $checktype && isset($initData['vin']) && \in_array($initData['vin'], $testvin)) {
        ...
        if (!isset($swapData['mode'])) {
            $swapData['date'] = \date('d.m.Y', isset($initData['reqdate']) ? \strtotime($initData['reqdate']) : \time());
...
*/
        ...
        //            echo "\n";
            ...
            if ('policyInfo' == $swapData['mode']) {
                ...
                if (isset($swapData['ready'])) {
                    ...
                    $params = false;
                    $url = $site.'/dkbm-web-1.0/checkPolicyInfoStatus.htm?'.\http_build_query(['processId' => $swapData['id'], '_' => \time()]);
                }
                ...
                $params = [
                    ...
                    'driverDocNumber' => isset($initData['driver_number']) ? \trim(\mb_substr($initData['driver_number'], 4)) : '',
    ...
    public function computeRequest(array $params, &$rContext)
    {
            ...
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \file_put_contents('./logs/rsa/'.$initData['checktype'].'_captcha_err_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
        ...
        if (!empty(\trim($content))) {
            \file_put_contents('./logs/rsa/'.$swapData['mode'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
        }
        ...
        if (\is_array($res) && isset($res['validCaptcha']) && !$res['validCaptcha']) {
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                /*
                                $params = array(
                ...
                */
                $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),44,4,'invalidcaptcha','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
            ...
            unset($swapData['captcha_token']);
        } elseif ('policyInfo' == $swapData['mode'] && \is_array($res) && !isset($swapData['id']) && isset($res['processId'])) {
            $swapData['id'] = $res['processId'];
            --$swapData['iteration'];
        } elseif ('policyInfo' == $swapData['mode'] && \is_array($res) && isset($swapData['id']) && isset($res['RequestStatusInfo']['RequestStatusCode'])) {
            if (3 == $res['RequestStatusInfo']['RequestStatusCode']) {
                ...
                if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                    /*
                                        ...
                                        $res = file_get_contents($url);
                    */
                    $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),44,3,'success','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
            ...
            if (\preg_match("/<table class=\"bso-tbl[^>]+>(.*?)<\/table>/sim", $content, $matches)) {
                ...
                if (\count($parts) > 1) {
                    ...
                    foreach ($parts as $part) {
                        $data = [];
                        if (\preg_match_all("/<td[^>]*>(.*?)<\/td>/sim", \trim(\substr($part, 0, \strpos($part, '<div'))).\trim(\substr($part, \strpos($part, '</div>') + 6)), $matches)) {
                            $values = $matches[1];
                            ...
                            foreach ($values as $i => $value) {
                                if (isset($titles[$i])) {
                                    ...
                                    if (!$value || 'Сведения отсутствуют' == $value) {
                                        ...
                                        $data['Insurant'] = new ResultDataField('string', 'Insurant', \trim(\preg_replace("/[0-9]{2}\.[0-9]{2}\.[0-9]{4}/", '', $value)), 'Cтрахователь', 'Cтрахователь');
                                        ...
                                        $data['Owner'] = new ResultDataField('string', 'Owner', \trim(\preg_replace("/[0-9]{2}\.[0-9]{2}\.[0-9]{4}/", '', $value)), 'Cобственник', 'Cобственник');
                                        if (\preg_match("/([0-9]{2}\.[0-9]{2}\.[0-9]{4})/", $value, $matches)) {
                                        ...
                                        $data['Limited'] = new ResultDataField('string', 'Limited', \trim(\preg_replace("/\([^\)]*\)/", '', $value)), 'Ограничения', 'Ограничения');
                        ...
                        if (\preg_match_all("/<td class=\"table-td-header\">([^<]+)<\/td>[^<]*<td>([^<]+)<\/td>/sim", $part, $matches)) {
                            ...
                            foreach ($values2 as $i => $value) {
                                ...
                                if (!$value || 'Сведения отсутствуют' == $value) {
                                    ...
                                    $data['Model'] = new ResultDataField('string', 'Model', \trim(\preg_replace("/\([^\)]*\)/", '', $value)), 'Марка и модель', 'Марка и модель');
                                    ...
                                    if (!\strpos($value, 'тсутств')) {
                                        $data['VIN'] = new ResultDataField('string', 'VIN', \strtr($value, [' ' => '', 'I' => '1', 'O' => '0', 'Q' => '0', 'А' => 'A', 'В' => 'B', 'С' => 'C', 'Е' => 'E', 'Н' => 'H', 'К' => 'K', 'М' => 'M', 'О' => '0', 'Р' => 'P', 'Т' => 'T', 'У' => 'Y', 'Х' => 'X', 'а' => 'a', 'с' => 'c', 'е' => 'e', 'к' => 'k', 'м' => 'm', 'о' => '0', 'р' => 'p', 'т' => 't', 'у' => 'y', 'х' => 'x']), 'VIN', 'VIN');
                                    }
                                } elseif ('Номер кузова' == $title) {
                                    $data['BodyNum'] = new ResultDataField('string', 'BodyNum', \strtr($value, [' ' => '', 'А' => 'A', 'В' => 'B', 'С' => 'C', 'Е' => 'E', 'Н' => 'H', 'К' => 'K', 'М' => 'M', 'О' => 'o', 'Р' => 'P', 'Т' => 'T', 'У' => 'Y', 'Х' => 'X', 'а' => 'a', 'с' => 'c', 'е' => 'e', 'к' => 'k', 'м' => 'm', 'о' => 'o', 'р' => 'p', 'т' => 't', 'у' => 'y', 'х' => 'x']), 'Номер кузова', 'Номер кузова');
                                }
                                //                                else echo "$title: $value\r\n";
                            }
                        }
            ...
            if (\preg_match_all('/<img src="([^"]+)">/sim', $content, $matches)) {
                foreach ($matches[1] as $src) {
                    $img = \file_get_contents('https://dkbm-web.autoins.ru/dkbm-web-1.0/'.$src);
                    \file_put_contents('./logs/rsa/'.\strtr($src, ['.htm?id=' => '_']).'.jpg', $img);
                }
            }
            $rContext->setResultData($resultData);
            $rContext->setFinished();
            ...
            if (\array_key_exists('lossCRTTypeList', $res) && $res['lossCRTTypeList']) {
                $data['LossCount'] = new ResultDataField('string', 'LossCount', \count($res['lossCRTTypeList']), 'Страховых случаев', 'Страховых случаев');
        ...
        //            }
            ...
            */
            ...
            if ($content) {
                \file_put_contents('./logs/rsa/'.$swapData['mode'].'_err_'.\time().'.txt', $content);
            }
            if ($content) {
                if (!$swapData['session']->nocaptcha) {
                    $mysqli->query("UPDATE isphere.session SET sessionstatusid=3,statuscode='invalid' WHERE statuscode='used' AND id=".$swapData['session']->id);
                }
