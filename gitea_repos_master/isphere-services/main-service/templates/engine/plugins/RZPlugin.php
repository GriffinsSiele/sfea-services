...
class RZPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в реестре залогов федеральной нотариальной палаты', 'rz_person' => 'Реестр залогов ФНП - поиск заложенного имущества физлица', 'rz_org' => 'Реестр залогов ФНП - поиск заложенного имущества организации', 'rz_auto' => 'Реестр залогов ФНП - проверка автомобиля'];
    ...
    public function getSessionData(array $params, $nocaptcha)
    {
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->executeStatement('UPDATE session SET '.($nocaptcha ? '' : "sessionstatusid=3,statuscode='used',endtime=now(),").'lasttime=now(),used=ifnull(used,0)+1 WHERE id='.$sessionData->id);
            }
        }
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            ...
            $swapData['session'] = $this->getSessionData($params, $swapData['iteration'] > 30);
            $rContext->setSwapData($swapData);
            if (!$swapData['session']) {
        ...
        if (!isset($swapData['captcha_token'])) {
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    $params = ['key' => $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['key'], 'method' => 'userrecaptcha', 'googlekey' => $this->googlekey, 'pageurl' => $page, 'version' => 'v3', 'action' => 'search_notary', 'min_score' => $this->minscore];
                    /*
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        */
        $res = \json_decode($content, true);
        //        if (!empty(trim($content))) \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/rz_'./*(isset($swapData['key'])?'':'key_').*/time().'.txt',$content);
        /*
        ...
        if (\is_array($res) && isset($res['data'])) {
            ...
            if (\is_array($res['data'])) {
                ...
                foreach ($res['data'] as $item) {
                    ...
                    if (isset($item['properties'])) {
                        foreach ($item['properties'] as $i => $prop) {
                            if (isset($prop['vehicleProperty'])) {
                                if (isset($prop['vehicleProperty']['vin'])) {
                    ...
                    $change = ['CREATION' => ['name' => 'start', 'title' => 'Возникновение'], 'CHANGE' => ['name' => 'change', 'title' => 'Изменение'], 'EXCLUSION' => ['name' => 'end', 'title' => 'Исключение']];
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                ...
                */
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,3,'success','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
            ...
            if (\strpos($res['message'], 'token') || \strpos($res['message'], 'капча')) {
                if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                    /*
                                        ...
                                        $res = file_get_contents($url);
                    */
                    $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,4,'invalidcaptcha','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                    //                    echo "Captcha ID ".$swapData['captcha_id']." reported as bad with result $res\n";
                }
                if (!$swapData['session']->nocaptcha) {
                    $mysqli->executeStatement("UPDATE session SET sessionstatusid=4,statuscode='invalidcaptcha' WHERE statuscode='used' AND id=".$swapData['session']->id);
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/rz_err_'.\time().'.txt', $content);
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_denied_'./*(isset($swapData['key'])?'':'key_').*/time().'.txt',$content);
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                /*
                                    $params = array(
                                        'key' => $this->captcha_service[$swapData['captcha_service']]['key'],
            ...
            if (!$swapData['session']->nocaptcha) {
                //                    $mysqli->query("UPDATE session SET sessionstatusid=4,statuscode='forbidden' WHERE statuscode='used' AND id=" . $swapData['session']->id);
                //                    unset($swapData['session']);
            }
            unset($swapData['captcha_id']);
            unset($swapData['captcha_token']);
            ...
            if (!$swapData['session']->nocaptcha) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,statuscode='unavailable' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
            unset($swapData['session']);
            ...
            if (!$swapData['session']->nocaptcha) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,statuscode='error' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
            unset($swapData['session']);
        } else {
            if ($content) {
            ...
            if ($content) {
                if (!$swapData['session']->nocaptcha) {
                    $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,statuscode='invalidanswer' WHERE statuscode='used' AND id=".$swapData['session']->id);
                }
                $error = 'Некорректный ответ сервиса';
            } else {
                if (!$swapData['session']->nocaptcha) {
                    $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,statuscode='empty' WHERE statuscode='used' AND id=".$swapData['session']->id);
                }
            }
            unset($swapData['session']);
        }
