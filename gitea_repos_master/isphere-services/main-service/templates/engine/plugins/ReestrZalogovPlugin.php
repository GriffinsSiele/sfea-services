...
class ReestrZalogovPlugin implements PluginInterface
{
    ...
    private $names = ['VIN' => ['vin', 'VIN', 'VIN'], 'PIN' => ['pin', 'PIN', 'PIN'], 'Номер кузова' => ['body', 'Номер кузова', 'Номер кузова'], 'Номер шасси (рамы)' => ['chassis', 'Номер шасси (рамы)', 'Номер шасси (рамы)'], 'Описание транспортного средства' => ['description', 'Описание', 'Описание'], 'Описание иного имущества' => ['description', 'Описание', 'Описание'], 'ID' => ['id', 'Идентификатор', 'Идентификатор'], 'Фамилия' => ['lastname', 'Фамилия', 'Фамилия'], 'Имя' => ['firstname', 'Имя', 'Имя'], 'Отчество' => ['middlename', 'Отчество', 'Отчество'], 'Дата рождения' => ['birth', 'Дата рождения', 'Дата рождения'], 'Документ, удостоверяющий личность' => ['doc', 'Удостоверение личности', 'Удостоверение личности'], 'Адрес фактического места жительства в Российской Федерации' => ['region', 'Регион', 'Регион'], 'Полное наименование' => ['orgname', 'Наименование организации', 'Наименование организации'], 'Полное наименование (латинскими буквами)' => ['foreignorgname', 'Наименование иностранной организации', 'Наименование иностранной организации'], 'ИНН' => ['inn', 'ИНН', 'ИНН'], 'ОГРН' => ['ogrn', 'ОГРН', 'ОГРН'], 'Регистрационный номер юридического лица в стране его регистрации' => ['regnumber', 'Регистрационный номер юридического лица', 'Регистрационный номер юридического лица'], 'Место нахождения' => ['region', 'Регион', 'Регион'], 'Адрес для направления корреспонденции' => ['country', 'Страна', 'Страна'], 'Наименование' => ['doc_name', 'Наименование документа', 'Наименование документа'], 'Дата договора' => ['doc_date', 'Дата документа', 'Дата документа'], 'Номер договора' => ['doc_num', 'Номер документа', 'Номер документа'], 'Срок исполнения обязательства, обеспеченного залогом движимого имущества' => ['doc_enddate', 'Срок действия', 'Срок действия']];
    ...
    {
        $title = ['' => 'Поиск в реестре залогов федеральной нотариальной палаты', 'reestrzalogov_person' => 'Реестр залогов ФНП - поиск заложенного имущества физлица', 'reestrzalogov_org' => 'Реестр залогов ФНП - поиск заложенного имущества организации', 'reestrzalogov_auto' => 'Реестр залогов ФНП - проверка автомобиля'];

        return isset($title[$checktype]) ? $title[$checktype] : $title[''];
        //        return 'Поиск в реестре залогов федеральной нотариальной палаты';
    }

    public function getSessionData(array $params, $sourceid = 60, $nocaptcha = 0)
    {
        /** @var \Doctrine\DBAL\Connection $mysqli */
        $mysqli = $params['_connection'];
        $reqId = $params['_reqId'];
        $sessionData = null;
        $result = $mysqli->executeQuery('UPDATE session s SET request_id='.$reqId." WHERE sessionstatusid=2 AND sourceid={$sourceid} AND (captcha='' OR unix_timestamp(now())-unix_timestamp(captchatime)<110) ORDER BY lasttime limit 1");
        ...
        if ($result && 0 == $result->rowCount() && $nocaptcha) {
            $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,'' captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sessionstatusid IN (1,2,3,4,5) AND sourceid={$sourceid} ORDER BY lasttime limit 1");
        } else {
            $nocaptcha = false;
        }
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->executeStatement('UPDATE session SET '.($sessionData->nocaptcha ? '' : 'sessionstatusid=3,endtime=now(),')."lasttime=now(),statuscode='used',used=ifnull(used,0)+1,request_id=NULL WHERE id=".$sessionData->id);
            }
        }
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            ...
            $swapData['session'] = $this->getSessionData($params, 60);
            $rContext->setSwapData($swapData);
            if (!$swapData['session']) {
        ...
        */
        if (!isset($swapData['captcha_token']) && !isset($swapData['captcha_id'.$swapData['num']])) {
            $swapData['captcha_session'] = $this->getSessionData($params, isset($swapData['row']) ? 59 : 5);
        ...
        if (!isset($swapData['captcha_token'])) {
            if (!isset($swapData['captcha_id'.$swapData['num']])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    $params = ['key' => $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['key'], 'method' => 'userrecaptcha', 'googlekey' => $this->googlekey, 'pageurl' => $page, 'version' => 'v3', 'action' => isset($swapData['row']) ? 'extract_actual_notification' : 'search_notary', 'min_score' => $this->minscore];
                    /*
                    ...
                    */
                    ...
                    $params = ['clientKey' => $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['key'], 'task' => [
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    ...
                    \curl_setopt($ch, \CURLOPT_POST, true);
                    \curl_setopt($ch, \CURLOPT_POSTFIELDS, \json_encode($params, \JSON_UNESCAPED_UNICODE));
                    \curl_setopt($ch, \CURLOPT_HTTPHEADER, ['Content-Type: application/json;charset=UTF-8', 'Accept: application/json;charset=UTF-8']);
                }
            }
        ...
        //            echo "\n";
            ...
            if (isset($swapData['row'])) {
                ...
                if ('auto' == $checktype) {
                    $params['filter']['property']['vehicleProperty'] = ['vin' => isset($initData['vin']) ? $initData['vin'] : '', 'bodyNum' => !isset($initData['vin']) && isset($initData['bodynum']) ? $initData['bodynum'] : '', 'chassis' => !isset($initData['vin']) && !isset($initData['bodyNum']) && isset($initData['chassis']) ? $initData['chassis'] : ''];
                } elseif ('org' == $checktype) {
                    $params['filter']['pledgor']['russianOrganization'] = ['name' => '-', 'ogrn' => '', 'inn' => $initData['inn']];
                } else {
                    $params['filter']['pledgor']['privatePerson'] = ['firstName' => $initData['first_name'], 'lastName' => $initData['last_name'], 'middleName' => isset($initData['patronymic']) ? $initData['patronymic'] : '', 'birthday' => isset($initData['date']) ? \date('d.m.Y', \strtotime($initData['date'])) : ''];
                }
                /*
                            } else {
            ...
            if ($params) {
                \curl_setopt($ch, \CURLOPT_POST, true);
                \curl_setopt($ch, \CURLOPT_POSTFIELDS, \json_encode($params, \JSON_UNESCAPED_UNICODE));
                \curl_setopt($ch, \CURLOPT_HTTPHEADER, ['Content-Type: application/json;charset=UTF-8', 'Origin: '.$site, 'Referer: '.$page, 'X-Requested-With: XMLHttpRequest']);
            }
    ...
    public function computeRequest(array $params, &$rContext)
    {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service'.$swapData['num']]]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                            './logs/rz/'.$initData['checktype'].'_captcha_err_'.\time().'.txt',
        ...
        */
        if (isset($swapData['row']) && '%PDF' == \substr($content, 0, 4)) {
            ...
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents("./logs/rz/{$filename}.pdf", $content);
            $descriptorspec = [
                //                0 => ['pipe', 'r'], //stdin
                1 => ['pipe', 'w'],
                // stdout
            ...
            if (++$swapData['row'] >= \count($swapData['data'])) {
                ...
                foreach ($swapData['data'] as $item) {
                    ...
                    $change = ['CREATION' => ['name' => 'start', 'title' => 'Возникновение'], 'CHANGE' => ['name' => 'change', 'title' => 'Изменение'], 'EXCLUSION' => ['name' => 'end', 'title' => 'Исключение'], 'JUDICIAL_ACT_EXCLUSION' => ['name' => 'end', 'title' => 'Исключение по суду']];
                            ...
                            foreach ($row as $key => $value) {
                                if ($value) {
                                    if (isset($this->names[$key])) {
                                        ...
                                        //                                } elseif ($key!='type') {
                                        //                                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fields/rz_'.time().'_'.$key.'.txt', $value);
                                    }
                                }
                            }
                    ...
                    if (isset($item['pdfdata'][1])) {
                        foreach ($item['pdfdata'][1] as $row) {
                            if (\count($row) > 1 && (!isset($initData['vin']) || isset($row['VIN']) && $row['VIN'] == $initData['vin'] || isset($row['ID']) && $row['ID'] == $initData['vin'])) {
...
' => ''])));
                                        if (isset($this->names[$key])) {
                                            ...
                                            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/fields/rz_properties_'.\time().'_'.$key.'.txt', $value);
                                        }
                                    }
                                }
                                if (isset($item['referenceStatus'])) {
                ...
                $rContext->setFinished();
                $mysqli->executeStatement("UPDATE session SET success=ifnull(success,0)+1,statuscode='success' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),59,3,'success','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
            ...
            if (isset($swapData['captcha_session'])) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,success=ifnull(success,0)+1,statuscode='success' WHERE statuscode='used' AND id=".$swapData['captcha_session']->id);
            ...
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_denied_'.\time().'.txt', $content);
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),59,4,'forbidden','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
                //                    echo "Captcha ID ".$swapData['captcha_id']." from ".$swapData['captcha_service']." reported as bad with result $res\n";
            }
            if (isset($swapData['captcha_session'])) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=4,statuscode='forbidden' WHERE statuscode='used' AND id=".$swapData['captcha_session']->id);
                unset($swapData['captcha_session']);
            }
            unset($swapData['captcha_id']);
            unset($swapData['captcha_token']);
        ...
        $res = \json_decode($content, true);
        //        if (!empty(trim($content))) \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_'./*(isset($swapData['key'])?'':'key_').*/time().'.txt',$content);
        /*
        ...
        */
        if (\is_array($res) && isset($res['data']) && \is_array($res['data'])) {
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,3,'success','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
                //                echo "Captcha ID ".$swapData['captcha_id']." from ".$swapData['captcha_service']." reported as good with result $res\n";
            }
            if (isset($swapData['captcha_session'])) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,success=ifnull(success,0)+1,statuscode='success' WHERE statuscode='used' AND id=".$swapData['captcha_session']->id);
            ...
            if (\strpos($res['message'], 'token') || \strpos($res['message'], 'капча')) {
                if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                    $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,4,'invalidcaptcha','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
                    //                    echo "Captcha ID ".$swapData['captcha_id']." from ".$swapData['captcha_service']." reported as bad with result $res\n";
                }
                if (isset($swapData['captcha_session'])) {
                    $mysqli->executeStatement("UPDATE session SET sessionstatusid=4,statuscode='invalidcaptcha' WHERE statuscode='used' AND id=".$swapData['captcha_session']->id);
                    unset($swapData['captcha_session']);
                }
                unset($swapData['captcha_id']);
                unset($swapData['captcha_token']);
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_err_'.\time().'.txt', $content);
                $error = \trim($res['message']);
                $mysqli->executeStatement('UPDATE session SET '.($swapData['session']->nocaptcha ? '' : 'sessionstatusid=3,endtime=now(),')."success=ifnull(success,0)+1,statuscode='success' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
        } elseif (\preg_match('/Доступ запрещен/', $content)) {
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_denied_'.\time().'.txt', $content);
            /*
                            $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 1 hour),sessionstatusid=6,statuscode='forbidden' WHERE id=" . $swapData['session']->id);
            ...
            */
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service'])) {
                $mysqli->executeStatement("INSERT INTO session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,4,'forbidden','".$swapData['captcha_token']."','".$swapData['captcha_service']."','".$swapData['captcha_id']."')");
                //                    echo "Captcha ID ".$swapData['captcha_id']." from ".$swapData['captcha_service']." reported as bad with result $res\n";
            }
            if (isset($swapData['captcha_session'])) {
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=4,statuscode='forbidden' WHERE statuscode='used' AND id=".$swapData['captcha_session']->id);
            ...
            $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,endtime=now(),statuscode='unavailable' WHERE statuscode='used' AND id=".$swapData['session']->id);
            unset($swapData['session']);
            ...
            //            if (!$swapData['session']->nocaptcha)
            $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,endtime=now(),statuscode='error' WHERE statuscode='used' AND id=".$swapData['session']->id);
            unset($swapData['session']);
        } else {
            if ($content) {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rz/'.$initData['checktype'].'_err_'.\time().'.txt', $content);
            ...
            if ($content) {
                //                if (!$swapData['session']->nocaptcha)
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,endtime=now(),statuscode='invalidanswer' WHERE statuscode='used' AND id=".$swapData['session']->id);
                $error = 'Некорректный ответ сервиса';
            } else {
                //                if (!$swapData['session']->nocaptcha)
                $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,endtime=now(),statuscode='empty' WHERE statuscode='used' AND id=".$swapData['session']->id);
            }
            unset($swapData['session']);
        }
        $rContext->setSwapData($swapData);
