...
class ReestrZalogovPlugin_old implements PluginInterface
{
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            ...
            $swapData['session'] = $this->getSessionData(5, $swapData['iteration'] > 30);
            $rContext->setSwapData($swapData);
            if (!$swapData['session']) {
        ...
        if (isset($swapData['row']) && !isset($swapData['captcha_token'])) {
            $swapData['row_session'] = $this->getSessionData(59);
            if ($swapData['row_session']) {
                $swapData['captcha_token'] = $swapData['row_session']->code;
            }
        }
...
' => ''])));
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                ...
                */
                $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),59,3,'success','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                //                echo "Captcha ID ".$swapData['captcha_id']." reported as good with result $res\n";
            }

            if (isset($swapData['row_session'])) {
                $mysqli->query("UPDATE isphere.session SET sessionstatusid=3,success=ifnull(success,0)+1,statuscode='success' WHERE statuscode='used' AND id=".$swapData['row_session']->id);
            }
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                /*
                                    ...
                                    $res = file_get_contents($url);
                */
                $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),59,4,'forbidden','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                //                    echo "Captcha ID ".$swapData['captcha_id']." reported as bad with result $res\n";
            }

            if (isset($swapData['row_session'])) {
                $mysqli->query("UPDATE isphere.session SET sessionstatusid=4,statuscode='forbidden' WHERE statuscode='used' AND id=".$swapData['row_session']->id);
                unset($swapData['row_session']);
            }
            unset($swapData['captcha_id']);
            unset($swapData['captcha_token']);
            //                unset($swapData['key']);
        ...
        if (\is_array($res) && isset($res['data']) && \is_array($res['data'])) {
            ...
            if (isset($swapData['captcha_id']) && isset($swapData['captcha_token']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                /*
                                    ...
                                    $res = file_get_contents($url);
                */
                $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captcha,captcha_service,captcha_id) VALUES (now(),now(),5,4,'forbidden','".$swapData['captcha_token']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                //                    echo "Captcha ID ".$swapData['captcha_id']." reported as bad with result $res\n";
            }
