...
class RossvyazPlugin implements PluginInterface
{
    ...
    public function getSessionData(array $params)
    {
        ...
        if ($result) {
            ...
            if ($row) {
                ...
                $sessionData->proxy = $row['proxy'];
                $sessionData->proxy_auth = \strlen($row['proxy_auth']) > 1 ? $row['proxy_auth'] : false;
                $connection->executeStatement("UPDATE session SET lasttime=now(),used=ifnull(used,0)+1,statuscode='used' WHERE id=".$sessionData->id);
            }
        }
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        /** @var Connection $rossvyazConnection */
        $rossvyazConnection = $params['_rossvyazConnection'];
        $initData = $rContext->getInitData();
        $swapData = $rContext->getSwapData();
        ...
        if (10 == \strlen($phone_number) && ('3' == \substr($phone_number, 0, 1) || '4' == \substr($phone_number, 0, 1) || '8' == \substr($phone_number, 0, 1) || '9' == \substr($phone_number, 0, 1))) {
            ...
            $result = $rossvyazConnection->executeQuery("SELECT p.*,ps.name phone_standart FROM phoneinfo p, phonestandart ps WHERE p.code={$phone_code} AND p.start<={$phone_shortnumber} AND p.end>={$phone_shortnumber} AND p.phonestandartid=ps.id");
            if ($result) {
                $row = $result->fetchAssociative();
                if ($row) {
            ...
            $result = $rossvyazConnection->executeQuery("SELECT * FROM phoneinfo WHERE othercode={$phone_code} AND otherstart IS NOT NULL AND otherstart<={$phone_shortnumber} AND otherend>={$phone_shortnumber}");
            if ($result) {
                $row = $result->fetchAssociative();
                if ($row) {
                    if (null != $row['start']) {
        ...
        if (!$swapData['session']) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rossvyaz/bdpn_'.time().'.txt',$content);
            ...
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/rossvyaz/bdpn_err_'.\time().'.txt', $content);
            $mysqli->executeStatement("UPDATE session SET sessionstatusid=3,statuscode='error',endtime=now() WHERE statuscode='used' AND id=".$swapData['session']->id);
