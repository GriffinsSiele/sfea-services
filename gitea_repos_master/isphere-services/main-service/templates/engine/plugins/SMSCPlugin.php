...
class SMSCPlugin implements PluginInterface
{
    ...
    private $hlrErrors = [0 => 'Доступен', 1 => 'Не существует', 6 => 'Не в сети', 11 => 'Нет услуги SMS', 13 => 'Заблокирован', 21 => 'Не принимает SMS', 99 => 'Неизвестная ошибка', 248 => 'Неизвестный оператор', 249 => 'Неверный номер', 250 => 'Ограничен доступ', 251 => 'Превышен лимит', 252 => 'Номер запрещен', 253 => 'Услуга не поддерживается', 255 => 'Запрос отклонен'];
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $host = 'http://www2.smsc.ru';
        if (isset($swapData['mode']) && 'status' == $swapData['mode']) {
            $params = ['phone' => $initData['phone'], 'login' => $this->login, 'psw' => $this->password, 'id' => $swapData['smsc_id'], 'fmt' => 3, 'over' => 1];
            $url = $host.'/sys/status.php';
        } else {
            $params = ['phones' => $initData['phone'], 'login' => $this->login, 'psw' => $this->password, 'hlr' => 1];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/smsc/smsc_' . time() . '.json', $content);
            ...
            if (isset($swapData['mode']) && 'status' == $swapData['mode']) {
                ...
                if ('OK' == \substr($content, 0, 2)) {
                    // $swapData['smsc_id'] = \App\Utils\Legacy\StrUtilStatic::str_between($content . ',','ID - ',',');
                    ...
                    if (false !== \strpos($content, 'ERROR')) {
                        ...
                        $error = 'Невозможно обработать ответ сервиса';
                    }
                    if ($error) {
                        $rContext->setError($error);
                        $rContext->setFinished();
