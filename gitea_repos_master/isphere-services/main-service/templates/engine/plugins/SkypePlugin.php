...
class SkypePlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в Skype', 'skype_phone' => 'Skype - поиск по номеру телефона', 'skype_email' => 'Skype - поиск по email', 'skype' => 'Skype - профиль пользователя'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $reqId = $params['_reqId'];
        $sessionData = null;
        $mysqli->executeStatement("UPDATE session s SET request_id={$reqId} WHERE sessionstatusid=2 AND sourceid=16 AND unix_timestamp(now())-unix_timestamp(lasttime)>1 ORDER BY lasttime limit 1");
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sourceid=16 AND request_id={$reqId} ORDER BY lasttime limit 1");
        if ($result) {
            ...
            if ($row) {
                ...
                $mysqli->executeStatement("UPDATE session SET lasttime=now(),statuscode='used',used=ifnull(used,0)+1 WHERE id=".$sessionData->id);
                if (!$row['proxyid']) {
                    $result = $mysqli->executeQuery("SELECT id proxyid, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM proxy WHERE enabled=1 AND status=1 AND proxygroup=1 AND id NOT IN (SELECT proxyid FROM session WHERE sourceid=16 AND proxyid IS NOT NULL) ORDER BY lasttime limit 1");
                    if ($result) {
                        $row = $result->fetchAssociative();
                        if ($row) {
                            $sessionData->proxyid = $row['proxyid'];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!$content) {
            ...
            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/skype/skype_empty_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$full_content."\n".$error."\n".$code);
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/skype/skypesearch'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$full_content);
            $res = \json_decode($content, true);
            if ($res && isset($res['results'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/skype/skypesearch_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$full_content);
            }
        } elseif (!isset($swapData['photo'])) {
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/skype/skypeprofile'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$full_content);
            $res = \json_decode($content, true);
            if ($res && isset($res[0])) {
                $data = [];
                if (isset($res[0]['status']['text'])) {
                    if (false !== \strpos($res[0]['status']['text'], 'invalid username') || false !== \strpos($res[0]['status']['text'], 'not found') || false !== \strpos($res[0]['status']['text'], 'required parameters')) {
                        ...
                        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/skype/skypeprofile_err_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$full_content);
                    }
            ...
            if (!\strpos($content, 'W5M0MpCehiHzreSzNTczkc9d')) {
                ...
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./'.$name,$content);
        ...
        if (\preg_match('/Skypetoken has/', $error)) {
            $error = false;
            $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 6 hour),sessionstatusid=6,statuscode='revoked' WHERE id=".$swapData['session']->id);
        }
        if ($error) {
