...
class TwitterPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск в Twitter', 'twitter_phone' => 'Twitter - проверка телефона на наличие пользователя', 'twitter_email' => 'Twitter - проверка email на наличие пользователя', 'twitter_url' => 'Twitter - профиль пользователя'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $reqId = $params['_reqId'];
        $sessionData = null;
        $mysqli->executeStatement("UPDATE session s SET request_id={$reqId} WHERE sessionstatusid=2 AND sourceid=50 AND unix_timestamp(now())-unix_timestamp(lasttime)>10 ORDER BY lasttime limit 1");
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,data,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sessionstatusid=2 AND sourceid=50 AND request_id={$reqId} ORDER BY lasttime limit 1");
        if ($result) {
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        if (isset($swapData['path'])) {
            ...
            $post = ['authenticity_token' => $swapData['session']->token, 'account_identifier' => isset($initData['phone']) ? $initData['phone'] : $initData['email']];
            $header = ['Referer: '.$url, 'Origin: '.$site, 'Content-Type: application/x-www-form-urlencoded'];
        }
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$curl_error) {
            ...
            if (!isset($swapData['path'])) {
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/twitter/twitter_'.time().'.html',/*curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".*/$content);
                if (\strpos($content, 'send_password_reset') || \strpos($content, 'verify_user_info') || \strpos($content, 'нашли несколько') || \strpos($content, 'нашли более') || \strpos($content, 'не можем найти') || \strpos($content, "couldn't find") || \strpos($content, 'tidak dapat menemukan')) {
                    ...
                    //                    $mysqli->query("UPDATE session SET endtime=now(),sessionstatusid=3,statuscode='success' WHERE id=" . $swapData['session']->id);
                    ...
                    //                    $mysqli->query("UPDATE session SET unlocktime=date_add(now(),interval 1 minute),sessionstatusid=6,statuscode='limit' WHERE id=" . $swapData['session']->id);
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/twitter/twitter_err_'.\time().'.html', $content);
                    $mysqli->executeStatement("UPDATE session SET unlocktime=date_add(now(),interval 30 minute),sessionstatusid=6,statuscode='invalid' WHERE id=".$swapData['session']->id);
                    unset($swapData['session']);
                }
            } else {
                //                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/twitter/twitter_url_'.time().'.html',$content);
                $resultData = new ResultDataList();
                //                    $rContext->setResultData($resultData);
                $rContext->setFinished();
            }
