...
class VKCheckPlugin_new implements PluginInterface
{
...
//        array('host' => 'api.capmonster.cloud', 'key' => 'afb26cbb248d650ea8b8d88822984242'),
    ...
    public function getSessionData($nocaptcha)
    {
        ...
        $mysqli->query('UPDATE isphere.session s SET lasttime=now(),request_id='.$reqId.' WHERE request_id IS NULL AND sessionstatusid=2 AND sourceid=22 AND unix_timestamp(now())-unix_timestamp(lasttime)>5 ORDER BY lasttime limit 1');
        $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,captcha,token,enckey,device,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=22 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
        if ($result && 0 == $result->num_rows && $nocaptcha) {
            $mysqli->query('UPDATE isphere.session s SET lasttime=now(),request_id='.$reqId.' WHERE request_id IS NULL AND sessionstatusid IN (3,5) AND sourceid=22 ORDER BY lasttime limit 1');
            $result = $mysqli->query("SELECT id,cookies,starttime,lasttime,'' captcha,token,enckey,device,proxyid,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM isphere.session s WHERE sourceid=22 AND request_id=".$reqId.' ORDER BY lasttime limit 1');
            $nocaptcha = true;
        } else {
            $nocaptcha = false;
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        if (!isset($swapData['session'])) {
            ...
            $swapData['session'] = $this->getSessionData($swapData['iteration'] > 50);
            if (!$swapData['session']) {
            ...
            if (($swapData['iteration'] > 50) && \rand(0, 2)) {
                $astro = ['213.108.196.179:10687'/* ,'94.247.132.131:10127',193.23.50.59:10451 */];
...
//                            "maxLength" => 6,
    ...
    public function computeRequest(array $params, &$rContext)
    {
            ...
            if (!isset($swapData['captcha_id'])) {
                ...
                if ('rucaptcha.com' == $this->captcha_service[$swapData['captcha_service']]['host']) {
                    if (false !== \strpos($content, 'OK|')) {
                        ...
                        \file_put_contents('./logs/vk/'.$initData['checktype'].'_captcha_image_err_'.$swapData['iteration'].'_'.\time().'.txt', /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */ $content."\r\n".$this->captcha_service[$swapData['captcha_service']]['host']);
                    }
                } else {
                    if (isset($res['taskId'])) {
        ...
        */
        if (!$error) {
            \file_put_contents('./logs/vk/vk_restore_'.$swapData['iteration'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
            //            if (microtime(true)-$swapData['start_time']>60 || $swapData['iteration']>10)
            //                file_put_contents('./logs/vk/vk_restore_slow_'.$swapData['iteration'].'_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
            ...
            if ($res && isset($res['response'][1])) {
                if (isset($swapData['captcha_id']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                    ...
                    */
                    //                    $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captchaimage,captcha,captcha_service,captcha_id) VALUES (now(),now(),22,3,'success','".$swapData['captcha_image']."','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                ...
                if (isset($swapData['captcha_image']) && isset($swapData['captcha_value'])) {
                    \file_put_contents('./logs/vk/captcha/good/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
                }
                //                unset($swapData['captcha']);
                //                unset($swapData['captcha_image']);
                ...
                if (!isset($swapData['hash'])) {
                    ...
                    if (!isset($initData['last_name'])) {
                        ...
                        \file_put_contents('./logs/vk/vk_restore_nohash_'.$swapData['iteration'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
                        ...
                        /*
                                                ...
                                                file_put_contents('./logs/vk/vk_restore_err_'.$swapData['iteration'].'_'.time().'.txt',curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".$content);
                ...
                if (100 == $res['error']['error_code']) {
                    ...
                    $mysqli->query('UPDATE isphere.session SET unlocktime=date_add(now(),interval '.($swapData['session']->proxyid < 100 ? '15 second' : '30 minute')."),sessionstatusid=6,statuscode='limit' WHERE sourceid=22 AND proxyid=".$swapData['session']->proxyid.' ORDER BY lasttime DESC LIMIT 10');
                    //                    if ($swapData['session']->proxyid)
                    //                        $mysqli->query("UPDATE isphere.proxy SET success=success+1,successtime=now() WHERE id=".$swapData['session']->proxyid);
                    unset($swapData['session']);
                //                    unset($swapData['captcha']);
                    ...
                    if (isset($swapData['captcha_id']) && isset($swapData['captcha_service']) /* && $this->captcha_service[$swapData['captcha_service']]['host']=='rucaptcha.com' */) {
                        /*
                                                ...
                                                $res = file_get_contents($url);
                        */
                        //                        $mysqli->query("INSERT INTO isphere.session (endtime,captchatime,sourceid,sessionstatusid,statuscode,captchaimage,captcha,captcha_service,captcha_id) VALUES (now(),now(),22,4,'invalidcaptcha','".$swapData['captcha_image']."','".$swapData['captcha_value']."','".$this->captcha_service[$swapData['captcha_service']]['host']."','".$swapData['captcha_id']."')");
                    ...
                    if (isset($swapData['captcha_image']) && isset($swapData['captcha_value'])) {
                        \file_put_contents('./logs/vk/captcha/bad/'.$swapData['captcha_value'].'.jpg', \base64_decode($swapData['captcha_image']));
                    }
                    unset($swapData['captcha_image']);
                    ...
                    */
                    ...
                    \file_put_contents('./logs/vk/vk_unknown_err_'.$swapData['iteration'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
                }
                ...
                if ($content) {
                    \file_put_contents('./logs/vk/vk_invalid_err_'.$swapData['iteration'].'_'.\time().'.txt', \curl_getinfo($rContext->getCurlHandler(), \CURLINFO_HEADER_OUT)."\r\n".$content);
