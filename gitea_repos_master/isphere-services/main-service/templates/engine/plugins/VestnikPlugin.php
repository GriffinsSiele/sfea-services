...
class VestnikPlugin implements PluginInterface
{
    ...
    {
        $title = ['' => 'Поиск сведений в вестнике государственной регистрации', 'vestnik_org' => 'Поиск сообщений организации в вестнике государственной регистрации', 'vestnik_fns' => 'Поиск решений ФНС о предстоящем исключении недействующей организации из ЕГРЮЛ'];
    ...
    public function getSessionData(array $params)
    {
        ...
        $sessionData = null;
        $result = $mysqli->executeQuery("SELECT id,cookies,starttime,lasttime,captcha,token,proxyid,(SELECT server FROM proxy WHERE id=s.proxyid) server,(SELECT concat(server,':',port) FROM proxy WHERE id=s.proxyid) proxy,(SELECT concat(login,':',password) FROM proxy WHERE id=s.proxyid) proxy_auth FROM session s WHERE sessionstatusid=2 AND sourceid=45 ORDER BY lasttime limit 1");
        //        $result = $mysqli->query("SELECT id proxyid, server, concat(server,':',port) proxy, concat(login,':',password) proxy_auth FROM proxy WHERE status=1 ORDER BY lasttime limit 1");
        if ($result) {
        ...
        */
        ...
        if ('org' == $checktype) {
            ...
            if (!isset($swapData['captcha_token'])) {
                ...
                $url = $page;
                $a = \ip2long($swapData['session']->ip ?: '78.140.221.69');
                $b = 8369;
                $c = 4104;
                $d = 4786;
                $validate = \round(\abs(($a * $b / $c / $d + $d) ** 3 / ($b * $c) - $d) * 100) / 100;
                $post = ['query' => isset($initData['inn']) ? $initData['inn'] : $initData['ogrn'], 'validate' => $validate, 'page' => 1, 'key' => '6wg19hdz', 'g-recaptcha-response' => $swapData['captcha_token']];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!isset($swapData['captcha_token'])) {
            if (!isset($swapData['captcha_id'])) {
                if (false !== \strpos($content, 'OK|')) {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                        './logs/vestnik/'.$initData['checktype'].'_captcha_err_'.\time().'.txt',
                ...
                if ('CAPCHA_NOT_READY' == $content) {
                    $rContext->setSleep(3);
                } elseif (false !== \strpos($content, 'OK|')) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/vestnik/vestnik_org_'.time().'_'.$swapData['iteration'].'.html',/*curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n".*/$content);
            ...
            if (\preg_match('/<div class="search-result-entries">/', $content)) {
                ...
                foreach ($records as $i => $record) {
                    ...
                    if (\preg_match('/№([\\d\\(\\)]+)\\s/', $record, $matches)) {
                        $data['issue_number'] = new ResultDataField('string', 'issue_number', \trim(\html_entity_decode($matches[1])), 'Номер выпуска', 'Номер выпуска');
                    }
                    if (\preg_match('/часть ([\\d\\(\\)]+)\\s/', $record, $matches)) {
                        $data['issue_part'] = new ResultDataField('string', 'issue_part', \trim(\html_entity_decode($matches[1])), 'Часть выпуска', 'Часть выпуска');
                    }
                    if (\preg_match('/от ([\\d\\.]{10})/', $record, $matches)) {
                        $data['issue_date'] = new ResultDataField('string', 'issue_date', \trim(\html_entity_decode($matches[1])), 'Дата', 'Дата выпуска');
                    }
                    if (\preg_match('/\\/ ([\\d]+)</', $record, $matches)) {
                        $data['message_number'] = new ResultDataField('string', 'message_number', \trim(\html_entity_decode($matches[1])), 'Номер сообщения', 'Номер сообщения');
                    }
                    if (\preg_match('/ИНН: (\\d+)/', $record, $matches)) {
                        $data['inn'] = new ResultDataField('string', 'inn', \trim(\html_entity_decode($matches[1])), 'ИНН', 'ИНН');
                    }
                    if (\preg_match('/ОГРН: (\\d+)/', $record, $matches)) {
                        $data['ogrn'] = new ResultDataField('string', 'ogrn', \trim(\html_entity_decode($matches[1])), 'ОГРН', 'ОГРН');
                    }
                    if (\preg_match('/<b>([^<]+)<\\/b>/', $record, $matches)) {
                        $data['subject'] = new ResultDataField('string', 'subject', \trim(\html_entity_decode($matches[1])), 'Тема', 'Тема сообщения');
                    }
                    if (\preg_match('/<div class="body-massege-content">(.*?)<br>/sim', $record, $matches)) {
                        $data['message'] = new ResultDataField('string', 'message', \trim(\html_entity_decode(\strip_tags($matches[1]))), 'Сообщение', 'Сообщение');
                    }
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents(
                    './logs/vestnik/vestnik_org_err_'.\time().'.html',
                    /* curl_getinfo($rContext->getCurlHandler(),CURLINFO_HEADER_OUT)."\r\n". */
                    $content
                );
            }
