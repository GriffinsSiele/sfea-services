...
class ViberWinPlugin implements PluginInterface
{
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        $total_timeout = $params['_total_timeout'];
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$content) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/viber/viber_'.(isset($swapData['path'])?$swapData['path'].'_':'').time().'.txt',$content);
            ...
            if ($res && isset($res['status']) && 'success' == $res['status'] && isset($res['viberstatus']) && ('error' != $res['viberstatus'] || $swapData['iteration'] > 10)) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/viber/viber_err_'.(isset($swapData['path']) ? $swapData['path'].'_' : '').\time().'.txt', $content);
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/viber/viber_err_'.(isset($swapData['path']) ? $swapData['path'].'_' : '').\time().'.txt', $content);
