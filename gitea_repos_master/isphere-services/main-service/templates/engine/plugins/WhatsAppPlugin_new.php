...
class WhatsAppPlugin_new implements PluginInterface
{
    ...
    public function prepareRequest(array $params, &$rContext)
    {
        ...
        */
        ...
        \file_put_contents('./logs/whatsapp/whatsapp_'.\time().'.txt', $content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \file_put_contents('./logs/whatsapp/whatsapp_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \file_put_contents('./logs/whatsapp/whatsapp_err_'.\time().'.txt', $content);
