...
class YaMapPlugin implements PluginInterface
{
...
//                $db->connect('172.16.11.1', 6379, 1, null, 100, 1);
                ...
                $db->rpush('yamap_queue', $initData['phone']);
                $swapData['db'] = $db;
                //                $rContext->setSleep(1);
        ...
        try {
            ...
            if ($db->hexists('yamap', $initData['phone'])) {
                $content = $db->hget('yamap', $initData['phone']);
                $db->close();
                unset($swapData['db']);
            } else {
                if ($swapData['iteration'] > 20) {
                    $db->hdel('yamap_queue', $initData['phone']);
                    $db->close();
                    $rContext->setFinished();
        ...
        $error = false;
        //        \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/yamap/yamap_'.time().'.txt',$content);
        if (!$content) {
            $rContext->setFinished();
            ...
            if (\is_array($res) && isset($res['status']) && 'ok' == $res['status'] && isset($res['records'])) {
                ...
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/yamap/yamap_err_'.\time().'.txt', $content);
                $error = $res['message'];
            } else {
                \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/yamap/yamap_err_'.\time().'.txt', $content);
