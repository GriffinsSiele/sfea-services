...
//                           'Объект закупки: цена за единицу, рублей' => array('item_price','Цена объекта закупки','Цена объекта закупки'),
//                           'Объект закупки: количество' => array('item_amount','Количество объекта закупки','Количество объекта закупки'),
//                           'Объект закупки: сумма, рублей' => array('item_total','Сумма объекта закупки','Сумма объекта закупки'),
...
//                           'ОГРН' => array('ogrn'),
    ...
    public function computeRequest(array $params, &$rContext)
    {
        ...
        if (!$error) {
            ...
            //            \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/zakupki/zakupki_'.$checktype.'_'.$initData['inn'].'_'.$swapData['page'].'_'.time().'.'.$format,$content);
            ...
            if ('csv' == $format && !$content) {
                ...
                if (\preg_match('/<div class="search-registry-entry-block/', $content)) {
                    ...
                    \App\Utils\Legacy\LoggerUtilStatic::file_put_contents('./logs/zakupki/zakupki_'.$checktype.'_err_'.$initData['inn'].'_'.$swapData['page'].'_'.\time().'.'.$format, $content);
