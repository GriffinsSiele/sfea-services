...
echo '<option value="all"'.(isset($_REQUEST['pay']) && 'separate' == $_REQUEST['pay'] ? ' selected' : '').'>Все тарифы</option>';
...
echo '<option value="login"'.(!isset($_REQUEST['order']) || 'login' == $_REQUEST['order'] ? ' selected' : '').'>По логину</option>';
echo '<option value="created"'.(isset($_REQUEST['order']) && 'created' == $_REQUEST['order'] ? ' selected' : '').'>По дате создания</option>';
echo '<option value="lastused"'.(isset($_REQUEST['order']) && 'lastused' == $_REQUEST['order'] ? ' selected' : '').'>По дате последнего запроса</option>';
echo '</select>';
echo ' <input type="submit" value="Обновить"></form>';

if (isset($_REQUEST['client_id']) && 0 != (int) $_REQUEST['client_id']) {
    $conditions .= ' AND ClientId='.(int) $_REQUEST['client_id'];
}
...
if (isset($_REQUEST['user_id']) && 0 != (int) $_REQUEST['user_id']) {
    $conditions .= ' AND (Id='.(int) $_REQUEST['user_id'];
    if ($user_area >= 1) {
        $conditions .= ' OR Id IN (SELECT Id FROM SystemUsers WHERE MasterUserId='.(int) $_REQUEST['user_id'].')';
        if ($user_area > 1) {
            $conditions .= ' OR Id IN (SELECT Id FROM SystemUsers WHERE MasterUserId IN (SELECT id FROM SystemUsers WHERE MasterUserId='.(int) $_REQUEST['user_id'].'))';
        }
    }
    $conditions .= ')';
}
...
if (isset($_REQUEST['access']) && '' != $_REQUEST['access']) {
    $conditions .= ' AND u.AccessLevel='.(int) $_REQUEST['access'];
}
...
if (5 == $userid && isset($_REQUEST['debug'])) {
    echo \strtr($sql."\n", ["\n" => '<br>']).'<br><br>';
}
