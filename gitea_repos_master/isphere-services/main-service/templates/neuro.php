...
function neuro_post($image, $method)
{
    ...
    if ((false !== \strpos($res['text'], '[UNK]')) || (false !== \strpos($res['text'], '-'))) {
...
function neuro_token($method, $sitekey, $action = '')
{
    ...
    $tries = 0;
    do {
        $result = \file_get_contents('http://172.16.1.25'.(4 - $tries % 2).":8003/$method?sitekey=$sitekey".($action ? "&action=$action" : ''), false, $context);
    } while (!$result && ++$tries <= 3);
    $res = \json_decode($result, true);
    if (!$res) {
        return 'ERROR_BAD_ANSWER';
