...
echo ' Период с <input type="text" name="from" value="'.(isset($_REQUEST['from']) ? $_REQUEST['from'] : \date('01.m.Y')).'">';
echo ' по <input type="text" name="to" value="'.(isset($_REQUEST['to']) ? $_REQUEST['to'] : '').'">';
if ($user_level < 0) {
...
echo '<option value="dates"'.('dates' == $type || '' == $type ? ' selected' : '').'>По датам</option>';
...
echo '<option value="sources"'.('sources' == $type ? ' selected' : '').'>По источникам</option>';
...
if (!$type) {
    ...
    $addfields = ('dates' == $type || 'months' == $type || \strpos($conditions, 'created_date') ? 'created_date,' : '')./* (strpos($conditions,'created_at')?'created_at,':''). */ /* ($type=='sources'?'source_name,':'').($type=='clients'?'client_id,':'').($type=='users'?'user_id,':''). */ ('checktypes' == $type || \strpos($conditions, 'checktype') ? 'checktype,' : '');
    $addgroups = $addfields;
    ...
    while ($result = $sqlRes->fetch_assoc()) {
        ...
        foreach ($result as $key => $val) {
            if (!isset($hide[$viewtype][$key]) && isset($title[$key])) {
                ...
                if ('minute' == $key && isset($_REQUEST['from']) && \strtotime($_REQUEST['from']) && ((isset($_REQUEST['to']) && \strtotime($_REQUEST['to']) && \date('d.m.Y H', \strtotime($_REQUEST['from'])) == \date('d.m.Y H', \strtotime($_REQUEST['to']))) || ((!isset($_REQUEST['to']) || !$_REQUEST['to']) && \date('d.m.Y H', \strtotime($_REQUEST['from'])) == \date('d.m.Y H')))) {
                    $params = $_REQUEST;
                ...
                if ($params && ('hours' == $params['type'] || 'minutes' == $params['type']) && isset($params['from']) && \strtotime($params['from']) && ((isset($params['to']) && \strtotime($params['to']) && \date('d.m.Y H', \strtotime($params['from'])) == \date('d.m.Y H', \strtotime($params['to']) - 1)) || (!isset($params['to']) && \date('d.m.Y H', \strtotime($params['from'])) == \date('d.m.Y H')))) {
                    unset($params['type']);
                    unset($params['pay']);
                    unset($params['order']);
                    echo '<a href="history.php?'.\http_build_query($params).'">';
                } elseif ($params) {
                    echo '<a href="reports_test_.php?'.\http_build_query($params).'">';
                }
                if ('process' == $key) {
                    $val = \number_format($val, 1, ',', '');
                }
