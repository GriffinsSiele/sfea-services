...
if ($theResult) {
    $theResult = \preg_replace("/<\?xml[^>]+>/", '', \strtr(\substr($theResult, \strpos($theResult, '<')), ['<request' => '<Request', '</request' => '</Request']));
}
if (!$theResult) {
    if (!isset($_REQUEST['mode']) || (isset($_REQUEST['mode']) && 'xml' != $_REQUEST['mode'] && 'json' != $_REQUEST['mode'])) {
