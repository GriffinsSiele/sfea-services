...
if (isset($_REQUEST['mode']) && 'xml' == $_REQUEST['mode']) {
    ...
    $doc = xml_transform(\strtr($theResult, ['request>' => 'Request>']), isset($_REQUEST['mode']) && 'pdf' == $_REQUEST['mode'] ? 'isphere_view_pdf_new.xslt' : 'isphere_view_new.xslt');
    if ($doc) {
        $servicename = isset($servicenames[$_SERVER['HTTP_HOST']]) ? 'платформой '.$servicenames[$_SERVER['HTTP_HOST']] : '';
        $html = \strtr($doc->saveHTML(), ['$servicename' => $servicename]);
            ...
            while ($i++ <= 3 && !$pdf) {
                ...
                if (\is_resource($process)) {
                    ...
                    if ($start) {
                        ...
                        if ($pdf) {
                            \file_put_contents('./logs/pdf/'.$_REQUEST['id'].'_'.\time().'.txt', $pdf);
                        } elseif ($i < 5) {
                            \sleep(5);
                        }
                        $pdf = false;
