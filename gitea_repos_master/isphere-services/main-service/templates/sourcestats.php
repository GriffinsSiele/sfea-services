...
$db = \mysqli_connect($database['server'], $database['login'], $database['password'], $database['name']) || exit(\mysqli_errno($db).': '.\mysqli_error($db));
if ($db) {
...
while ($row = $response->fetch_row()) {
    if ($row[3] >= 20) {
        $stats[$row[0]] = ['successrate' => (float) $row[1], 'avgtime' => (float) $row[2], 'interval' => $interval * 60];
    } else {
        $rare[] = "'".$row[0]."'";
    }
    $count[$row[0]] = $row[2];
...
while ($row = $response->fetch_row()) {
    if ($row[3] >= 20) {
        $stats[$row[0]] = ['successrate' => (float) $row[1], 'avgtime' => (float) $row[2], 'interval' => $rare_interval * 60];
    }
}
