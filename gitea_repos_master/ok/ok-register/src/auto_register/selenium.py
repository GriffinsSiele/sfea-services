...
class SeleniumDriver:
    ...
    def _create_profile(self):
        ...
        profile.set_preference("dom.webnotifications.serviceworker.enabled", self.serviceworker)
        profile.set_preference("dom.webnotifications.enabled", self.webnotifications)
    ...
    def _create_driver(self):
        driver = webdriver.Firefox(self.profile, options=self._create_options(), executable_path=self.executable_path)
    ...
    def click(self, tag, delay_before=0, delay_after=0):
        if not delay_before:
            ...
            element = WebDriverWait(self.driver, delay_before).until(EC.element_to_be_clickable(tag))
    ...
    def __str__(self):
        return f'SeleniumDriver(window_sizes: {self.window_sizes}, accept_languages={self.accept_languages}, proxy={self.proxy})'
