...
from src.fastapi.routers.default import default_router
from src.fastapi.routers.email import email_router
from src.fastapi.routers.phone import phone_router
from src.fastapi.schemas import SearchResponse
from src.fastapi.server import mFastAPI
...
@asynccontextmanager
async def lifespan(app: mFastAPI) -> AsyncGenerator:
    await proxy_cache_manager.get_proxy()
    yield


app = mFastAPI(title="holehe", docs_url=None, lifespan=lifespan)
...
async def prepare_not_found_response(exc: RequestValidationError) -> JSONResponse:
    ...
    source_incorrect_data = SearchResponse(
...
async def common_error(e: Exception) -> JSONResponse:
    ...
    logging.info(f'Start search for payload: "{payload}"')
    logging.warning(e)
    internal_error = SearchResponse(**KeyDBResponseBuilder.error(e))
    result = JSONResponse(
