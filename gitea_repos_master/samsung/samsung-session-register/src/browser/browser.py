...
Так как для получения сессии нужен не последний запрос браузера, а запрос из истории
использована библиотека seleniumwire, которая предоставляет функционал получения истории запросов.
"""
...
from src.interfaces import AbstractSeleniumWireProxy
...
class FirefoxBrowser:
    ...
    explicit_wait_delay = EXPLICIT_WAIT_DELAY
    ...
    def __init__(self, headless: bool = False) -> None:
        ...
        self.current_proxy_id: dict | None = None
        self.service = Service(
            executable_path=PUtils.bp(_root_path, "driver", "geckodriver")
        )
    ...
    async def start_browser(self) -> None:
        ...
        if self.seleniumwire_proxy_options:
            self.current_proxy_id = get(self.seleniumwire_proxy_options, "proxy.proxy_id")
        if self.headless:
    ...
    def get_proxy_id(self) -> dict | None:
