...
from src.fastapi.router import xiaomi_router
from src.fastapi.schemas import XiaomiSearchResponse
from src.fastapi.server import XiaomiFastAPI
...
app.include_router(xiaomi_router)
...
async def prepare_not_found_response(exc: RequestValidationError) -> JSONResponse:
    ...
    source_incorrect_data = XiaomiSearchResponse(
...
async def common_error(e: Exception) -> JSONResponse:
    ...
    """
    logging.warning(e)
    internal_error = XiaomiSearchResponse(**KeyDBResponseBuilder.error(e))
    result = JSONResponse(
