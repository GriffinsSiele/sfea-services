...
class ResponseSchemaValidationMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        try:
            ...
            if request.url.path.endswith("/api/v1/xiaomi/parse"):
                AggregatedResponse.model_validate_json(text)
            elif request.url.path.endswith("/parse") and "/api/v1/xiaomi/" in request.url.path:
...
def add_middlewares(app: FastAPI) -> None:
    ...
    # Optionally add Redis-backed rate limiting if configured
    ...
    if settings.redis_url and aioredis:
        ...
        class RedisRateLimitMiddleware(BaseHTTPMiddleware):
            async def dispatch(self, request: Request, call_next: Callable):
                ...
                key = f"ratelimit:{client_id}:{int(request.state.request_id[:8], 16) % window}"
